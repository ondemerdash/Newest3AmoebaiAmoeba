...
c
      ...
      real*8 elrc,vlrc
      real*8 t1mpi,t2mpi,t3mpi,t4mpi
...
c         call kewald
      ...
      if(taskid.lt.numtasks_emreal) then
        call cutoffs_ewald
      else if(taskid.ge.numtasks_emreal) then
        call cutoffs_vdw
      end if
...
c     &       mpi_comm_world,ierr)
            ...
            if(taskid.ge.numtasks_emreal) then
...
c             call gradient_emrecip_totfield
            end if
              call gradient_emrecip_totfield_send
c             call gradient_vdw_reduce_load_balanced

c            if(taskid.le.numtasks_emreal-1) then
c            call gradient_emreal_reduce_load_balanced_totfield_dEtensorV
c            end if
             call grad_vandr_reduce_totfield_dEtensor_vir
...
c            if(approxmode.eq.'1BODYMODE') then
              call ewtotfieldsmooth2_gradient_polar_1body_dEtensor_mpi
c            else
...
c            print*,"Before waitall"
c            call mpi_waitall(6,reqs,stats2,ierr)
c            print*,"After waitall"
c           call mpi_barrier(mpi_comm_world,ierr)
...
c                  print*,"dep3b_x",i,dep3b(1,i)
c                  print*,"dep3b_y",i,dep3b(2,i)
c                  print*,"dep3b_z",i,dep3b(3,i)
                 if (use(i) .and. mass(i).ne.0.0d0) then
                   speed = maxwell (mass(i),kelvin)
                   call ranvec (vec)
                   do j = 1, 3
...
c            t1mpi=mpi_Wtime()
     ...
     &      mpi_comm_world,ierr)
c             t2mpi=mpi_Wtime()
c            print*,"xyz bcast time",t2mpi-t1mpi,"taskid",taskid
           ! call mpi_bcast(rpole,13*n,mpi_real8,master,
     &     !  mpi_comm_world,ierr)
            call prep_pole
...
c      print*,"Timing vlist",tot_time
              if(firstload_vdw.and.taskid.eq.numtasks_emreal) then
c                firstload_vdw=.false.
                call vdw_load_balance_sizelist_half
c              else
c      call system_clock(t1,clock_rate)
c                call vdw_sizelist_only
c      call system_clock(t2,clock_rate)
c      tot_time =  (t2-t1)/real(clock_rate)
c      print*,"Timing vdw_sizelist_only",tot_time
              end if
...
c              else
c      call system_clock(t1,clock_rate)
c                call emreal_sizelist_only
c      call system_clock(t2,clock_rate)
c      tot_time =  (t2-t1)/real(clock_rate)
c      print*,"Timing emreal_sizelist_only",tot_time
              end if
c            else if(taskid.eq.master2
c     &              .and.(approxmode.ne.'1BODYMODE')) then
c                call mollist2bodyOO6_8_par
c
c               if(firstload_polz) then
c                firstload_polz = .false.
c                call ewpolar_load_balance 
...
c            t2mpi=mpi_Wtime() 
c            print*,"emrecip1 bcast time",t2mpi-t1mpi,"taskid",taskid
            if(taskid.eq.master) then
                 call empole1d_3b_Perm_selfeng_bcast
