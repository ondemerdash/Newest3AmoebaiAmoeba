...
c
     ...
     &  ntpair_dEtmp,dEindextmp,dEd1tmp,dEd2tmp,dEp1tmp,dEp2tmp)
      use sizes
      use atoms
      use boxes
      use chgpot
...
!$    integer omp_get_thread_num
      ...
      integer ik,ierr
      logical done_ik
      integer ntpair_dEtmp,dEindextmp(2,*)
      real*8 dEd1tmp(9,*),dEd2tmp(9,*),dEp1tmp(9,*),dEp2tmp(9,*)
      external erfc
c
...
c      if(allocated(dEindextmp)) deallocate(dEindextmp) 
c      if(allocated(dEd1tmp)) deallocate(dEd1tmp)
c      if(allocated(dEd2tmp)) deallocate(dEd2tmp)
c      if(allocated(dEp1tmp)) deallocate(dEp1tmp)
c      if(allocated(dEp2tmp)) deallocate(dEp2tmp)
c      maxlocal_allthread=int(dble(sz)*dble(maxsize_elst(taskid)))
c      allocate(dEindextmp(2,maxlocal_allthread))
c      allocate(dEd1tmp(9,maxlocal_allthread))
c      allocate(dEd2tmp(9,maxlocal_allthread))
c      allocate(dEp1tmp(9,maxlocal_allthread))
c      allocate(dEp2tmp(9,maxlocal_allthread))

      print*,"taskid=",taskid,"maxlocal=",maxlocal
      print*,"taskid=",taskid,"sz=",sz
      print*,"maxsize_elst(taskid)=",maxsize_elst(taskid)
      print*,"MPIOMPTHG VERSION"
c
...
!$OMP& done_ik,ik,tau1p,tau1d,tau2p,tau2d,tid,
...
!$OMP& tauzx2p,tauzy2p,tauzz2p,nlocal)
...
!$    tid = omp_get_thread_num ()
      ...
      !ntpair_dEtmp = nlocal
      print*,"taskid=",taskid,"ntpair_dEtmp=",ntpair_dEtmp
!$OMP END CRITICAL
         k = toffset(tid)
         do i = 1, nlocal
            m = k + i
...
c        if(taskid.ne.master) then
c          call mpi_send(ntpair_dEtmp,1,mpi_integer,master,
c     &                 6*taskid,emreal_comm,ierr)
c          call mpi_send(dEindextmp,2*ntpair_dEtmp,mpi_integer,master,
c     &                 6*taskid+1,emreal_comm,ierr)
c          call mpi_send(dEd1tmp,9*ntpair_dEtmp,mpi_real8,master,
c     &                 6*taskid+2,emreal_comm,ierr)
c          call mpi_send(dEd2tmp,9*ntpair_dEtmp,mpi_real8,master,
c     &                 6*taskid+3,emreal_comm,ierr)
c          call mpi_send(dEp1tmp,9*ntpair_dEtmp,mpi_real8,master,
c     &                 6*taskid+4,emreal_comm,ierr)
c          call mpi_send(dEp2tmp,9*ntpair_dEtmp,mpi_real8,master,
c     &                 6*taskid+5,emreal_comm,ierr)
c        end if
      return
      end
