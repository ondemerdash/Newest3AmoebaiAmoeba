c
      subroutine emrecip1_3b_Perm2_noqpole_nodpole
      use sizes
      use atoms
      use bound
      use boxes
...
c
      print*,"NO Quadrupole NO Dipole Version"
...
c      allocate (cmp(4,npole))
c      allocate (fmp(4,npole))
c      allocate (fphi(10,npole))
c      allocate (cphi(4,npole))

      allocate (cmp(npole))
      allocate (fphi(4,npole))
      allocate (cphi(npole))
...
c      if (.not. use_polar) then
       !  call bspline_fill_noqpole_nodpole
         call bspline_fill_omp
         !print*,"After bspline_fill"
         call table_fill_omp
c      end if
...
c         end do
c      else
       !  call cmp_to_fmp_noqpole_nodpole (cmp,fmp)

         call grid_mpole_noqpole_nodpole (cmp)
         !print*,"After grid_mpole_noqpole_nodpole"
         call fftfront
...
c
      call fftback
      call fphi_mpole_noqpole_nodpole (fphi)
      !print*,"After fphi_mpole_noqpole_nodpole"
      do i = 1, npole
c         do j = 1, 20
        ! do j = 1, 10
         do j = 1, 4
...
c      call fphi_to_cphi_noqpole_nodpole (fphi,cphi)
...
c         do k = 1, 10
        ...
        ! end do
           e = e + cmp(i)*fphi(1,i)
           f1 = f1 + cmp(i)*fphi(deriv1,i)
           f2 = f2 + cmp(i)*fphi(deriv2,i)
           f3 = f3 + cmp(i)*fphi(deriv3,i)
