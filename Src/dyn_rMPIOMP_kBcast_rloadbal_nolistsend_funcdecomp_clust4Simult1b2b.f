...
c        if(.not.allocated(nmollst3)) allocate(nmollst3(clustcount))
c        if(.not.allocated(mollst3)) 
c     &         allocate(mollst3(5*clustcount,clustcount))
c        if(.not.allocated(save2bfor3b)) 
c     &         allocate(save2bfor3b(clustcount,clustcount))
c        if(.not.allocated(ind2b_counter))
c     &      allocate(ind2b_counter(clustcount,clustcount))
...
c                 call polar_load_balance_clust_splitlist_offset3b_simple
              end if
           end if
...
c               call bcast_polar_load_balance_clust_splitlist3b
c               call mpi_bcast(nmollst3,clustcount,mpi_integer,master,
c     &              mpi_comm_world,ierr)
c               call mpi_bcast(mollst3,5*clustcount*clustcount,
c     &              mpi_integer,master,mpi_comm_world,ierr)
c               call mpi_bcast(save2bfor3b,clustcount*clustcount,
c     &              mpi_logical,master,mpi_comm_world,ierr)
c              call mpi_bcast(num2bsave,1,mpi_integer,master,
c     &              mpi_comm_world,ierr)
c              call mpi_bcast(ind2b_counter,clustcount*clustcount,
c     &              mpi_integer,master,mpi_comm_world,ierr)
...
c
c        if(.not. allocated(ep2bmatmod))
c     &           allocate(ep2bmatmod(num2bsave))
c        if(.not. allocated(dep2bmatmod)) allocate(dep2bmatmod(3,
c     &          2*3*maxsizeclust,num2bsave))
c        if(.not. allocated(virep2bmatmod)) allocate(virep2bmatmod(3,
c     &          3,num2bsave))
c             call init2bmatmodPolar
c
c       end if
...
c           if( (taskid.ge.(numtasks_polar+numtasks_polar1)).and.
c     &         (taskid.lt.
c     &         (numtasks_polar+numtasks_polar1+numtasks_polar3b))
c     &         .and.mlistclust) then 
c              call clust3blists_offset
c              print*,"Completed clust3blists_offset"
c              if(use_pred) then
c                call initpolarpred3b_offset
c              print*,"Completed initpolarpred3b_offset"
c              end if
c           end if
...
c            deallocate(taskidemreal)
          ...
          call clust_gradient_Polar1b2bsimult_ireducematcommsmall2(
     &      start,start+offset-1, moli1rmndr)
...
c             call mpi_wait(reqs28,stat,ierr)
c             call mpi_wait(reqs29,stat,ierr)
c             call mpi_wait(reqs30,stat,ierr)
      end if
...
c             call mpi_wait(reqs31,stat,ierr)
c             call mpi_wait(reqs32,stat,ierr)
c             call mpi_wait(reqs33,stat,ierr)
c      end if
      call sum_gradient_Polar1b2bsimult_ireducematcommsmall2
