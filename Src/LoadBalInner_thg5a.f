...
c
c
      subroutine LoadBal_ereal1d_3b_Perm_sentlist2_totfield_dEtensor(
...
c
      ...
      print*,"start=",start_emreal2(taskid)
      print*,"last=",last_emreal2(taskid)
...
c!$OMP& ftm2,ftm2r,ttm2,ttm3,fact1,fact2,fact3,fact4,
...
c!$OMP DO reduction(+:emrealt,viremrealt,demi,demk,fieldt,fieldtp)
...
c
C 1ST,4th,5th TERM W/ SCREENING
c
            fact1=0.5d0*bn(2)
            t2xxd = t2xxd + fact1*(dk(1)*xr+xr*(ck*xr + dk(1)))
            t2xyd = t2xyd + fact1*(dk(1)*yr+xr*(ck*yr + dk(2)))
            t2xzd = t2xzd + fact1*(dk(1)*zr+xr*(ck*zr + dk(3)))

            t2yxd = t2yxd + fact1*(dk(2)*xr+yr*(ck*xr + dk(1)))
            t2yyd = t2yyd + fact1*(dk(2)*yr+yr*(ck*yr + dk(2)))
            t2yzd = t2yzd + fact1*(dk(2)*zr+yr*(ck*zr + dk(3)))

            t2zxd = t2zxd + fact1*(dk(3)*xr+zr*(ck*xr + dk(1)))
            t2zyd = t2zyd + fact1*(dk(3)*yr+zr*(ck*yr + dk(2)))
            t2zzd = t2zzd + fact1*(dk(3)*zr+zr*(ck*zr + dk(3)))

            t1xxd = t1xxd - fact1*(-di(1)*xr+xr*(ci*xr - di(1)))
            t1xyd = t1xyd - fact1*(-di(1)*yr+xr*(ci*yr - di(2)))
            t1xzd = t1xzd - fact1*(-di(1)*zr+xr*(ci*zr - di(3)))

            t1yxd = t1yxd - fact1*(-di(2)*xr+yr*(ci*xr - di(1)))
            t1yyd = t1yyd - fact1*(-di(2)*yr+yr*(ci*yr - di(2)))
            t1yzd = t1yzd - fact1*(-di(2)*zr+yr*(ci*zr - di(3)))

            t1zxd = t1zxd - fact1*(-di(3)*xr+zr*(ci*xr - di(1)))
            t1zyd = t1zyd - fact1*(-di(3)*yr+zr*(ci*yr - di(2)))
            t1zzd = t1zzd - fact1*(-di(3)*zr+zr*(ci*zr - di(3)))

c
C 1st and 2ND TERM W SCREENING
c
            gfi(2) = 0.5d0*(-ck*bn(1) + sc(4)*bn(2) - sc(6)*bn(3))
            fact1=-0.5d0*bn(3)*sc(4)
            fact2=0.5d0*bn(4)*sc(6)
            t2xxd = t2xxd + xr*(-bn(3)*qkr(1)+(fact1+fact2)*xr)+gfi(2)
            t2xyd = t2xyd + xr*(-bn(3)*qkr(2)+(fact1+fact2)*yr)
            t2xzd = t2xzd + xr*(-bn(3)*qkr(3)+(fact1+fact2)*zr)

            t2yxd = t2yxd + yr*(-bn(3)*qkr(1)+(fact1+fact2)*xr)
            t2yyd = t2yyd + yr*(-bn(3)*qkr(2)+(fact1+fact2)*yr)+gfi(2)
            t2yzd = t2yzd + yr*(-bn(3)*qkr(3)+(fact1+fact2)*zr)

            t2zxd = t2zxd + zr*(-bn(3)*qkr(1)+(fact1+fact2)*xr)
            t2zyd = t2zyd + zr*(-bn(3)*qkr(2)+(fact1+fact2)*yr)
            t2zzd = t2zzd + zr*(-bn(3)*qkr(3)+(fact1+fact2)*zr)+gfi(2)
c
C 1st and 3RD TERM W SCREENING
c
            gfi(3) = 0.5d0*(ci*bn(1) + sc(3)*bn(2) + sc(5)*bn(3))
            fact1=-0.5d0*bn(3)*sc(3)
            fact2=-0.5d0*bn(4)*sc(5)
            t1xxd = t1xxd + xr*(bn(3)*qir(1)+(fact1+fact2)*xr)+gfi(3)
            t1xyd = t1xyd + xr*(bn(3)*qir(2)+(fact1+fact2)*yr)
            t1xzd = t1xzd + xr*(bn(3)*qir(3)+(fact1+fact2)*zr)

            t1yxd = t1yxd + yr*(bn(3)*qir(1)+(fact1+fact2)*xr)
            t1yyd = t1yyd + yr*(bn(3)*qir(2)+(fact1+fact2)*yr)+gfi(3)
            t1yzd = t1yzd + yr*(bn(3)*qir(3)+(fact1+fact2)*zr)

            t1zxd = t1zxd + zr*(bn(3)*qir(1)+(fact1+fact2)*xr)
            t1zyd = t1zyd + zr*(bn(3)*qir(2)+(fact1+fact2)*yr)
            t1zzd = t1zzd + zr*(bn(3)*qir(3)+(fact1+fact2)*zr)+gfi(3)
c
C 6TH,7th,8th TERM W SCREENING
c
            t2xxd = t2xxd + bn(2)*qk(1) - bn(3)*qkr(1)*xr
            t2xyd = t2xyd + bn(2)*qk(4) - bn(3)*qkr(1)*yr
            t2xzd = t2xzd + bn(2)*qk(7) - bn(3)*qkr(1)*zr

            t2yxd = t2yxd + bn(2)*qk(2) - bn(3)*qkr(2)*xr
            t2yyd = t2yyd + bn(2)*qk(5) - bn(3)*qkr(2)*yr
            t2yzd = t2yzd + bn(2)*qk(8) - bn(3)*qkr(2)*zr

            t2zxd = t2zxd + bn(2)*qk(3) - bn(3)*qkr(3)*xr
            t2zyd = t2zyd + bn(2)*qk(6) - bn(3)*qkr(3)*yr
            t2zzd = t2zzd + bn(2)*qk(9) - bn(3)*qkr(3)*zr

            t1xxd = t1xxd - bn(2)*qi(1) + bn(3)*qir(1)*xr
            t1xyd = t1xyd - bn(2)*qi(4) + bn(3)*qir(1)*yr
            t1xzd = t1xzd - bn(2)*qi(7) + bn(3)*qir(1)*zr

            t1yxd = t1yxd - bn(2)*qi(2) + bn(3)*qir(2)*xr
            t1yyd = t1yyd - bn(2)*qi(5) + bn(3)*qir(2)*yr
            t1yzd = t1yzd - bn(2)*qi(8) + bn(3)*qir(2)*zr

            t1zxd = t1zxd - bn(2)*qi(3) + bn(3)*qir(3)*xr
            t1zyd = t1zyd - bn(2)*qi(6) + bn(3)*qir(3)*yr
            t1zzd = t1zzd - bn(2)*qi(9) + bn(3)*qir(3)*zr
...
C 1ST and 2nd TERM W/OUT SCREENING
c
            gfri_d(2) =-0.5d0*(-rr3*ck*dsc3+rr5*sc(4)*dsc5
     &                 -rr7*sc(6)*dsc7)
            gfri_p(2) =-0.5d0*(-rr3*ck*psc3+rr5*sc(4)*psc5
     &                  -rr7*sc(6)*psc7)
            fact1=-0.5d0*rr5*dsc3         
            fact2=-0.5d0*rr5*psc3         
            t2xxd = t2xxd + fact1*xr*(ck*xr + dk(1)) + gfri_d(2)
            t2xxp = t2xxp + fact2*xr*(ck*xr + dk(1)) + gfri_p(2)
            ...
            t2yyd = t2yyd + fact1*yr*(ck*yr + dk(2))  + gfri_d(2)
            t2yyp = t2yyp + fact2*yr*(ck*yr + dk(2))  + gfri_p(2)
            ...
            t2zzd = t2zzd + fact1*zr*(ck*zr + dk(3))  + gfri_d(2)
            t2zzp = t2zzp + fact2*zr*(ck*zr + dk(3))  + gfri_p(2)
c
C 1st and 3RD TERM W/OUT SCREENING
c
            gfri_d(3) =-0.5d0*(rr3*ci*dsc3+rr5*sc(3)*dsc5
     &                   +rr7*sc(5)*dsc7)
            gfri_p(3) =-0.5d0*(rr3*ci*psc3+rr5*sc(3)*psc5
     &                   +rr7*sc(5)*psc7)
            t1xxd = t1xxd - fact1*xr*(ci*xr - di(1)) + gfri_d(3)
            t1xxp = t1xxp - fact2*xr*(ci*xr - di(1)) + gfri_p(3)
            ...
            t1yyd = t1yyd - fact1*yr*(ci*yr - di(2)) + gfri_d(3)
            t1yyp = t1yyp - fact2*yr*(ci*yr - di(2)) + gfri_p(3)
            ...
            t1zzd = t1zzd - fact1*zr*(ci*zr - di(3)) + gfri_d(3)
            t1zzp = t1zzp - fact2*zr*(ci*zr - di(3)) + gfri_p(3)

            fact1=0.5d0*rr7*dsc5*sc(3)
            fact2=0.5d0*rr7*psc5*sc(3)
            fact3=0.5d0*rr9*dsc7*sc(5)
            fact4=0.5d0*rr9*psc7*sc(5)
            t1xxd = t1xxd + (fact1+fact3)*xr*xr
            t1xxp = t1xxp + (fact2+fact4)*xr*xr
            t1xyd = t1xyd + (fact1+fact3)*xr*yr
            t1xyp = t1xyp + (fact2+fact4)*xr*yr
            t1xzd = t1xzd + (fact1+fact3)*xr*zr
            t1xzp = t1xzp + (fact2+fact4)*xr*zr

            t1yxd = t1yxd + (fact1+fact3)*yr*xr
            t1yxp = t1yxp + (fact2+fact4)*yr*xr
            t1yyd = t1yyd + (fact1+fact3)*yr*yr
            t1yyp = t1yyp + (fact2+fact4)*yr*yr
            t1yzd = t1yzd + (fact1+fact3)*yr*zr
            t1yzp = t1yzp + (fact2+fact4)*yr*zr

            t1zxd = t1zxd + (fact1+fact3)*zr*xr
            t1zxp = t1zxp + (fact2+fact4)*zr*xr
            t1zyd = t1zyd + (fact1+fact3)*zr*yr
            t1zyp = t1zyp + (fact2+fact4)*zr*yr
            t1zzd = t1zzd + (fact1+fact3)*zr*zr
            t1zzp = t1zzp + (fact2+fact4)*zr*zr

            fact1=0.5d0*rr7*dsc5*sc(4)
            fact2=0.5d0*rr7*psc5*sc(4)
            fact3=-0.5d0*rr9*dsc7*sc(6)
            fact4=-0.5d0*rr9*psc7*sc(6)
            t2xxd = t2xxd + (fact1+fact3)*xr*xr
            t2xxp = t2xxp + (fact2+fact4)*xr*xr
            t2xyd = t2xyd + (fact1+fact3)*xr*yr
            t2xyp = t2xyp + (fact2+fact4)*xr*yr
            t2xzd = t2xzd + (fact1+fact3)*xr*zr
            t2xzp = t2xzp + (fact2+fact4)*xr*zr

            t2yxd = t2yxd + (fact1+fact3)*yr*xr
            t2yxp = t2yxp + (fact2+fact4)*yr*xr
            t2yyd = t2yyd + (fact1+fact3)*yr*yr
            t2yyp = t2yyp + (fact2+fact4)*yr*yr
            t2yzd = t2yzd + (fact1+fact3)*yr*zr
            t2yzp = t2yzp + (fact2+fact4)*yr*zr

            t2zxd = t2zxd + (fact1+fact3)*zr*xr
            t2zxp = t2zxp + (fact2+fact4)*zr*xr
            t2zyd = t2zyd + (fact1+fact3)*zr*yr
            t2zyp = t2zyp + (fact2+fact4)*zr*yr
            t2zzd = t2zzd + (fact1+fact3)*zr*zr
            t2zzp = t2zzp + (fact2+fact4)*zr*zr
c
...
c
            ...
            fact3=-rr5*dsc5
            fact4=-rr5*psc5
            t1xxd = t1xxd + fact1*qir(1)*xr-fact3*qi(1)
            t1xxp = t1xxp + fact2*qir(1)*xr-fact4*qi(1)
            t1xyd = t1xyd + fact1*qir(2)*xr-fact3*qi(4)
            t1xyp = t1xyp + fact2*qir(2)*xr-fact4*qi(4)
            t1xzd = t1xzd + fact1*qir(3)*xr-fact3*qi(7)
            t1xzp = t1xzp + fact2*qir(3)*xr-fact4*qi(7)

            t1yxd = t1yxd + fact1*qir(1)*yr-fact3*qi(2)
            t1yxp = t1yxp + fact2*qir(1)*yr-fact4*qi(2)
            t1yyd = t1yyd + fact1*qir(2)*yr-fact3*qi(5)
            t1yyp = t1yyp + fact2*qir(2)*yr-fact4*qi(5)
            t1yzd = t1yzd + fact1*qir(3)*yr-fact3*qi(8)
            t1yzp = t1yzp + fact2*qir(3)*yr-fact4*qi(8)

            t1zxd = t1zxd + fact1*qir(1)*zr-fact3*qi(3)
            t1zxp = t1zxp + fact2*qir(1)*zr-fact4*qi(3)
            t1zyd = t1zyd + fact1*qir(2)*zr-fact3*qi(6)
            t1zyp = t1zyp + fact2*qir(2)*zr-fact4*qi(6)
            t1zzd = t1zzd + fact1*qir(3)*zr-fact3*qi(9)
            t1zzp = t1zzp + fact2*qir(3)*zr-fact4*qi(9)

            t2xxd = t2xxd - fact1*qkr(1)*xr+fact3*qk(1)
            t2xxp = t2xxp - fact2*qkr(1)*xr+fact4*qk(1)
            t2xyd = t2xyd - fact1*qkr(2)*xr+fact3*qk(4)
            t2xyp = t2xyp - fact2*qkr(2)*xr+fact4*qk(4)
            t2xzd = t2xzd - fact1*qkr(3)*xr+fact3*qk(7)
            t2xzp = t2xzp - fact2*qkr(3)*xr+fact4*qk(7)

            t2yxd = t2yxd - fact1*qkr(1)*yr+fact3*qk(2)
            t2yxp = t2yxp - fact2*qkr(1)*yr+fact4*qk(2)
            t2yyd = t2yyd - fact1*qkr(2)*yr+fact3*qk(5)
            t2yyp = t2yyp - fact2*qkr(2)*yr+fact4*qk(5)
            t2yzd = t2yzd - fact1*qkr(3)*yr+fact3*qk(8)
            t2yzp = t2yzp - fact2*qkr(3)*yr+fact4*qk(8)

            t2zxd = t2zxd - fact1*qkr(1)*zr+fact3*qk(3)
            t2zxp = t2zxp - fact2*qkr(1)*zr+fact4*qk(3)
            t2zyd = t2zyd - fact1*qkr(2)*zr+fact3*qk(6)
            t2zyp = t2zyp - fact2*qkr(2)*zr+fact4*qk(6)
            t2zzd = t2zzd - fact1*qkr(3)*zr+fact3*qk(9)
            t2zzp = t2zzp - fact2*qkr(3)*zr+fact4*qk(9)

C 1st and 7TH TERM W/OUT SCREENING
c
            fact1=-0.5d0*rr5*dsc5
            fact2=-0.5d0*rr5*psc5
            fact3=-rr7*dsc7
            fact4=-rr7*psc7
            t1xxd = t1xxd + (fact1*di(1)+fact3*qir(1))*xr
            t1xxp = t1xxp + (fact2*di(1)+fact4*qir(1))*xr
            t1xyd = t1xyd + (fact1*di(1)+fact3*qir(1))*yr
            t1xyp = t1xyp + (fact2*di(1)+fact4*qir(1))*yr
            t1xzd = t1xzd + (fact1*di(1)+fact3*qir(1))*zr
            t1xzp = t1xzp + (fact2*di(1)+fact4*qir(1))*zr

            t1yxd = t1yxd + (fact1*di(2)+fact3*qir(2))*xr
            t1yxp = t1yxp + (fact2*di(2)+fact4*qir(2))*xr
            t1yyd = t1yyd + (fact1*di(2)+fact3*qir(2))*yr
            t1yyp = t1yyp + (fact2*di(2)+fact4*qir(2))*yr
            t1yzd = t1yzd + (fact1*di(2)+fact3*qir(2))*zr
            t1yzp = t1yzp + (fact2*di(2)+fact4*qir(2))*zr

            t1zxd = t1zxd + (fact1*di(3)+fact3*qir(3))*xr
            t1zxp = t1zxp + (fact2*di(3)+fact4*qir(3))*xr
            t1zyd = t1zyd + (fact1*di(3)+fact3*qir(3))*yr
            t1zyp = t1zyp + (fact2*di(3)+fact4*qir(3))*yr
            t1zzd = t1zzd + (fact1*di(3)+fact3*qir(3))*zr
            t1zzp = t1zzp + (fact2*di(3)+fact4*qir(3))*zr

            t2xxd = t2xxd + (fact1*dk(1)+fact3*qkr(1))*xr
            t2xxp = t2xxp + (fact2*dk(1)+fact4*qkr(1))*xr
            t2xyd = t2xyd + (fact1*dk(1)+fact3*qkr(1))*yr
            t2xyp = t2xyp + (fact2*dk(1)+fact4*qkr(1))*yr
            t2xzd = t2xzd + (fact1*dk(1)+fact3*qkr(1))*zr
            t2xzp = t2xzp + (fact2*dk(1)+fact4*qkr(1))*zr

            t2yxd = t2yxd + (fact1*dk(2)+fact3*qkr(2))*xr
            t2yxp = t2yxp + (fact2*dk(2)+fact4*qkr(2))*xr
            t2yyd = t2yyd + (fact1*dk(2)+fact3*qkr(2))*yr
            t2yyp = t2yyp + (fact2*dk(2)+fact4*qkr(2))*yr
            t2yzd = t2yzd + (fact1*dk(2)+fact3*qkr(2))*zr
            t2yzp = t2yzp + (fact2*dk(2)+fact4*qkr(2))*zr

            t2zxd = t2zxd + (fact1*dk(3)+fact3*qkr(3))*xr
            t2zxp = t2zxp + (fact2*dk(3)+fact4*qkr(3))*xr
            t2zyd = t2zyd + (fact1*dk(3)+fact3*qkr(3))*yr
            t2zyp = t2zyp + (fact2*dk(3)+fact4*qkr(3))*yr
            t2zzd = t2zzd + (fact1*dk(3)+fact3*qkr(3))*zr
...
c
            ...
            t2xxd = t2xxd + fact1*(ck*xr + dk(1))
            t2xxp = t2xxp + fact2*(ck*xr + dk(1)) 
            t2xyd = t2xyd + fact1*(ck*yr + dk(2)) 
            t2xyp = t2xyp + fact2*(ck*yr + dk(2)) 
            t2xzd = t2xzd + fact1*(ck*zr + dk(3)) 
            t2xzp = t2xzp + fact2*(ck*zr + dk(3)) 
            t1xxd = t1xxd + fact1*(-ci*xr + di(1)) 
            t1xxp = t1xxp + fact2*(-ci*xr + di(1)) 
            t1xyd = t1xyd + fact1*(-ci*yr + di(2)) 
            t1xyp = t1xyp + fact2*(-ci*yr + di(2)) 
            t1xzd = t1xzd + fact1*(-ci*zr + di(3)) 
            t1xzp = t1xzp + fact2*(-ci*zr + di(3)) 

            fact1=-0.5d0*rr3*dscale(kk)*ddsc3(2)
            fact2=-0.5d0*rr3*pscale(kk)*ddsc3(2)
            t2yxd = t2yxd + fact1*(ck*xr + dk(1)) 
            t2yxp = t2yxp + fact2*(ck*xr + dk(1)) 
            t2yyd = t2yyd + fact1*(ck*yr + dk(2)) 
            t2yyp = t2yyp + fact2*(ck*yr + dk(2)) 
            t2yzd = t2yzd + fact1*(ck*zr + dk(3)) 
            t2yzp = t2yzp + fact2*(ck*zr + dk(3)) 
            t1yxd = t1yxd + fact1*(-ci*xr + di(1)) 
            t1yxp = t1yxp + fact2*(-ci*xr + di(1)) 
            t1yyd = t1yyd + fact1*(-ci*yr + di(2)) 
            t1yyp = t1yyp + fact2*(-ci*yr + di(2)) 
            t1yzd = t1yzd + fact1*(-ci*zr + di(3)) 
            t1yzp = t1yzp + fact2*(-ci*zr + di(3)) 

            fact1=-0.5d0*rr3*dscale(kk)*ddsc3(3)
            fact2=-0.5d0*rr3*pscale(kk)*ddsc3(3)
            t2zxd = t2zxd + fact1*(ck*xr + dk(1)) 
            t2zxp = t2zxp + fact2*(ck*xr + dk(1)) 
            t2zyd = t2zyd + fact1*(ck*yr + dk(2)) 
            t2zyp = t2zyp + fact2*(ck*yr + dk(2)) 
            t2zzd = t2zzd + fact1*(ck*zr + dk(3)) 
            t2zzp = t2zzp + fact2*(ck*zr + dk(3)) 
            t1zxd = t1zxd + fact1*(-ci*xr + di(1)) 
            t1zxp = t1zxp + fact2*(-ci*xr + di(1)) 
            t1zyd = t1zyd + fact1*(-ci*yr + di(2)) 
            t1zyp = t1zyp + fact2*(-ci*yr + di(2)) 
            t1zzd = t1zzd + fact1*(-ci*zr + di(3)) 
            t1zzp = t1zzp + fact2*(-ci*zr + di(3)) 
c         
C  TENSOR TERMS CORRESPONDING TO TEMP5 FOR 'fridmp(j)' terms
c
            fact1=0.5d0*rr5*dscale(kk)*sc(3)*ddsc5(1)
            fact2=0.5d0*rr5*pscale(kk)*sc(3)*ddsc5(1)
            fact3=0.5d0*rr7*dscale(kk)*sc(5)*ddsc7(1)
            fact4=0.5d0*rr7*pscale(kk)*sc(5)*ddsc7(1)
            t1xxd = t1xxd + (fact1+fact3)*xr 
            t1xxp = t1xxp + (fact2+fact4)*xr 
            t1xyd = t1xyd + (fact1+fact3)*yr 
            t1xyp = t1xyp + (fact2+fact4)*yr 
            t1xzd = t1xzd + (fact1+fact3)*zr 
            t1xzp = t1xzp + (fact2+fact4)*zr 

            fact1=fact1*ddsc5(2)/ddsc5(1)
            fact2=fact2*ddsc5(2)/ddsc5(1)
            fact3=fact3*ddsc7(2)/ddsc7(1)
            fact4=fact4*ddsc7(2)/ddsc7(1)
            t1yxd = t1yxd + (fact1+fact3)*xr 
            t1yxp = t1yxp + (fact2+fact4)*xr 
            t1yyd = t1yyd + (fact1+fact3)*yr 
            t1yyp = t1yyp + (fact2+fact4)*yr 
            t1yzd = t1yzd + (fact1+fact3)*zr 
            t1yzp = t1yzp + (fact2+fact4)*zr 

            fact1=fact1*ddsc5(3)/ddsc5(2)
            fact2=fact2*ddsc5(3)/ddsc5(2)
            fact3=fact3*ddsc7(3)/ddsc7(2)
            fact4=fact4*ddsc7(3)/ddsc7(2)
            t1zxd = t1zxd + (fact1+fact3)*xr 
            t1zxp = t1zxp + (fact2+fact4)*xr 
            t1zyd = t1zyd + (fact1+fact3)*yr 
            t1zyp = t1zyp + (fact2+fact4)*yr 
            t1zzd = t1zzd + (fact1+fact3)*zr 
            t1zzp = t1zzp + (fact2+fact4)*zr 

            fact1=0.5d0*rr5*dscale(kk)*sc(4)*ddsc5(1)
            fact2=0.5d0*rr5*pscale(kk)*sc(4)*ddsc5(1)
            fact3=-0.5d0*rr7*dscale(kk)*sc(6)*ddsc7(1)
            fact4=-0.5d0*rr7*pscale(kk)*sc(6)*ddsc7(1)
            t2xxd = t2xxd + (fact1+fact3)*xr 
            t2xxp = t2xxp + (fact2+fact4)*xr 
            t2xyd = t2xyd + (fact1+fact3)*yr 
            t2xyp = t2xyp + (fact2+fact4)*yr 
            t2xzd = t2xzd + (fact1+fact3)*zr 
            t2xzp = t2xzp + (fact2+fact4)*zr 

            fact1=fact1*ddsc5(2)/ddsc5(1)
            fact2=fact2*ddsc5(2)/ddsc5(1)
            fact3=fact3*ddsc7(2)/ddsc7(1)
            fact4=fact4*ddsc7(2)/ddsc7(1)
            t2yxd = t2yxd + (fact1+fact3)*xr 
            t2yxp = t2yxp + (fact2+fact4)*xr 
            t2yyd = t2yyd + (fact1+fact3)*yr 
            t2yyp = t2yyp + (fact2+fact4)*yr 
            t2yzd = t2yzd + (fact1+fact3)*zr 
            t2yzp = t2yzp + (fact2+fact4)*zr 

            fact1=fact1*ddsc5(3)/ddsc5(2)
            fact2=fact2*ddsc5(3)/ddsc5(2)
            fact3=fact3*ddsc7(3)/ddsc7(2)
            fact4=fact4*ddsc7(3)/ddsc7(2)
            t2zxd = t2zxd + (fact1+fact3)*xr 
            t2zxp = t2zxp + (fact2+fact4)*xr 
            t2zyd = t2zyd + (fact1+fact3)*yr 
            t2zyp = t2zyp + (fact2+fact4)*yr 
            t2zzd = t2zzd + (fact1+fact3)*zr 
            t2zzp = t2zzp + (fact2+fact4)*zr 
