...
c     &   eptemp,deptemp2,virtemp)
      subroutine ereal1c1_3b_totfieldnpole3b(npole3b,pnum,uind,uinp,
...
c
      ...
      mode = 'EWALD'
      call switch (mode)
      ! print*,"use_replica=",use_replica,off2
...
c               gfri(6) = -rr7 * (sci(3)*psc7+scip(3)*dsc7)
     ...
     &                 + 0.5d0*rr9*(gli(3)*psc7)
               ...
               gfri(4) = 2.0d0 * rr5
               gfri(5) = rr7 * (sci(4)*psc7)
               gfri(6) = -rr7 * (sci(3)*psc7)
c
...
c               gtri(6) = gfri(6)
               ...
               gti(2) = 0.5d0 * bn(2) * (sci(4))
               gti(3) = 0.5d0 * bn(2) * (sci(3))
               gti(4) = gfi(4)
               gti(5) = gfi(5)
               gti(6) = gfi(6)
               gtri(2) = 0.5d0 * rr5 * (sci(4)*psc5)
               gtri(3) = 0.5d0 * rr5 * (sci(3)*psc5)
               gtri(4) = gfri(4)
               gtri(5) = gfri(5)
               gtri(6) = gfri(6)
...
c     &           +(uixqkrp(3)+rxqkuip(3))*dsc5)*0.5d0 - gtri(6)*rxqkr(3)
               ...
               ttm2ri(1) = -rr3*(dixuk(1)*psc3)*0.5d0
     &           + gtri(2)*dixr(1) + gtri(4)*((ukxqir(1)+rxqiuk(1))*psc5
     &           )*0.5d0 - gtri(5)*rxqir(1)
               ttm2ri(2) = -rr3*(dixuk(2)*psc3)*0.5d0
     &           + gtri(2)*dixr(2) + gtri(4)*((ukxqir(2)+rxqiuk(2))*psc5
     &           )*0.5d0 - gtri(5)*rxqir(2)
               ttm2ri(3) = -rr3*(dixuk(3)*psc3)*0.5d0
     &           + gtri(2)*dixr(3) + gtri(4)*((ukxqir(3)+rxqiuk(3))*psc5
     &           )*0.5d0 - gtri(5)*rxqir(3)
               ttm3ri(1) = -rr3*(dkxui(1)*psc3)*0.5d0
     &           + gtri(3)*dkxr(1) - gtri(4)*((uixqkr(1)+rxqkui(1))*psc5
     &           )*0.5d0 - gtri(6)*rxqkr(1)
               ttm3ri(2) = -rr3*(dkxui(2)*psc3)*0.5d0
     &           + gtri(3)*dkxr(2) - gtri(4)*((uixqkr(2)+rxqkui(2))*psc5
     &           )*0.5d0 - gtri(6)*rxqkr(2)
               ttm3ri(3) = -rr3*(dkxui(3)*psc3)*0.5d0
     &           + gtri(3)*dkxr(3) - gtri(4)*((uixqkr(3)+rxqkui(3))*psc5
     &           )*0.5d0 - gtri(6)*rxqkr(3)
