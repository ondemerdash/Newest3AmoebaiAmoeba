...
c         if(numtasks.ge.13) then
c           numtasks_tmp=numtasks-2
c           numtasks_vdw=int(numtasks_tmp/11)
c           numtasks_emreal=10*numtasks_vdw+mod(numtasks_tmp,11)
c         else
c           numtasks_vdw=2
c           numtasks_emreal=numtasks-4         
c         end if
         numtasks_vdw=1
         numtasks_emreal=1
...
c            listsend_vdw = .false.
            ...
            call grad_covemrecip_vandr_noreduce_totfield 
...
c            call mpi_waitall(6,reqs,stats2,ierr)
              ...
              if(taskid.eq.master) then
                 ...
                 call mpi_recv(emreal_tmp,1,mpi_real8,2,1,
     &            mpi_comm_world,stat,ierr)
                 call mpi_recv(demreal_tmp,3*n,mpi_real8,2,2,
     &            mpi_comm_world,stat,ierr)
                 call mpi_recv(viremreal_tmp,3*3,mpi_real8,2,3,
     &            mpi_comm_world,stat,ierr)


                 call mpi_recv(ev,1,mpi_real8,3,4,
     &            mpi_comm_world,stat,ierr)
                 call mpi_recv(dev,3*n,mpi_real8,3,5,
     &            mpi_comm_world,stat,ierr)
                 call mpi_recv(virev,3*3,mpi_real8,3,6,
     &            mpi_comm_world,stat,ierr)
