...
c     &   eptemp,deptemp,virtemp)
      subroutine ereal1c2_3b_totfieldnpole3b(npole3b,pnum,uind,uinp,
     &   deptemp,virtemp)
      use sizes
      use atoms
      use bound
...
cc     &            pscale(i14(j,ii)) = p4scale * p41scale
...
c               gfri(6) = -rr7 * (sci(3)*psc7+scip(3)*dsc7)
               ...
               gfi(1) = 0.5d0*bn(2)*(glip(1)+glip(6))
     &                   !  + 0.5d0*bn(2)*scip(2)
     &                     + 0.5d0*bn(3)*(glip(2)+glip(7))
     ...
     &                 + 0.5d0*rr9*(glip(3)*dsc7)
               ...
               gfri(4) = 2.0d0 * rr5
               gfri(5) = rr7 * (scip(4)*dsc7)
               gfri(6) = -rr7 * (scip(3)*dsc7)
...
c     &            + gfri(5)*qir(3) + gfri(6)*qkr(3)
     ...
     &            + (rr3*ci*(uinp(1,l2)*dsc3)
     &            + rr5*sc(3)*(uinp(1,l2)*dsc5)
     &            + rr7*sc(5)*(uinp(1,l2)*dsc7))*0.5d0
     &            + 0.5d0*(scip(4)*dsc5)*rr5*di(1)
     &            + 0.5d0*(scip(3)*dsc5)*rr5*dk(1)
     &            + 0.5d0*gfri(4)*(
     ...
     &            + (rr3*ci*(uinp(2,l2)*dsc3)
     &            + rr5*sc(3)*(uinp(2,l2)*dsc5)
     &            + rr7*sc(5)*(uinp(2,l2)*dsc7))*0.5d0
     &            + 0.5d0*(scip(4)*dsc5)*rr5*di(2)
     &            + 0.5d0*(scip(3)*dsc5)*rr5*dk(2)
     &            + 0.5d0*gfri(4)*(
     ...
     &            + (rr3*ci*(uinp(3,l2)*dsc3)
     &            + rr5*sc(3)*(uinp(3,l2)*dsc5)
     &            + rr7*sc(5)*(uinp(3,l2)*dsc7))*0.5d0
     &            + 0.5d0*(scip(4)*dsc5)*rr5*di(3)
     &            + 0.5d0*(scip(3)*dsc5)*rr5*dk(3)
     &            + 0.5d0*gfri(4)*(
...
c               gti(3) = 0.5d0 * bn(2) * (sci(3)+scip(3))
               gti(2) = 0.5d0 * bn(2) * (scip(4))
               gti(3) = 0.5d0 * bn(2) * (scip(3))
               gti(4) = gfi(4)
               gti(5) = gfi(5)
               gti(6) = gfi(6)
c               gtri(2) = 0.5d0 * rr5 * (sci(4)*psc5+scip(4)*dsc5)
c               gtri(3) = 0.5d0 * rr5 * (sci(3)*psc5+scip(3)*dsc5)
               gtri(2) = 0.5d0 * rr5 * (scip(4)*dsc5)
               gtri(3) = 0.5d0 * rr5 * (scip(3)*dsc5)
               gtri(4) = gfri(4)
               gtri(5) = gfri(5)
               gtri(6) = gfri(6)
c
...
c     &           +(uixqkrp(3)+rxqkuip(3))*dsc5)*0.5d0 - gtri(6)*rxqkr(3)
               ...
               ttm2ri(1) = -rr3*(dixukp(1)*dsc3)*0.5d0
     &           + gtri(2)*dixr(1) + gtri(4)*(
     &           (ukxqirp(1)+rxqiukp(1))*dsc5)*0.5d0 - gtri(5)*rxqir(1)
               ttm2ri(2) = -rr3*(dixukp(2)*dsc3)*0.5d0
     &           + gtri(2)*dixr(2) + gtri(4)*(
     &           (ukxqirp(2)+rxqiukp(2))*dsc5)*0.5d0 - gtri(5)*rxqir(2)
               ttm2ri(3) = -rr3*(dixukp(3)*dsc3)*0.5d0
     &           + gtri(2)*dixr(3) + gtri(4)*(
     &           (ukxqirp(3)+rxqiukp(3))*dsc5)*0.5d0 - gtri(5)*rxqir(3)
               ttm3ri(1) = -rr3*(dkxuip(1)*dsc3)*0.5d0
     &           + gtri(3)*dkxr(1) - gtri(4)*(
     &            (uixqkrp(1)+rxqkuip(1))*dsc5)*0.5d0 - gtri(6)*rxqkr(1)
               ttm3ri(2) = -rr3*(dkxuip(2)*dsc3)*0.5d0
     &           + gtri(3)*dkxr(2) - gtri(4)*(
     &           (uixqkrp(2)+rxqkuip(2))*dsc5)*0.5d0 - gtri(6)*rxqkr(2)
               ttm3ri(3) = -rr3*(dkxuip(3)*dsc3)*0.5d0
     &           + gtri(3)*dkxr(3) - gtri(4)*(
