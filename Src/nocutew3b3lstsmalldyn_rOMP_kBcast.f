...
c      real*8 efindiff(10),analyticg(10)
c      real*8 step
...
c
      ...
      master1 =1
      master2 =2
c      numtasks_emreal=64
c
c      allocate (taskidemreal(0:numtasks_emreal-1))
c      allocate (taskidother(0:(numtasks-1-numtasks_emreal)))
...
c      offset_emreal=int(n/numtasks_emreal)
c      remainder_emreal=mod(n,numtasks_emreal)
c
c      if(taskid.le.remainder_emreal-1) then
c        atomind=numtasks_emreal*offset_emreal+taskid+1
c      else if(taskid.gt.remainder_emreal-1) then
c        atomind=0
c      end if
c
c      if(taskid.le.numtasks_emreal-1) then
c       start_emreal=taskid*offset_emreal+1
c       print*,"Taskid start",taskid,start_emreal
c      end if
...
c              call mollist3bodycobarPar
c              call polar_load_balance3b_sizelist
              call gradient_setup    
              allocate (derivs(3,n))
              call prep_pole
              print*,"listbcast=",listbcast
              print*,"listbcast3b=",listbcast3b
c              call allocPermElec1 
            else if(taskid .eq. master2) then
c               call mlist
     ...
     &       mpi_comm_world,ierr)
            call mpi_bcast(listsend_mpole,1,mpi_logical,master,
     &       mpi_comm_world,ierr)
            call mpi_bcast(listbcast3b,1,mpi_logical,master,
     &       mpi_comm_world,ierr)
            if(listbcast) then
...
c     &       mpi_comm_world,ierr)
c            call mpi_bcast(nmollst2,nmol,mpi_integer,master,
c     &      mpi_comm_world,ierr)
c            call mpi_bcast(mollst2,max2blst2*nmol,mpi_integer,master,
c     &       mpi_comm_world,ierr)
c            print*,"After all 2body list bcast"
            end if
            listbcast = .false.

c            if(listbcast3b) then
c              call bcast_polar_load_balance3b
c
c              call sendmollst3_load_balanced_sizelist
c              print*,"After 3BODY SEND/RECV" 
c            end if
            listbcast3b = .false.
c            if(listsend_mpole) then
c              call sendmlist
c            end if
c            listsend_mpole = .false.
            switchmode = 'EWALD3B'
...
c              call gradient_emreal_bcast
            end if

c            if(taskid.le.numtasks_emreal-1) then
c             call gradient_emreal_reduce(start_emreal,
c     &       start_emreal+offset_emreal-1,atomind)
c            end if
            
c            call smallsmooth2_gradient_polar_load_balanced 
           call nocut_ew_2body_gradient_polar(start,start+offset-1,
     &           moli1rmndr)
           call nocut_ew_3body_gradient_polar(start,start+offset-1,
     &           moli1rmndr)
...
c           call mpi_bcast(emreal_tmp,1,mpi_real8,master2,
c     &       mpi_comm_world,ierr)
c           call mpi_bcast(demreal_tmp,3*n,mpi_real8,master2,
c     &       mpi_comm_world,ierr)
c           call mpi_bcast(viremreal_tmp,3*3,mpi_real8,master2,
c     &       mpi_comm_world,ierr)
     ...
     &                   + ev + em+ep3b + ep2b
               ...
               print*,"ep2b in verlet/nose/beeman setup",ep2b
               print*,"NumTriples in verlet/nose/beeman setup",ntriples
            print*,"ep2b+ep3b in verlet/nose/beeman setup",ep2b+ep3b
               ...
               print*,"ett in verlet/nose/beeman setup",ett
                do i = 1, n
                  do j = 1, 3
c               derivs(j,i) =deb(j,i) + dea(j,i) + deba(j,i) +deub(j,i)
c     &                   + deopb(j,i) + det(j,i) + dept(j,i) + dett(j,i)
c     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)+dep2b(j,i)
                  end do
                  print*,"i dep3b_x",i,dep3b(1,i)+dep2b(1,i)
                  print*,"i dep3b_y",i,dep3b(2,i)+dep2b(2,i)
                  print*,"i dep3b_z",i,dep3b(3,i)+dep2b(3,i)
                 if (use(i) .and. mass(i).ne.0.0d0) then
                   speed = maxwell (mass(i),kelvin)
                   call ranvec (vec)
                   do j = 1, 3
...
cc           call mpi_bcast(nmollst,nmol,mpi_integer,master,
c     &       mpi_comm_world,ierr)
c            call mpi_bcast(mollst,max2blst*nmol,mpi_integer,master,
c     &       mpi_comm_world,ierr)
            end if
...
c     &       mpi_comm_world,ierr)
     ...
     &                   + ev + em+ep3b+ep2b
              do i = 1, n
               ...
               derivs(j,i) =deb(j,i) + dea(j,i) + deba(j,i) +deub(j,i)
     &                   + deopb(j,i) + det(j,i) + dept(j,i) + dett(j,i)
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)+dep2b(j,i)
                end do
               if (use(i)) then
                do j = 1, 3
                 a(j,i) = -convert * derivs(j,i) / mass(i)
       ...
       print*," istep Vdw, PermElec,Pol3b,Pol2b",istep,ev,em,ep3b,ep2b
...
c             call allocPermElec1
     ...
     &       mpi_comm_world,ierr)
            ...
            if(listbcast) then
c            call bcast_polar_load_balance
c            call mpi_bcast(nmollst,nmol,mpi_integer,master,
c     &       mpi_comm_world,ierr)
c            call mpi_bcast(mollst,max2blst*nmol,mpi_integer,master,
c     &       mpi_comm_world,ierr)
...
c            end if
           ...
           call smallsmooth2body_gradient_polar_load_balanced
           call smallsmooth3body_gradient_polar_load_balanced
     ...
     &       mpi_comm_world,ierr)
       ...
       call mpi_bcast(emreal_tmp,1,mpi_real8,master2,
     &       mpi_comm_world,ierr)
        call mpi_bcast(demreal_tmp,3*n,mpi_real8,master2,
     &       mpi_comm_world,ierr)
        call mpi_bcast(viremreal_tmp,3*3,mpi_real8,master2,
     &       mpi_comm_world,ierr)
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)+dep2b(j,i)
       ...
       print*,"Noseistep Vdw,PermElec,Polz2b+3b",istep,ev,em,ep2b,ep3b
               print*,"Noseistep eb",eb
               print*,"Noseistep ea",ea
               print*,"Noseistep eba",eba
               print*,"Noseistep eub",eub
               print*,"Noseistep eopb",eopb
               print*,"Noseistep et",et
               print*,"Noseistep ept",ept
               print*,"Noseistep ett",ett
