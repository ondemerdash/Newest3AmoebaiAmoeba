      subroutine ewpolar_load_balance7_clust
      use mpidat
      use neigh3b
      use molcul
      use neigh2clust
...
c      do while (countmol.lt.nmol)
      do while (countmol.lt.clustcount)
         load=0
...
c            if(countmol.eq.nmol+1) then
            if(countmol.eq.clustcount+1) then
               goto 31 
            end if
         end do
   31             continue
