...
c
     ...
     &      !               - bcn(1)*di(3) - 2.0d0*bcn(2)*qir(3)
            ...
            !end do
              ...
              fid1 = -xr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr)
     &                        - rr3_field*dkx + 2.0d0*rr5_field*qkx
              fid2 = -yr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr)
     &                       - rr3_field*dky + 2.0d0*rr5_field*qky
              fid3 = -zr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr)
     &                        - rr3_field*dkz + 2.0d0*rr5_field*qkz
              fkd1 = xr*(rr3_field*ci+rr5_field*dir+rr7_field*qir)
     &                        - rr3_field*dix - 2.0d0*rr5_field*qix
              fkd2 = yr*(rr3_field*ci+rr5_field*dir+rr7_field*qir)
     &                        - rr3_field*diy - 2.0d0*rr5_field*qiy
              fkd3 = zr*(rr3_field*ci+rr5_field*dir+rr7_field*qir)
     &                        - rr3_field*diz - 2.0d0*rr5_field*qiz
             ...
             !     end do
                   ...
                   fieldt(1,i) = fieldt(1,i) + fid1*dscale(kk)
                   fieldt(1,k) = fieldt(1,k) + fkd1*dscale(kk)
                   fieldtp(1,i) = fieldtp(1,i) + fid1*pscale(kk)
                   fieldtp(1,k) = fieldtp(1,k) + fkd1*pscale(kk)

                   fieldt(2,i) = fieldt(2,i) + fid2*dscale(kk)
                   fieldt(2,k) = fieldt(2,k) + fkd2*dscale(kk)
                   fieldtp(2,i) = fieldtp(2,i) + fid2*pscale(kk)
                   fieldtp(2,k) = fieldtp(2,k) + fkd2*pscale(kk)

                   fieldt(3,i) = fieldt(3,i) + fid3*dscale(kk)
                   fieldt(3,k) = fieldt(3,k) + fkd3*dscale(kk)
                   fieldtp(3,i) = fieldtp(3,i) + fid3*pscale(kk)
                   fieldtp(3,k) = fieldtp(3,k) + fkd3*pscale(kk)
