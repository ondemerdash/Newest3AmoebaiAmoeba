...
c
c
      subroutine molfullNew4Bodies_com
c      implicit none
c      include 'sizes.i'
c      include 'atoms.i'
c      include 'bound.i'
...
c      include 'atmtyp.i'
      ...
      real*8 lbuffer2b
      integer moli1,moli2
      real*8 xcm2,ycm2,zcm2,xcm3,ycm3,zcm3
c
c
...
c            if(j .ne. i) then
            ...
            if (kbz(i) .le. kez(i)) then
               ...
               if(r1.le.molbufshell) then
                 ...
                 xcm2=(xi+xsort(j))/2.0d0
                 ycm2=(yi+ysort(kgy))/2.0d0
                 zcm2=(zi+zsort(kgz))/2.0d0
               end if
...
c                r1=sqrt(xr1*xr1 + yr1*yr1 + zr1*zr1)
     ...
     &        !  (r3.le.molbufshell)) ) then
               if(r3.le.molbufshell) then
                  ...
                  xcm3=(xi+xsort(j)+xsort(j1))/3.0d0
                  ycm3=(yi+ysort(kgy)+ysort(kgy1))/3.0d0
                  zcm3=(zi+zsort(kgz)+zsort(kgz1))/3.0d0
               end if
