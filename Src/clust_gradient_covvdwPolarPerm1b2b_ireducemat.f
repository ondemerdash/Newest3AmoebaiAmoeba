      subroutine clust_gradient_covvdwPolarPerm1b2b_ireducemat(start,
     & last,moli1rmndr)
      use mpole
      use mpidat
      use deriv3b
...
c          print*,"Done w ClustEmpole1_1b_NoOmp",ep3bt,taskid
     ...
     &   .and.(taskid.lt.(numtasks_vdw2+numtasks_polar1))) then
            call loadbal_ehal1c_half_nolistsend_simultclust(
...
c            t3=mpi_Wtime()
     ...
     &      mpi_real8,mpi_sum,master,mpi_comm_world,reqs21,ierr)
c
       call mpi_ireduce(em1btmat,em1bmat,clustcount,mpi_real8,
     &       mpi_sum,master,mpi_comm_world,reqs22,ierr)
       call mpi_ireduce(dem1btmat,dem1bmat,
     &      3*3*maxsizeclust*clustcount,mpi_real8,
     &       mpi_sum,master,mpi_comm_world,reqs23,ierr)

                  call mpi_ireduce(ep3bt,ep3b,1,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,reqs24,ierr)
                  call mpi_ireduce(dep3bt,dep3b,npole*3,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,reqs25,ierr)
                  call mpi_ireduce(virep3bt,virep3b,3*3,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,reqs26,ierr)

                  call mpi_ireduce(em3bt,em,1,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,reqs27,ierr)
                  call mpi_ireduce(dem3bt,dem,npole*3,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,reqs28,ierr)
c           t8=mpi_Wtime()
...
c            virem3bt(j,i)=0.0d0
      ...
      if(do2waterclustlist.and.(taskid.lt.numtasks_polar)) then
          if(mlistclust) then
c           t5=mpi_Wtime()
           call ClustEmpole1_2b_NoOmplistRtrn2BNoSubtr1b(
