...
c
      ...
      integer emreal_group,k,othergroup
...
c         if (.not. allocated(start_emreal2))
c     &          allocate (start_emreal2(0:numtasks_emreal-1))
c         if (.not. allocated(last_emreal2))
c     &          allocate (last_emreal2(0:numtasks_emreal-1))
c         if (.not. allocated(maxsize_elst))
c     &         allocate (maxsize_elst(0:numtasks_emreal-1))
...
c        end if
      ...
      if((taskid.gt.1).and.(taskid.lt.numtasks_emreal+2)) then
        call cutoffs_ewald
c      else if(taskid.ge.numtasks_emreal+2) then
c        call cutoffs_vdw
      end if
      if(taskid.ge.clustcount) then
        call cutoffs_vdw
      end if
...
C  ALLOCATIONS FOR CLUSTER VERSION
     ...
     &    3,clustcount))
        if(.not. allocated(em1bmat)) allocate(em1bmat(clustcount))
        if(.not. allocated(dem1bmat)) allocate(dem1bmat(3,
     &    3*maxsizeclust,clustcount))
         if (poltyp .eq. 'MUTUAL') then
            if (.not.allocated(tindex))
     &         allocate (tindex(2,2*3*maxsizeclust*maxelst))
            if (.not.allocated(tdipdip))
     &         allocate (tdipdip(6,2*3*maxsizeclust*maxelst))
            if (.not.allocated(minv))
     &         allocate (minv(6*maxulst*2*3*maxsizeclust))
            if (.not.allocated(mindex))
     &         allocate (mindex(2*3*maxsizeclust))
         end if
      else
...
c           if((taskid.ge.numtasks_emreal+2)
c     &       .and.(taskid.lt.(numtasks_vdw2+numtasks_emreal+2)))then
c              print*,"In verlet/nose/beeman setup"
c              print*,"Before alloc3b_nblist verlet/nose/beeman setup"
c               call vlist_par_clust
c              if(firstload_vdw.and.(taskid.eq.numtasks_emreal+2)) then
c                firstload_vdw=.false.
c                call vdw_load_balance_sizelist_half
c              end if
...
c                if(firstload_emreal.and.(taskid.eq.master2)) then
c                 firstload_emreal = .false.
c                   call emreal_load_balance_sizelist
c                end if
           if(taskid.eq.master) then
              ...
              if(firstload_polz.and.do2waterclustlist) then
                ...
                call polar_load_balance_clust_splitlist
              end if
           end if


            if(clustcount.ge.numtasks) then
              offset=int(clustcount/numtasks)
              remainder=mod(clustcount,numtasks)
...
c            listbcast = .false.
           numtasks_emreal=numtasks_polar1
           numtasks_emreal2=numtasks_polar1
     ...
     &            emreal_comm,ierr)
           ...
           !deallocate(taskidother)

           if((taskid.ge.numtasks_emreal)
     &       .and.(taskid.lt.(numtasks_vdw2+numtasks_emreal)))then
               call vlist_par_clust
              if(firstload_vdw.and.(taskid.eq.numtasks_emreal)) then
            ...
            if((taskid.lt.numtasks_polar).and.mlistclust) then
              !if(useboxclust) then
                !call clust2blatticeboxes
              !end if
              !if(useboxclust) then
                !call clust2blistsboxclust
              !else 
                call clust2blists
              !end if
              print*,"Completed clust2blists"
              if(use_pred) then
                call initpolarpred2b
              end if 
            end if
            call mpi_barrier(mpi_comm_world,ierr)
     ...
     &            numtasks_emreal,mpi_comm_world,ierr)
     ...
     &             numtasks_emreal,mpi_comm_world,ierr)
             ...
             !call clust_gradient_covvdwPolarPerm1b2b(start,
             !call clust_gradient_covvdwPolarPerm1b2b_ireducemat(start,
      call clust_gradient_covvdwPolarPerm1b2b_ireducemat_commsmall
     &     (start,start+offset-1, moli1rmndr)


c            if(taskid.eq.master1) then
c             call mpi_wait(reqs1,stat,ierr)
c             call mpi_wait(reqs2,stat,ierr)
c             call mpi_wait(reqs3,stat,ierr)
c            else if(taskid.eq.master) then
c             call mpi_wait(reqs4,stat,ierr)
c             call mpi_wait(reqs5,stat,ierr)
c             call mpi_wait(reqs6,stat,ierr)
c            end if
c             call mpi_wait(reqs13,stat,ierr)
c             call mpi_wait(reqs14,stat,ierr)
c             call mpi_wait(reqs15,stat,ierr)
     ...
     &                +virev(j,i)
c       print*,"j i virep3b(j,i)",j,i,virep3b(j,i)
                   end do
                 end do
...
c           else if(taskid.eq.master) then
            !  if(doclust) then
            !    call molfullNew2BodiesCobar
            !  else if(doclust3) then
            !    call molfullNew3Bodies
            !  else if(doclust4) then
            !    call molfullNew4Bodies 
            !  end if

            !  if(do2waterclustlist) call mollist2body_NewBodies
            !  if(firstload_polz.and.do2waterclustlist) then
            !    firstload_polz = .false.
             !!! !  call polar_load_balance7
            !    call ewpolar_load_balance
...
c              efindif(istep)=energy-ep3b-em!-ep3bmut-em-ev
...
c                 firstload_emreal = .false.
            ...
            call mpi_bcast(listbcast,1,mpi_logical,master,
     &       mpi_comm_world,ierr)
            call mpi_bcast(clust,2*nmol,mpi_integer,master,
     &          mpi_comm_world,ierr)
            call mpi_bcast(sizeclust,nmol,mpi_integer,master,
     &          mpi_comm_world,ierr)
            call mpi_bcast(clust_cm,3*nmol,mpi_real8,master,
     &          mpi_comm_world,ierr)
            call mpi_bcast(clustcount,1,mpi_integer,master,
     &          mpi_comm_world,ierr)
            call mpi_bcast(distmax,nmol,mpi_real8,master,
     &          mpi_comm_world,ierr)
...
c             firstload_polz = .false.
     ...
     &          numtasks_emreal+2,mpi_comm_world,ierr)
            ...
            !if(approxmode.ne.'1BODYMODE') then
             call clustsmooth2_gradient_polar_load_balanced(start,
     &        start+offset-1, moli1rmndr)
            !end if
