...
c
c
      subroutine torque3_PolarPerm (npole3b,pnum,
     &     i,trq1,trq2,frcx,frcy,frcz,demi,depi)
      use sizes
      use atoms
      use mpole
      use mpidat
      ...
      integer i,j,l1
      integer ia,ib,ic,id
      integer ia_l1,ib_l1,ic_l1,id_l1
...
c
c      if(i.eq.0) then
c        print*,"Tilt! torque3_PolarPerm i=",i,"task=",taskid,
c     &    "npole3b=",npole3b
c      end if
      ia = zaxis(i)
      ib = ipole(i)
...
c
      if (axetyp .eq. 'Z-Only') then
         do j = 1, 3
            ...
            demi(j,ia_l1) = demi(j,ia_l1) + du
            demi(j,ib_l1) = demi(j,ib_l1) - du
            frcz(j) = frcz(j) + du
         end do
c
...
c
         ...
         do j = 1, 3
            ...
            demi(j,ia_l1) = demi(j,ia_l1) + du
            demi(j,ic_l1) = demi(j,ic_l1) + dv
            demi(j,ib_l1) = demi(j,ib_l1) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
...
c
     ...
     &              / (wsiz*(ut1sin+ut2sin))
            demi(j,ia_l1) = demi(j,ia_l1) + du
            demi(j,ic_l1) = demi(j,ic_l1) + dv
            demi(j,id_l1) = demi(j,id_l1) + dw
            demi(j,ib_l1) = demi(j,ib_l1) - du - dv - dw
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
            frcy(j) = frcy(j) + dw
         end do
...
c
      if (axetyp .eq. 'Z-Only') then
         do j = 1, 3
            du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
            depi(j,ia_l1) = depi(j,ia_l1) + du
            depi(j,ib_l1) = depi(j,ib_l1) - du
            frcz(j) = frcz(j) + du
         end do
c
c     force distribution for the Z-then-X local coordinate method
c
         ...
         do j = 1, 3
            ...
            dv = -uv(j)*dphidu/(vsiz*uvsin)
            depi(j,ia_l1) = depi(j,ia_l1) + du
            depi(j,ic_l1) = depi(j,ic_l1) + dv
            depi(j,ib_l1) = depi(j,ib_l1) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
...
c
     ...
     &              / (wsiz*(ut1sin+ut2sin))
            depi(j,ia_l1) = depi(j,ia_l1) + du
            depi(j,ic_l1) = depi(j,ic_l1) + dv
            depi(j,id_l1) = depi(j,id_l1) + dw
            depi(j,ib_l1) = depi(j,ib_l1) - du - dv - dw
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
            frcy(j) = frcy(j) + dw
         end do
