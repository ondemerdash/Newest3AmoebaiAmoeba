...
c
c
      subroutine ulspred2b(npole3b,cnt)
      use sizes
      use mpole
      use uprior
      use uprior2b
...
c
         ...
         do i = 1, npole3b
            do j = 1, 3
               do k = 1, nualt2b(cnt)
                  udk = udalt2b(k,j,i,cnt)
                  upk = upalt2b(k,j,i,cnt)
                  do m = k, nualt2b(cnt)
                     c(k,m) = c(k,m) + udk*udalt2b(m,j,i,cnt)
                     cp(k,m) = cp(k,m) + upk*upalt2b(m,j,i,cnt)
                  end do
               end do
            end do
         end do
...
c
         ...
         bpred2b(nualt2b(cnt),cnt) = 0.0d0
         bpredp2b(nualt2b(cnt),cnt) = 0.0d0
      end if
      return
      end
c
