      subroutine gradientemrealallreduce_loadbalanced_totfield_dEtensorV
      use mpole
      use mpidat
      use deriv
      use energi
      ...
      !end if
       ...
       !print*,"PermElec Real time w/o MPIReduce,taskid",t4-t3,taskid
        ...
        !    call mpi_barrier(emreal_comm,ierr)
         ...
         !    t1=mpi_Wtime()
       call mpi_allreduce(emrealt,emreal_tmp,1,mpi_real8,
     &      mpi_sum,mpi_comm_world,ierr)
       call mpi_allreduce(demrealt,demreal_tmp,npole*3,mpi_real8,
     &      mpi_sum,mpi_comm_world,ierr)
       call mpi_allreduce(viremrealt,viremreal_tmp,3*3,mpi_real8,
     &      mpi_sum,mpi_comm_world,ierr)

       call mpi_allreduce(fieldnpolet,fieldnpole,npole*3,mpi_real8,
     &      mpi_sum,mpi_comm_world,ierr)
       call mpi_allreduce(fieldpnpolet,fieldpnpole,npole*3,mpi_real8,
     &      mpi_sum,mpi_comm_world,ierr)
...
c      if(taskid.eq.master1.and.approxmode.eq.'1BODYMODE') then
        ep3b=0.0d0
        do i=1,npole
