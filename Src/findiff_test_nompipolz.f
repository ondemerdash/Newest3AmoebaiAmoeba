...
c
      ...
      real*8 efindiff(3),analyticg(3)
      real*8 efindiff_ep3b(3),analyticg_dep3b(3)
      real*8 delx
...
c              call allocPermElec1_mlist 
     ...
     &       mpi_comm_world,ierr)
            ...
            if(taskid.eq.master) then
             ...
             call gradient_polar(start,start+offset-1,moli1)
             if(istep.eq.1) then
               print*,"Task ID",taskid,start,start+offset-1,moli1
             end if
     ...
     &                       + viremrecip(i,j)
                ...
                energy = eb + ea + eba + eub + eopb + et + ept  + ett
     &                   + ev + em+ep3b
              efindiff(istep)=energy
              efindiff_ep3b(istep)=ep3b
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)
       ...
       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b
              analyticg(istep)=derivs(1,1)
              analyticg_dep3b(istep)=dep3b(1,1) 
              deallocate (derivs)
c              call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
                print*,"Finite Diff TotEng, TotGrad",
     & (efindiff(3)-efindiff(1))/2.0d0/delx,analyticg(2)
                print*,"Finite Diff PolEng, PolGrad",
     & (efindiff_ep3b(3)-efindiff_ep3b(1))/2.0d0/delx,analyticg_dep3b(2)
