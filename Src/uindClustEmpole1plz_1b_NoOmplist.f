      subroutine uindClustEmpole1plz_1b_NoOmplist 
     &  (ep3bt,virep3bt,dep3bt,ntript,start,last,moli1rmndr,
     &  ep1btmat,dep1btmat,virep1btmat,uind1bt,uind1btmat)!,em3bt,dem3bt,em1btmat,
     &  !dem1btmat) 
      use sizes
      use atoms
      use atomid
...
c      real*8 em3bt,dem3bt(3,npole),virem3bt(3,3)
      logical do2
...
c      real*8 dep2moli12nosubtr(3,36)
      ...
      integer cnt
      real*8 uind1bt(3,npole),uind1btmat(3,3*maxsizeclust,clustcount)
      real*8, allocatable :: uind(:,:)
c      real*8 em1btmat(clustcount)
c      real*8 dem1btmat(3,3*maxsizeclust,clustcount)
c      real*8 virem1btmat(3,3,clustcount)
...
c     &         eptemp,deptemp,virtemp,cnt,emtemp,demtemp)
     ...
     &    !     eptemp,deptemp,virtemp,cnt,emtemp,demtemp) 
          ! else
          call uindempole1d_3b_Polar1btest2(npole3b,pnum,
     &         eptemp,deptemp,virtemp,cnt,uind)!,emtemp,demtemp)
          ! end if
          !else
     ...
     &    !     eptemp,deptemp,virtemp,cnt,emtemp,demtemp)
          ...
          ep3bt = ep3bt + eptemp
c          em3bt = em3bt + emtemp
          ep1btmat(clust1)=eptemp
c          em1btmat(clust1)=emtemp
          do l1 = 1, npole3b
             i = pnum(l1)
             do j = 1, 3
...
c              dem1btmat(j,l1,clust1)=demtemp(j,l1)
              ...
              uind1bt(j,i) = uind1bt(j,i)+uind(j,l1)
              uind1btmat(j,l1,clust1)=uind(j,l1)
c              dem3bt(j,i) = dem3bt(j,i)+demtemp(j,i)
c              dem1btmat(j,l1,clust1)=demtemp(j,i)
...
c              dem1btmat(j,l1,clust1)=demtemp(j,l1)
              ...
              uind1bt(j,i) = uind1bt(j,i)+uind(j,l1)
              uind1btmat(j,l1,moli1rmndr)=uind(j,l1)
c              dem3bt(j,i) = dem3bt(j,i)+demtemp(j,i)
c              dem1btmat(j,l1,moli1rmndr)=demtemp(j,i)
