      subroutine grad_covemrecip_vandr_reduce_totfield_dEtensor_vir2 
      use mpole
      use mpidat
      use mpidat2
      use deriv
...
c      call mpi_recv(em,1,mpi_real8,master1,1,mpi_comm_world,
c     &   stat,ierr)
c      call mpi_recv(dem,3*n,mpi_real8,master1,2,mpi_comm_world,
c     &    stat,ierr)
c      call mpi_recv(viremrecip,3*3,mpi_real8,master1,3,mpi_comm_world,
...
c     &       mpi_comm_world,ierr)
         ...
         allocate(dep3bt1(3,npole))
         allocate(uindt(3,npole))
         allocate(uinpt(3,npole))
         ...
         if((taskid.gt.1).and.(taskid.lt.numtasks_emreal2+2)) then
           call empole1c_3bPolar_totfield_dEtensor1bmut(
     &          dep3bt1,virep3bt1,uindt,uinpt)
         end if

        call mpi_reduce(dep3bt1,dep3b1,npole*3,mpi_real8,
     &       mpi_sum,master,mpi_comm_world,ierr)
        call mpi_reduce(virep3bt1,virep3b1,3*3,mpi_real8,
     &       mpi_sum,master,mpi_comm_world,ierr)

        call mpi_reduce(uindt,uind,npole*3,mpi_real8,
     &       mpi_sum,master1,mpi_comm_world,ierr)
        call mpi_reduce(uinpt,uinp,npole*3,mpi_real8,
     &       mpi_sum,master1,mpi_comm_world,ierr)
        
c        if(taskid.eq.master) then
c          do i=1,npole
c          print*,"HEY YOU! dep3b1=",dep3b1(1,i),dep3b1(2,i),dep3b1(3,i)
c          end do
c        end if
        if(taskid.eq.master1) then
          ep3b=0.0d0
          do i=1,npole
c      print*,"Fuckin uind aftr mpi_reduce",uind(1,i),uind(2,i),uind(3,i)
           ep3b=ep3b - 0.5d0*f*( uind(1,i)*fieldpnpole(1,i) +
     &     uind(2,i)*fieldpnpole(2,i) + uind(3,i)*fieldpnpole(3,i))
          end do 
        end if
