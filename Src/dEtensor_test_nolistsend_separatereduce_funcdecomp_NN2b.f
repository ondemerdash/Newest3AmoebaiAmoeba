...
c
c
      program dEtensor_test_nolistsend_separatereduce_funcdecomp_NN2b 
      use sizes
      use atoms
      use bath
      use bndstr
...
c            listsend_vdw = .false.
      ...
      call grad_covemrecip_vandr_reduce_totfield_dEtensor_vir8
...
c            call mpi_waitall(6,reqs,stats2,ierr)
             ...
             call mpi_wait(reqs3,stat,ierr)
c               if(uzepmedirpolz) then
c             call mpi_wait(reqs7,stat,ierr)
c             call mpi_wait(reqs8,stat,ierr)
c             call mpi_wait(reqs9,stat,ierr)
c               end if
            else if(taskid.eq.master) then
             call mpi_wait(reqs4,stat,ierr)
             call mpi_wait(reqs5,stat,ierr)
             call mpi_wait(reqs6,stat,ierr)
c               if(uzepmedirpolz) then
c             call mpi_wait(reqs10,stat,ierr)
c             call mpi_wait(reqs11,stat,ierr)
c             call mpi_wait(reqs12,stat,ierr)
c               end if
            end if
             call mpi_wait(reqs13,stat,ierr)
             call mpi_wait(reqs14,stat,ierr)
...
c!$OMP END PARALLEL DO
              ...
              analyticgx(istep)=derivs(1,6)-dep3b(1,6)!-(dep3bmut(1,6)
     ...
     &          !    +dep3b1(2,6)
              ...
              analyticgz(istep)=derivs(3,6)-dep3b(3,6)!-(dep3bmut(3,6)
...
c              end if
              ...
              if(istep.eq.2) then
                ...
                print*,"Step 2 virep3b",j,i,virep3b(j,i)!+virep3bmut(j,i)
                   end do
                end do
...
c            end if
     ...
     &          numtasks_emreal+2,mpi_comm_world,ierr)
      ...
      call grad_covemrecip_vandr_reduce_totfield_dEtensor_vir3
