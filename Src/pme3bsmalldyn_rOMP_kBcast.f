...
c              call polar_load_balance3b_sizelist
     ...
     &       mpi_comm_world,ierr)
            call mpi_bcast(listsend_mpole,1,mpi_logical,master,
     &       mpi_comm_world,ierr)
c            call mpi_bcast(listbcast3b,1,mpi_logical,master,
...
c           call ewsmallsmooth2body_gradient_polar_load_balanced
c           call ewsmallsmooth3body_gradient_polar_load_balanced

           !call ewsmallsmooth2_gradient_polar_load_balanced
...
c           call ewsmallsmooth3body_gradient_polar_load_balanced
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)
              ...
              analyticgx(istep)=derivs(1,6)!-(dep3bmut(1,6)
     &              !+dep3b_recip(1,6)+dep3b1(1,6)+dem(1,6)+dev(1,6))
              analyticg_dep3bx(istep)=dep3b(1,6)!+dep3bmut(1,6)
     &             ! +dep3b1(1,6)

              analyticgy(istep)=derivs(2,6)!-(dep3bmut(2,6)
     &            !  +dep3b_recip(2,6)+dep3b1(2,6)+dem(2,6)+dev(2,6))
              analyticg_dep3by(istep)=dep3b(2,6)!+dep3bmut(2,6)
     &            !  +dep3b1(2,6)

              analyticgz(istep)=derivs(3,6)!-(dep3bmut(3,6)
     &            !  +dep3b_recip(3,6)+dep3b1(3,6)+dem(3,6)+dev(3,6))
              analyticg_dep3bz(istep)=dep3b(3,6)!+dep3bmut(3,6)
     &           !   +dep3b1(3,6)


       print*," istep Vdw, PermElec,Pol3b,Pol2b",istep,ev,em,ep3b
              
              deallocate (derivs)
c              call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
                print*,"Finite Diff PairwiseAddEng, PairwiseAddGradz",
     & (efindif(3)-efindif(1))/2.0d0/delx,analyticgx(2)
                print*,"Finite Diff PolEng, PolGradz",
     & (efindif_ep3b(3)-efindif_ep3b(1))/2.0d0/delx,analyticg_dep3bx(2)
              end if

              if(istep.eq.2) then
                do i=1,3
                   do j=1,3
                 vir_analytic(j,i)=vir(j,i)!-virep3b(j,i)-virep3bmut(j,i)
                 virep3b_analytic(j,i)=virep3b(j,i)!+virep3bmut(j,i)
c                print*,"Step 2 virep3b",j,i,virep3b(j,i)+virep3bmut(j,i)
                   end do
                end do
                volbox_analytic=volbox
              end if
...
c           call smallsmooth2body_gradient_polar_load_balanced
c           call smallsmooth3body_gradient_polar_load_balanced
            !call ewsmallsmooth2_gradient_polar_load_balanced
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)
       ...
       print*,"Noseistep Vdw,PermElec,Polz3b",istep,ev,em,ep3b
