...
C  PERFORM LOAD BALANCING FOR REAL-SPACE PERM ELEC BASED ON THE NUMBER OF NEIGHBORS OF EACH OUTER LOOP MOLECULE INDEX
     ...
     &             master1,mpi_comm_world,ierr)

c            if(listsend_mpole.and.taskid.lt.numtasks_emreal) then
...
c            listsend_mpole = .false.
     ...
     &            master,mpi_comm_world,ierr)
              call mpi_bcast(start_vdw2,numtasks,mpi_integer,
     &             master,mpi_comm_world,ierr)
               call mpi_bcast(last_vdw2,numtasks,mpi_integer,
     &             master,mpi_comm_world,ierr)
               call mpi_bcast(maxsize_vlst,numtasks,mpi_integer,
     &             master,mpi_comm_world,ierr)
               call sendvlist_load_balanced3_sizelist

            if(taskid.eq.master) then
             call gradient_covalent2
            end if
c            else if(taskid.eq.master1) then
             call gradient_emrecip_totfield_send
c            end if

             call gradient_vdw_reduce_load_balanced
c            call mpi_barrier(mpi_comm_world,ierr)
             call field_emreal_reduce_load_balanced_totfield_uindcalc
...
c              call emrecip1_3b_Polar_usemlist_direct
c            end if            
