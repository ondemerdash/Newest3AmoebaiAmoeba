...
c
c
      subroutine induce0a_3b_scf_vacomp2bclust(npole3b,pnum,uind,uinp,
     & cnt)
      use sizes
      use atoms
      use inform
...
c
        if(use_ewaldclust) then
         call dfield0c_3b_vacomp2b(npole3b,pnum,field,fieldp,cnt)
        else
         call dfield0b_3b_vacomp2b(npole3b,pnum,field,fieldp,cnt)
        end if
...
c
      !print*,"use_pred=",use_pred
         ...
         if (use_pred) then
           if(nualt2b(cnt).eq.maxualt) then
            call ulspred2b(npole3b,cnt)
            !do i = 1, npole
            do l1 =1, npole3b
               do j = 1, 3
                  udsum = 0.0d0
                  upsum = 0.0d0
                  do k = 1, nualt2b(cnt)-1
                   udsum = udsum + bpred2b(k,cnt)*udalt2b(k,j,l1,cnt)
                   upsum = upsum + bpredp2b(k,cnt)*upalt2b(k,j,l1,cnt)
                  end do
                  uind(j,l1) = udsum
                  uinp(j,l1) = upsum
               end do
...
c
          if(use_ewaldclust) then
              if(useboxclust) then
               !call ufield0c_3b_vacompbox2bclust(npole3b,pnum,
...
c         else
           if(useboxclust) then
            !  call uscale0b_3b_vacomp2btestbox2bclust(npole3b,pnum,mode,
     &      !    rsd,rsdp,zrsd,zrsdp,cnt)
            ! mode = 'APPLY'
            !  call uscale0b_3b_vacomp2btestbox2bclust(npole3b,pnum,mode,
     &      !    rsd,rsdp,zrsd,zrsdp,cnt)
           else
               call uscale0b_3b_vacomp2btest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp,cnt)
             mode = 'APPLY'
               call uscale0b_3b_vacomp2btest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp,cnt)
...
c
      if (use_pred) then
         nualt2b(cnt) = min(nualt2b(cnt)+1,maxualt)
         !do i = 1, npole
         do l1 = 1, npole3b
            i=pnum(l1)
            do j = 1, 3
               do k = nualt2b(cnt), 2, -1
                  udalt2b(k,j,l1,cnt) = udalt2b(k-1,j,l1,cnt)
                  upalt2b(k,j,l1,cnt) = upalt2b(k-1,j,l1,cnt)
               end do
               udalt2b(1,j,l1,cnt) = uind(j,l1)
               upalt2b(1,j,l1,cnt) = uinp(j,l1)
            end do
         end do
      end if
...
c
c
      subroutine dfield0b_3b_vacomp2b(npole3b,pnum,field,fieldp,
     &  cnt)
      use sizes
      use atoms
      use bound
...
c
c
      subroutine uscale0b_3b_vacomp2b(npole3b,pnum,mode,rsd,rsdp,zrsd,
     &  zrsdp,cnt)
      use sizes
      use atoms
      use limits
...
c
c
      subroutine uscale0b_3b_vacomp2btest(npole3b,pnum,mode,rsd,rsdp,
     &  zrsd,zrsdp,cnt)
      use sizes
      use atoms
      use limits
