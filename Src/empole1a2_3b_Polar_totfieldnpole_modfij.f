...
c     &   uinp,deptemp,virtemp)
      subroutine empole1a2_3b_Polar_totfieldnpole_modfij(npole3b,pnum,
     &   uind,uinp,eptemp,deptemp,virtemp,deptempmat)
      use bound
      use sizes
      use atoms
      use chgpot
...
c              if(doi) then
c               ttm2i(1) = -rr3*(dixukp(1)*dsc3)*0.5d0
c     &           + gti(2)*dixr(1) + gti(4)*(
c     &           (ukxqirp(1)+rxqiukp(1))*dsc5)*0.5d0 - gti(5)*rxqir(1)
c               ttm2i(2) = -rr3*(dixukp(2)*dsc3)*0.5d0
c     &           + gti(2)*dixr(2) + gti(4)*(
c     &           (ukxqirp(2)+rxqiukp(2))*dsc5)*0.5d0 - gti(5)*rxqir(2)
c               ttm2i(3) = -rr3*(dixukp(3)*dsc3)*0.5d0
c     &           + gti(2)*dixr(3) + gti(4)*(
c     &           (ukxqirp(3)+rxqiukp(3))*dsc5)*0.5d0 - gti(5)*rxqir(3)
c              end if
c               ttm3i(1) = -rr3*(dkxuip(1)*dsc3)*0.5d0
c     &           + gti(3)*dkxr(1) - gti(4)*(
c     &           (uixqkrp(1)+rxqkuip(1))*dsc5)*0.5d0 - gti(6)*rxqkr(1)
c               ttm3i(2) = -rr3*(dkxuip(2)*dsc3)*0.5d0
c     &           + gti(3)*dkxr(2) - gti(4)*(
c     &           (uixqkrp(2)+rxqkuip(2))*dsc5)*0.5d0 - gti(6)*rxqkr(2)
c               ttm3i(3) = -rr3*(dkxuip(3)*dsc3)*0.5d0
c     &           + gti(3)*dkxr(3) - gti(4)*(
...
c              end if
              ...
              if(doi) then
               ...
               deptempmat(1,k,i)=deptempmat(1,k,i)+0.5d0*ftm2i(1)
               deptempmat(2,k,i)=deptempmat(2,k,i)+0.5d0*ftm2i(2)
               deptempmat(3,k,i)=deptempmat(3,k,i)+0.5d0*ftm2i(3)

               call torque_3b_new_npolefij(deptemp,i,
     &          ttm2,0.5d0*ttm2i,frcxi,frcyi,frczi,deptempmat,k)

               deptemp(1,k) = deptemp(1,k) - 0.5d0*ftm2i(1)
               deptemp(2,k) = deptemp(2,k) - 0.5d0*ftm2i(2)
               deptemp(3,k) = deptemp(3,k) - 0.5d0*ftm2i(3)
               deptempmat(1,i,k)= deptempmat(1,i,k)-0.5d0*ftm2i(1)
               deptempmat(2,i,k)= deptempmat(2,i,k)-0.5d0*ftm2i(2)
               deptempmat(3,i,k)= deptempmat(3,i,k)-0.5d0*ftm2i(3)

               call torque_3b_new_npolefij(deptemp,k,
     &            ttm3,0.5d0*ttm3i,frcxk,frcyk,frczk,deptempmat,i)
              else
               ...
               call torque_3b_new_npolefij(deptemp,i,
     &          ttm2,ttm2i,frcxi,frcyi,frczi,deptempmat,k)
               ...
               call torque_3b_new_npolefij(deptemp,k,
     &            ttm3,ttm3i,frcxk,frcyk,frczk,deptempmat,i)  
              end if
