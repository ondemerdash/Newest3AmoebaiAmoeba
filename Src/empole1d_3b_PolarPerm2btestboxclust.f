      ...
      subroutine empole1d_3b_PolarPerm2btestboxclust(npole3b,pnum,
     ...
     &   emtemp,demtemp)
      ...
      integer clock_rate
      real tot_time
c      real*8 t1,t2,t3,t4,t5,t6,t7,t8

c            t1=mpi_Wtime()
            call system_clock(t1,clock_rate)

      call kewald2bclust(cnt)
      eptemp = 0.0d0
      emtemp = 0.0d0
c
...
c            t3=mpi_Wtime()
c            call system_clock(t3,clock_rate)

       call induce0a_3b_scf_vacomp2bclust(npole3b,pnum,uind,uinp,cnt)
c            t4=mpi_Wtime()
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)
      
c       print*,"Cost of induce0a_3b_scf_vacomp2bclust",tot_time!t4-t3

c            t3=mpi_Wtime()
c            call system_clock(t3,clock_rate)
      call emrecip1_3b_PolarPermtestbox2bclust(npole3b,pnum,uind,uinp,
     & eptemp,deptemp,virtemp,emtemp,demtemp,cnt)
c            t4=mpi_Wtime()
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)
c       print*,"Cost of emrecip1_3b_PolarPerm",tot_time!t4-t3


c      print*,"Done w emrecip1_3b_Polar_new in 2bclust"

c            t3=mpi_Wtime()
c            call system_clock(t3,clock_rate)
      call ereal1d_3b_2bclustPolarPermtestbox2bclust(npole3b,pnum,uind,
     & uinp,cnt,eptemp,deptemp,virtemp,emtemp,demtemp)
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)
c            t4=mpi_Wtime()
c       print*,"Cost of ereal1d_3b_2bclustPolarPerm",tot_time!t4-t3
c      print*,"Done w ereal1d_3b_2bclust in 2bclust"
...
c         if(i.eq.0) then
c           print*,"Tilt! B4 pnum(l1) is 0 B4 SelfTorq"
c         end if
...
c      print*,"Completed all steps of empole1d 2bclust"
c            t4=mpi_Wtime()
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)

c       print*,"Cost of SelfEng SelfTorque",tot_time

c            t2=mpi_Wtime()
            call system_clock(t2,clock_rate)
       tot_time =  (t2-t1)/real(clock_rate)
      
      ! print*,"tsk=",taskid,"TotCost Within empole1d_3b_2btest",tot_time
      return
      end
c
c     ################################################################
...
c
c
      subroutine ereal1d_3b_2bclustPolarPermtestbox2bclust(npole3b,pnum,
     & uind,uinp,cnt,eptemp,deptemp,virtemp,emtemp,demtemp)
      use sizes
      use atoms
      use bound
      use boxes
...
c
c
      subroutine emrecip1_3b_PolarPermtestbox2bclust(npole3b,pnum,uind,
     & uinp,eptemp,deptemp,virtemp,emtemp,demtemp,cnt)
      use sizes
      use atoms
      use bound
      use boxes
...
c         do i = 1, npole
         ...
         call cmp_to_fmp3bclust2b (npole3b,pnum,cmp,fmp,cnt)
...
c
      ...
      do i = 1, ntot-1
         ...
         r2 = dble(m2)
         r3 = dble(m3)
         h1 = recip2b(1,1,cnt)*r1 + recip2b(1,2,cnt)*r2 
     &          + recip2b(1,3,cnt)*r3
         h2 = recip2b(2,1,cnt)*r1 + recip2b(2,2,cnt)*r2 
     &          + recip2b(2,3,cnt)*r3
         h3 = recip2b(3,1,cnt)*r1 + recip2b(3,2,cnt)*r2 
     &          + recip2b(3,3,cnt)*r3
         ...
         if (term .gt. -50.0d0) then
            ...
            if (.not. use_bounds) then
               expterm = expterm * (1.0d0-cos(pi*xbox2b(cnt)*sqrt(hsq)))
...
c
     ...
     &             + recip2b(3,3,cnt)*r3
            ...
            if (term .gt. -50.0d0) then
               ...
               expterm = exp(term) / denom
               if (.not. use_bounds) then
                  expterm = expterm * (1.0d0-cos(pi*xbox2b(cnt)
     &             *sqrt(hsq)))
               else if (octahedron) then
                  if (mod(m1+m2+m3,2) .ne. 0)  expterm = 0.0d0
               end if
...
c
      ...
      call fphi_to_cphi3bclust2b (npole3b,pnum,fphi,cphi,cnt)
c
...
c
      ...
      do l1 = 1, npole3b
         ...
         f3 = 0.0d0
         do k = 1, 10
            e = e + fmp(k,l1)*fphi(k,l1)
            f1 = f1 + fmp(k,l1)*fphi(deriv1(k),l1)
            f2 = f2 + fmp(k,l1)*fphi(deriv2(k),l1)
            f3 = f3 + fmp(k,l1)*fphi(deriv3(k),l1)
         end do
         f1 = dble(nfft1) * f1
         f2 = dble(nfft2) * f2
         f3 = dble(nfft3) * f3
         frc(1,i) = recip2b(1,1,cnt)*f1 + recip2b(1,2,cnt)*f2 
     &      + recip2b(1,3,cnt)*f3
         frc(2,i) = recip2b(2,1,cnt)*f1 + recip2b(2,2,cnt)*f2 
     &      + recip2b(2,3,cnt)*f3
         frc(3,i) = recip2b(3,1,cnt)*f1 + recip2b(3,2,cnt)*f2 
     &      + recip2b(3,3,cnt)*f3
      end do
      e = 0.5d0 * e
      emtemp = emtemp + e
      do l1 = 1, npole3b
         i=pnum(l1)
      !   ii = ipole(i)
c         demtemp(1,l1) = demtemp(1,l1) + frc(1,i)
c         demtemp(2,l1) = demtemp(2,l1) + frc(2,i)
c         demtemp(3,l1) = demtemp(3,l1) + frc(3,i)

         demtemp(1,i) = demtemp(1,i) + frc(1,i)
         demtemp(2,i) = demtemp(2,i) + frc(2,i)
         demtemp(3,i) = demtemp(3,i) + frc(3,i)
      end do
c
...
c
         ...
         call fphi_to_cphi3bclust2b (npole3b,pnum,fphidp,cphi,cnt)
c
