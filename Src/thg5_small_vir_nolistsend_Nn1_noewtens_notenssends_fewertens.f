...
c     &  ntpair_dEtmp,dEindextmp,dEd1tmp,dEd2tmp,dEp1tmp,dEp2tmp)
      subroutine thg5_small_vir_nolistsend_Nn1_noewtens_notenssends2
      use sizes
      use atoms
      use boxes
      use chgpot
...
c      real*8, allocatable :: demi(:,:)
c      real*8, allocatable :: demk(:,:)
...
c      real*8, allocatable :: dlocal1d(:,:) 
c      real*8, allocatable :: dlocal2d(:,:)
c      real*8, allocatable :: dlocal1p(:,:)
c      real*8, allocatable :: dlocal2p(:,:)
...
c      real*8 t1xxp,t1xyp,t1xzp,t1yxp,t1yyp,t1yzp,t1zxp,t1zyp,t1zzp
c      real*8 t2xxp,t2xyp,t2xzp,t2yxp,t2yyp,t2yzp,t2zxp,t2zyp,t2zzp
...
c      real*8, allocatable :: taulocal1d(:,:)
c      real*8, allocatable :: taulocal2d(:,:)
c      real*8, allocatable :: taulocal1p(:,:)
c      real*8, allocatable :: taulocal2p(:,:)
      integer ik,ierr
      logical done_ik
      integer nlocaltau1,nlocaltau2
c      integer, allocatable :: ilocaltau1(:,:)
c      integer, allocatable :: ilocaltau2(:,:)
c      integer ntpair_dEtmp,dEindextmp(2,*)
c      real*8 dEd1tmp(9,*),dEd2tmp(9,*),dEp1tmp(9,*),dEp2tmp(9,*)
c      integer toffset0tau1,toffset0tau2,maxlocaltau
c      integer, allocatable :: toffsettau1(:)
c      integer, allocatable :: toffsettau2(:)
      external erfc
c      real*8, allocatable :: frcztaulocal1d(:,:)
c      real*8, allocatable :: frcytaulocal1d(:,:)
c      real*8, allocatable :: frcxtaulocal1d(:,:)
c      real*8, allocatable :: frcztaulocal1p(:,:)
c      real*8, allocatable :: frcytaulocal1p(:,:)
c      real*8, allocatable :: frcxtaulocal1p(:,:)
c      real*8, allocatable :: frcztaulocal2d(:,:)
c      real*8, allocatable :: frcytaulocal2d(:,:)
c      real*8, allocatable :: frcxtaulocal2d(:,:)
c      real*8, allocatable :: frcztaulocal2p(:,:)
c      real*8, allocatable :: frcytaulocal2p(:,:)
c      real*8, allocatable :: frcxtaulocal2p(:,:)
...
c      nlocaltau2 = 0
      ...
      do i=1,npole
       ...
       !       dEp2_3(1,kkk,i)=0
            do j1=1,4
              elsttau1_index(j1,kkk,i)=0
              elsttau2_index(j1,kkk,i)=0
            end do
         end do
      end do
c      toffset0 = 0
...
c      if(numtasks_emreal.lt.numtasks) then
c         taskid_offset=taskid-2
c      else
c         taskid_offset=taskid
c      end if
      
c      sz=last_emreal2(taskid_offset)-start_emreal2(taskid_offset)+1
...
C  FEWER UNNECESSARY SHARED BELOW.

c!$OMP PARALLEL default(private) shared(fieldt,fieldtp,
c!$OMP& npole,electric,dielec,off2,toffset,
c!$OMP& toffset0,dEindextmp,ntpair_dE,dEd1_3,dEd2_3,dEp1_3,dEp2_3,
...
c!$OMP& taskid,tau1indextmp,tau2indextmp,
c!$OMP& taud1,taud2,taup1,taup2,ntpair_tau1,
c!$OMP& ntpair_tau2,toffsettau1,toffset0tau1,toffsettau2,
c!$OMP& toffset0tau2,maxlocaltau,taskid_offset,
c!$OMP& frcztau1dtot_3, frcztau1ptot_3,frcxtau1dtot_3,frcxtau1ptot_3,
c!$OMP& frcytau1dtot_3,frcytau1ptot_3,
c!$OMP& frcztau2dtot_3,frcztau2ptot_3,frcxtau2dtot_3,frcxtau2ptot_3,
c!$OMP& frcytau2dtot_3,frcytau2ptot_3,
c!$OMP& sz,elstgrad,elsttau1,elsttau2,nlocal,elsttau1_index,
c!$OMP& elsttau2_index,
c!$OMP& nlocaltau1,nlocaltau2,use,use_intra)
c!$OMP& firstprivate(pscale,dscale)

C  FEWER UNNECESSARY SHARED BELOW.
c!$OMP PARALLEL default(private) shared(fieldt,fieldtp,
c!$OMP& npole,electric,dielec,off2,
c!$OMP& dEd1_3,dEd2_3,dEp1_3,dEp2_3,
c!$OMP& n,x,y,z,rpole,ipole,pdamp,thole,n12,i12,
c!$OMP& p2scale,n13,p3scale,i13,n14,i14,p4scale,
c!$OMP& np11,ip11,p41scale,n15,p5scale,i15,
c!$OMP& np12,ip12,d1scale,d2scale,np13,ip13,d3scale,np14,ip14,d4scale,
c!$OMP& nelst2,elst2,xaxis,yaxis,zaxis,f,aewald,
c!$OMP& use,use_intra)
c!$OMP& firstprivate(pscale,dscale)
c
c!$OMP DO reduction(+:fieldt,fieldtp)
c!$OMP& schedule(guided)


!$OMP PARALLEL default(private) shared(fieldt,
!$OMP& npole,electric,dielec,off2,
!$OMP& dEd1_3,dEd2_3,
!$OMP& n,x,y,z,rpole,ipole,pdamp,thole,
!$OMP& np11,ip11,
...
!$OMP DO reduction(+:fieldt)
!$OMP& schedule(guided)

c      do i=start_emreal2(taskid_offset),last_emreal2(taskid_offset)
c         iter3=i-start_emreal2(taskid_offset)+1
      do i=1,npole
        ii = ipole(i)
         iz = zaxis(i)
         ix = xaxis(i)
...
c                   fieldtp(j,i) = fieldtp(j,i) + fid(j)*pscale(kk)
c                   fieldtp(j,k) = fieldtp(j,k) + fkd(j)*pscale(kk)
                  end do
...
C     being subtracted.   
            ...
            fact1= 0.5d0*rr5*dsc3
c            fact2= 0.5d0*rr5*psc3

            t2xxd = t2xxd + fact1*xr*(ck*xr + dk(1)) 
c            t2xxp = t2xxp + fact2*xr*(ck*xr + dk(1)) 
            t2xyd = t2xyd + fact1*xr*(ck*yr + dk(2))
c            t2xyp = t2xyp + fact2*xr*(ck*yr + dk(2))
            t2xzd = t2xzd + fact1*xr*(ck*zr + dk(3))
c            t2xzp = t2xzp + fact2*xr*(ck*zr + dk(3))

            t2yxd = t2yxd + fact1*yr*(ck*xr + dk(1))
c            t2yxp = t2yxp + fact2*yr*(ck*xr + dk(1))
            t2yyd = t2yyd + fact1*yr*(ck*yr + dk(2)) 
c            t2yyp = t2yyp + fact2*yr*(ck*yr + dk(2)) 
            t2yzd = t2yzd + fact1*yr*(ck*zr + dk(3))
c            t2yzp = t2yzp + fact2*yr*(ck*zr + dk(3))

            t2zxd = t2zxd + fact1*zr*(ck*xr + dk(1))
c            t2zxp = t2zxp + fact2*zr*(ck*xr + dk(1))
            t2zyd = t2zyd + fact1*zr*(ck*yr + dk(2))
c            t2zyp = t2zyp + fact2*zr*(ck*yr + dk(2))
            t2zzd = t2zzd + fact1*zr*(ck*zr + dk(3)) 
c            t2zzp = t2zzp + fact2*zr*(ck*zr + dk(3)) 

            t1xxd = t1xxd - fact1*xr*(ci*xr - di(1)) 
c            t1xxp = t1xxp - fact2*xr*(ci*xr - di(1)) 
            t1xyd = t1xyd - fact1*xr*(ci*yr - di(2))
c            t1xyp = t1xyp - fact2*xr*(ci*yr - di(2))
            t1xzd = t1xzd - fact1*xr*(ci*zr - di(3))
c            t1xzp = t1xzp - fact2*xr*(ci*zr - di(3))

            t1yxd = t1yxd - fact1*yr*(ci*xr - di(1))
c            t1yxp = t1yxp - fact2*yr*(ci*xr - di(1))
            t1yyd = t1yyd - fact1*yr*(ci*yr - di(2)) 
c            t1yyp = t1yyp - fact2*yr*(ci*yr - di(2)) 
            t1yzd = t1yzd - fact1*yr*(ci*zr - di(3))
c            t1yzp = t1yzp - fact2*yr*(ci*zr - di(3))

            t1zxd = t1zxd - fact1*zr*(ci*xr - di(1))
c            t1zxp = t1zxp - fact2*zr*(ci*xr - di(1))
            t1zyd = t1zyd - fact1*zr*(ci*yr - di(2))
c            t1zyp = t1zyp - fact2*zr*(ci*yr - di(2))
            t1zzd = t1zzd - fact1*zr*(ci*zr - di(3)) 
c            t1zzp = t1zzp - fact2*zr*(ci*zr - di(3)) 
...
C  Ewald
            ...
            fact1=-0.5d0*rr7*dsc5*sc(3)
c            fact2=-0.5d0*rr7*psc5*sc(3)
            
            t1xxd = t1xxd + fact1*xr*xr
c            t1xxp = t1xxp + fact2*xr*xr
            t1xyd = t1xyd + fact1*xr*yr
c            t1xyp = t1xyp + fact2*xr*yr
            t1xzd = t1xzd + fact1*xr*zr
c            t1xzp = t1xzp + fact2*xr*zr

            t1yxd = t1yxd + fact1*yr*xr
c            t1yxp = t1yxp + fact2*yr*xr
            t1yyd = t1yyd + fact1*yr*yr
c            t1yyp = t1yyp + fact2*yr*yr
            t1yzd = t1yzd + fact1*yr*zr
c            t1yzp = t1yzp + fact2*yr*zr

            t1zxd = t1zxd + fact1*zr*xr
c            t1zxp = t1zxp + fact2*zr*xr
            t1zyd = t1zyd + fact1*zr*yr
c            t1zyp = t1zyp + fact2*zr*yr
            t1zzd = t1zzd + fact1*zr*zr
c            t1zzp = t1zzp + fact2*zr*zr

            !fact1=0.5d0*rr7*dsc5*sc(4)
            !fact2=0.5d0*rr7*psc5*sc(4)

            fact1=-0.5d0*rr7*dsc5*sc(4)
c            fact2=-0.5d0*rr7*psc5*sc(4)

            t2xxd = t2xxd + fact1*xr*xr
c            t2xxp = t2xxp + fact2*xr*xr
            t2xyd = t2xyd + fact1*xr*yr
c            t2xyp = t2xyp + fact2*xr*yr
            t2xzd = t2xzd + fact1*xr*zr
c            t2xzp = t2xzp + fact2*xr*zr

            t2yxd = t2yxd + fact1*yr*xr
c            t2yxp = t2yxp + fact2*yr*xr
            t2yyd = t2yyd + fact1*yr*yr
c            t2yyp = t2yyp + fact2*yr*yr
            t2yzd = t2yzd + fact1*yr*zr
c            t2yzp = t2yzp + fact2*yr*zr

            t2zxd = t2zxd + fact1*zr*xr
c            t2zxp = t2zxp + fact2*zr*xr
            t2zyd = t2zyd + fact1*zr*yr
c            t2zyp = t2zyp + fact2*zr*yr
            t2zzd = t2zzd + fact1*zr*zr
c            t2zzp = t2zzp + fact2*zr*zr
...
c            t1xxp = t1xxp + fact2*qir(1)*xr
            t1xyd = t1xyd + fact1*qir(2)*xr
c            t1xyp = t1xyp + fact2*qir(2)*xr
            t1xzd = t1xzd + fact1*qir(3)*xr
c            t1xzp = t1xzp + fact2*qir(3)*xr

            t1yxd = t1yxd + fact1*qir(1)*yr
c            t1yxp = t1yxp + fact2*qir(1)*yr
            t1yyd = t1yyd + fact1*qir(2)*yr
c            t1yyp = t1yyp + fact2*qir(2)*yr
            t1yzd = t1yzd + fact1*qir(3)*yr
c            t1yzp = t1yzp + fact2*qir(3)*yr

            t1zxd = t1zxd + fact1*qir(1)*zr
c            t1zxp = t1zxp + fact2*qir(1)*zr
            t1zyd = t1zyd + fact1*qir(2)*zr
c            t1zyp = t1zyp + fact2*qir(2)*zr
            t1zzd = t1zzd + fact1*qir(3)*zr
c            t1zzp = t1zzp + fact2*qir(3)*zr

            t2xxd = t2xxd - fact1*qkr(1)*xr
c            t2xxp = t2xxp - fact2*qkr(1)*xr
            t2xyd = t2xyd - fact1*qkr(2)*xr
c            t2xyp = t2xyp - fact2*qkr(2)*xr
            t2xzd = t2xzd - fact1*qkr(3)*xr
c            t2xzp = t2xzp - fact2*qkr(3)*xr

            t2yxd = t2yxd - fact1*qkr(1)*yr
c            t2yxp = t2yxp - fact2*qkr(1)*yr
            t2yyd = t2yyd - fact1*qkr(2)*yr
c            t2yyp = t2yyp - fact2*qkr(2)*yr
            t2yzd = t2yzd - fact1*qkr(3)*yr
c            t2yzp = t2yzp - fact2*qkr(3)*yr

            t2zxd = t2zxd - fact1*qkr(1)*zr
c            t2zxp = t2zxp - fact2*qkr(1)*zr
            t2zyd = t2zyd - fact1*qkr(2)*zr
c            t2zyp = t2zyp - fact2*qkr(2)*zr
            t2zzd = t2zzd - fact1*qkr(3)*zr
c            t2zzp = t2zzp - fact2*qkr(3)*zr

            !fact1=-0.5d0*rr9*dsc7*sc(6)
            !fact2=-0.5d0*rr9*psc7*sc(6)
            fact1=0.5d0*rr9*dsc7*sc(6)
c            fact2=0.5d0*rr9*psc7*sc(6)
            t2xxd = t2xxd + fact1*xr*xr
c            t2xxp = t2xxp + fact2*xr*xr
            t2xyd = t2xyd + fact1*yr*xr
c            t2xyp = t2xyp + fact2*yr*xr
            t2xzd = t2xzd + fact1*zr*xr
c            t2xzp = t2xzp + fact2*zr*xr

            t2yxd = t2yxd + fact1*xr*yr
c            t2yxp = t2yxp + fact2*xr*yr
            t2yyd = t2yyd + fact1*yr*yr
c            t2yyp = t2yyp + fact2*yr*yr
            t2yzd = t2yzd + fact1*zr*yr
c            t2yzp = t2yzp + fact2*zr*yr

            t2zxd = t2zxd + fact1*xr*zr
c            t2zxp = t2zxp + fact2*xr*zr
            t2zyd = t2zyd + fact1*yr*zr
c            t2zyp = t2zyp + fact2*yr*zr
            t2zzd = t2zzd + fact1*zr*zr
c            t2zzp = t2zzp + fact2*zr*zr
            ...
            fact1=0.5d0*rr9*dsc7*sc(5)
c            fact2=0.5d0*rr9*psc7*sc(5)
            t1xxd = t1xxd - fact1*xr*xr
c            t1xxp = t1xxp - fact2*xr*xr
            t1xyd = t1xyd - fact1*yr*xr
c            t1xyp = t1xyp - fact2*yr*xr
            t1xzd = t1xzd - fact1*zr*xr
c            t1xzp = t1xzp - fact2*zr*xr

            t1yxd = t1yxd - fact1*xr*yr
c            t1yxp = t1yxp - fact2*xr*yr
            t1yyd = t1yyd - fact1*yr*yr
c            t1yyp = t1yyp - fact2*yr*yr
            t1yzd = t1yzd - fact1*zr*yr
c            t1yzp = t1yzp - fact2*zr*yr

            t1zxd = t1zxd - fact1*xr*zr
c            t1zxp = t1zxp - fact2*xr*zr
            t1zyd = t1zyd - fact1*yr*zr
c            t1zyp = t1zyp - fact2*yr*zr
            t1zzd = t1zzd - fact1*zr*zr
c            t1zzp = t1zzp - fact2*zr*zr
...
c            gfri_p(2) = -rr3*ck*psc3 + rr5*sc(4)*psc5 - rr7*sc(6)*psc7
C 
            ...
            t2xxd = t2xxd + 0.5d0*gfri_d(2)
c            t2xxp = t2xxp + 0.5d0*gfri_p(2)
            t2yyd = t2yyd + 0.5d0*gfri_d(2)
c            t2yyp = t2yyp + 0.5d0*gfri_p(2)
            t2zzd = t2zzd + 0.5d0*gfri_d(2)
c            t2zzp = t2zzp + 0.5d0*gfri_p(2)

c
C 3RD TERM W/OUT SCREENING
c
            gfri_d(3) =  rr3*ci*dsc3 + rr5*sc(3)*dsc5 + rr7*sc(5)*dsc7
c            gfri_p(3) =  rr3*ci*psc3 + rr5*sc(3)*psc5 + rr7*sc(5)*psc7
            ...
            t1xxd = t1xxd + 0.5d0*gfri_d(3)
c            t1xxp = t1xxp + 0.5d0*gfri_p(3)
            t1yyd = t1yyd + 0.5d0*gfri_d(3)
c            t1yyp = t1yyp + 0.5d0*gfri_p(3)
            t1zzd = t1zzd + 0.5d0*gfri_d(3)
c            t1zzp = t1zzp + 0.5d0*gfri_p(3)
...
c
            ...
            fact1=0.5d0*rr5*dsc5
c            fact2=0.5d0*rr5*psc5

            t1xxd = t1xxd + fact1*di(1)*xr
c            t1xxp = t1xxp + fact2*di(1)*xr
            t1xyd = t1xyd + fact1*di(1)*yr
c            t1xyp = t1xyp + fact2*di(1)*yr
            t1xzd = t1xzd + fact1*di(1)*zr
c            t1xzp = t1xzp + fact2*di(1)*zr

            t1yxd = t1yxd + fact1*di(2)*xr
c            t1yxp = t1yxp + fact2*di(2)*xr
            t1yyd = t1yyd + fact1*di(2)*yr
c            t1yyp = t1yyp + fact2*di(2)*yr
            t1yzd = t1yzd + fact1*di(2)*zr
c            t1yzp = t1yzp + fact2*di(2)*zr

            t1zxd = t1zxd + fact1*di(3)*xr
c            t1zxp = t1zxp + fact2*di(3)*xr
            t1zyd = t1zyd + fact1*di(3)*yr
c            t1zyp = t1zyp + fact2*di(3)*yr
            t1zzd = t1zzd + fact1*di(3)*zr
c            t1zzp = t1zzp + fact2*di(3)*zr

c
C 5TH TERM W/OUT SCREENING
c
            t2xxd = t2xxd + fact1*dk(1)*xr
c            t2xxp = t2xxp + fact2*dk(1)*xr
            t2xyd = t2xyd + fact1*dk(1)*yr
c            t2xyp = t2xyp + fact2*dk(1)*yr
            t2xzd = t2xzd + fact1*dk(1)*zr
c            t2xzp = t2xzp + fact2*dk(1)*zr

            t2yxd = t2yxd + fact1*dk(2)*xr
c            t2yxp = t2yxp + fact2*dk(2)*xr
            t2yyd = t2yyd + fact1*dk(2)*yr
c            t2yyp = t2yyp + fact2*dk(2)*yr
            t2yzd = t2yzd + fact1*dk(2)*zr
c            t2yzp = t2yzp + fact2*dk(2)*zr

            t2zxd = t2zxd + fact1*dk(3)*xr
c            t2zxp = t2zxp + fact2*dk(3)*xr
            t2zyd = t2zyd + fact1*dk(3)*yr
c            t2zyp = t2zyp + fact2*dk(3)*yr
            t2zzd = t2zzd + fact1*dk(3)*zr
c            t2zzp = t2zzp + fact2*dk(3)*zr
...
c            t2xxp = t2xxp + fact2*qk(1)
            t2xyd = t2xyd + fact1*qk(4)
c            t2xyp = t2xyp + fact2*qk(4)
            t2xzd = t2xzd + fact1*qk(7)
c            t2xzp = t2xzp + fact2*qk(7)

            t2yxd = t2yxd + fact1*qk(2)
c            t2yxp = t2yxp + fact2*qk(2)
            t2yyd = t2yyd + fact1*qk(5)
c            t2yyp = t2yyp + fact2*qk(5)
            t2yzd = t2yzd + fact1*qk(8)
c            t2yzp = t2yzp + fact2*qk(8)

            t2zxd = t2zxd + fact1*qk(3)
c            t2zxp = t2zxp + fact2*qk(3)
            t2zyd = t2zyd + fact1*qk(6)
c            t2zyp = t2zyp + fact2*qk(6)
            t2zzd = t2zzd + fact1*qk(9)
c            t2zzp = t2zzp + fact2*qk(9)

            t1xxd = t1xxd - fact1*qi(1)
c            t1xxp = t1xxp - fact2*qi(1)
            t1xyd = t1xyd - fact1*qi(4)
c            t1xyp = t1xyp - fact2*qi(4)
            t1xzd = t1xzd - fact1*qi(7)
c            t1xzp = t1xzp - fact2*qi(7)

            t1yxd = t1yxd - fact1*qi(2)
c            t1yxp = t1yxp - fact2*qi(2)
            t1yyd = t1yyd - fact1*qi(5)
c            t1yyp = t1yyp - fact2*qi(5)
            t1yzd = t1yzd - fact1*qi(8)
c            t1yzp = t1yzp - fact2*qi(8)

            t1zxd = t1zxd - fact1*qi(3)
c            t1zxp = t1zxp - fact2*qi(3)
            t1zyd = t1zyd - fact1*qi(6)
c            t1zyp = t1zyp - fact2*qi(6)
            t1zzd = t1zzd - fact1*qi(9)
c            t1zzp = t1zzp - fact2*qi(9)
c
...
c            t1xyp = t1xyp + fact2*qir(1)*yr
            t1xzd = t1xzd + fact1*qir(1)*zr
c            t1xzp = t1xzp + fact2*qir(1)*zr

            t1yxd = t1yxd + fact1*qir(2)*xr
c            t1yxp = t1yxp + fact2*qir(2)*xr
            t1yyd = t1yyd + fact1*qir(2)*yr
c            t1yyp = t1yyp + fact2*qir(2)*yr
            t1yzd = t1yzd + fact1*qir(2)*zr
c            t1yzp = t1yzp + fact2*qir(2)*zr

            t1zxd = t1zxd + fact1*qir(3)*xr
c            t1zxp = t1zxp + fact2*qir(3)*xr
            t1zyd = t1zyd + fact1*qir(3)*yr
c            t1zyp = t1zyp + fact2*qir(3)*yr
            t1zzd = t1zzd + fact1*qir(3)*zr
c            t1zzp = t1zzp + fact2*qir(3)*zr
c
...
c            t2xyp = t2xyp - fact2*qkr(1)*yr
            t2xzd = t2xzd - fact1*qkr(1)*zr
c            t2xzp = t2xzp - fact2*qkr(1)*zr

            t2yxd = t2yxd - fact1*qkr(2)*xr
c            t2yxp = t2yxp - fact2*qkr(2)*xr
            t2yyd = t2yyd - fact1*qkr(2)*yr
c            t2yyp = t2yyp - fact2*qkr(2)*yr
            t2yzd = t2yzd - fact1*qkr(2)*zr
c            t2yzp = t2yzp - fact2*qkr(2)*zr

            t2zxd = t2zxd - fact1*qkr(3)*xr
c            t2zxp = t2zxp - fact2*qkr(3)*xr
            t2zyd = t2zyd - fact1*qkr(3)*yr
c            t2zyp = t2zyp - fact2*qkr(3)*yr
            t2zzd = t2zzd - fact1*qkr(3)*zr
c            t2zzp = t2zzp - fact2*qkr(3)*zr
...
c            fact2=-0.5d0*rr3*pscale(kk)
            t2xxd = t2xxd + fact1*ddsc3(1)*(ck*xr + dk(1))
c            t2xxp = t2xxp + fact2*ddsc3(1)*(ck*xr + dk(1)) 
            t2xyd = t2xyd + fact1*ddsc3(1)*(ck*yr + dk(2)) 
c            t2xyp = t2xyp + fact2*ddsc3(1)*(ck*yr + dk(2)) 
            t2xzd = t2xzd + fact1*ddsc3(1)*(ck*zr + dk(3)) 
c            t2xzp = t2xzp + fact2*ddsc3(1)*(ck*zr + dk(3)) 
            t2yxd = t2yxd + fact1*ddsc3(2)*(ck*xr + dk(1)) 
c            t2yxp = t2yxp + fact2*ddsc3(2)*(ck*xr + dk(1)) 
            t2yyd = t2yyd + fact1*ddsc3(2)*(ck*yr + dk(2)) 
c            t2yyp = t2yyp + fact2*ddsc3(2)*(ck*yr + dk(2)) 
            t2yzd = t2yzd + fact1*ddsc3(2)*(ck*zr + dk(3)) 
c            t2yzp = t2yzp + fact2*ddsc3(2)*(ck*zr + dk(3)) 
            t2zxd = t2zxd + fact1*ddsc3(3)*(ck*xr + dk(1)) 
c            t2zxp = t2zxp + fact2*ddsc3(3)*(ck*xr + dk(1)) 
            t2zyd = t2zyd + fact1*ddsc3(3)*(ck*yr + dk(2)) 
c            t2zyp = t2zyp + fact2*ddsc3(3)*(ck*yr + dk(2)) 
            t2zzd = t2zzd + fact1*ddsc3(3)*(ck*zr + dk(3)) 
c            t2zzp = t2zzp + fact2*ddsc3(3)*(ck*zr + dk(3)) 

            fact1=-0.5d0*rr3*dscale(kk)
c            fact2=-0.5d0*rr3*pscale(kk)
            t1xxd = t1xxd + fact1*ddsc3(1)*(-ci*xr + di(1)) 
c            t1xxp = t1xxp + fact2*ddsc3(1)*(-ci*xr + di(1)) 
            t1xyd = t1xyd + fact1*ddsc3(1)*(-ci*yr + di(2)) 
c            t1xyp = t1xyp + fact2*ddsc3(1)*(-ci*yr + di(2)) 
            t1xzd = t1xzd + fact1*ddsc3(1)*(-ci*zr + di(3)) 
c            t1xzp = t1xzp + fact2*ddsc3(1)*(-ci*zr + di(3)) 
            t1yxd = t1yxd + fact1*ddsc3(2)*(-ci*xr + di(1)) 
c            t1yxp = t1yxp + fact2*ddsc3(2)*(-ci*xr + di(1)) 
            t1yyd = t1yyd + fact1*ddsc3(2)*(-ci*yr + di(2)) 
c            t1yyp = t1yyp + fact2*ddsc3(2)*(-ci*yr + di(2)) 
            t1yzd = t1yzd + fact1*ddsc3(2)*(-ci*zr + di(3)) 
c            t1yzp = t1yzp + fact2*ddsc3(2)*(-ci*zr + di(3)) 
            t1zxd = t1zxd + fact1*ddsc3(3)*(-ci*xr + di(1)) 
c            t1zxp = t1zxp + fact2*ddsc3(3)*(-ci*xr + di(1)) 
            t1zyd = t1zyd + fact1*ddsc3(3)*(-ci*yr + di(2)) 
c            t1zyp = t1zyp + fact2*ddsc3(3)*(-ci*yr + di(2)) 
            t1zzd = t1zzd + fact1*ddsc3(3)*(-ci*zr + di(3)) 
c            t1zzp = t1zzp + fact2*ddsc3(3)*(-ci*zr + di(3)) 

c         
C  TENSOR TERMS CORRESPONDING TO TEMP5 FOR 'fridmp(j)' terms
c
            fact1=0.5d0*rr5*dscale(kk)*sc(3)
c            fact2=0.5d0*rr5*pscale(kk)*sc(3)
            t1xxd = t1xxd + ddsc5(1)*fact1*xr 
c            t1xxp = t1xxp + ddsc5(1)*fact2*xr 
            t1xyd = t1xyd + ddsc5(1)*fact1*yr 
c            t1xyp = t1xyp + ddsc5(1)*fact2*yr 
            t1xzd = t1xzd + ddsc5(1)*fact1*zr 
c            t1xzp = t1xzp + ddsc5(1)*fact2*zr 

            t1yxd = t1yxd + ddsc5(2)*fact1*xr 
c            t1yxp = t1yxp + ddsc5(2)*fact2*xr 
            t1yyd = t1yyd + ddsc5(2)*fact1*yr 
c            t1yyp = t1yyp + ddsc5(2)*fact2*yr 
            t1yzd = t1yzd + ddsc5(2)*fact1*zr 
c            t1yzp = t1yzp + ddsc5(2)*fact2*zr 

            t1zxd = t1zxd + ddsc5(3)*fact1*xr 
c            t1zxp = t1zxp + ddsc5(3)*fact2*xr 
            t1zyd = t1zyd + ddsc5(3)*fact1*yr 
c            t1zyp = t1zyp + ddsc5(3)*fact2*yr 
            t1zzd = t1zzd + ddsc5(3)*fact1*zr 
c            t1zzp = t1zzp + ddsc5(3)*fact2*zr 
       
            fact1=0.5d0*rr5*dscale(kk)*sc(4)
c            fact2=0.5d0*rr5*pscale(kk)*sc(4)
            t2xxd = t2xxd + ddsc5(1)*fact1*xr 
c            t2xxp = t2xxp + ddsc5(1)*fact2*xr 
            t2xyd = t2xyd + ddsc5(1)*fact1*yr 
c            t2xyp = t2xyp + ddsc5(1)*fact2*yr 
            t2xzd = t2xzd + ddsc5(1)*fact1*zr 
c            t2xzp = t2xzp + ddsc5(1)*fact2*zr 

            t2yxd = t2yxd + ddsc5(2)*fact1*xr 
c            t2yxp = t2yxp + ddsc5(2)*fact2*xr 
            t2yyd = t2yyd + ddsc5(2)*fact1*yr 
c            t2yyp = t2yyp + ddsc5(2)*fact2*yr 
            t2yzd = t2yzd + ddsc5(2)*fact1*zr 
c            t2yzp = t2yzp + ddsc5(2)*fact2*zr 

            t2zxd = t2zxd + ddsc5(3)*fact1*xr 
c            t2zxp = t2zxp + ddsc5(3)*fact2*xr 
            t2zyd = t2zyd + ddsc5(3)*fact1*yr 
c            t2zyp = t2zyp + ddsc5(3)*fact2*yr 
            t2zzd = t2zzd + ddsc5(3)*fact1*zr 
c            t2zzp = t2zzp + ddsc5(3)*fact2*zr 

            fact1=-rr5*dscale(kk)
c            fact2=-rr5*pscale(kk)
            t1xxd = t1xxd + fact1*qir(1)*ddsc5(1)
c            t1xxp = t1xxp + fact2*qir(1)*ddsc5(1)
            t1xyd = t1xyd + fact1*qir(2)*ddsc5(1)
c            t1xyp = t1xyp + fact2*qir(2)*ddsc5(1)
            t1xzd = t1xzd + fact1*qir(3)*ddsc5(1)
c            t1xzp = t1xzp + fact2*qir(3)*ddsc5(1)

            t1yxd = t1yxd + fact1*qir(1)*ddsc5(2)
c            t1yxp = t1yxp + fact2*qir(1)*ddsc5(2)
            t1yyd = t1yyd + fact1*qir(2)*ddsc5(2)
c            t1yyp = t1yyp + fact2*qir(2)*ddsc5(2)
            t1yzd = t1yzd + fact1*qir(3)*ddsc5(2)
c            t1yzp = t1yzp + fact2*qir(3)*ddsc5(2)

            t1zxd = t1zxd + fact1*qir(1)*ddsc5(3)
c            t1zxp = t1zxp + fact2*qir(1)*ddsc5(3)
            t1zyd = t1zyd + fact1*qir(2)*ddsc5(3)
c            t1zyp = t1zyp + fact2*qir(2)*ddsc5(3)
            t1zzd = t1zzd + fact1*qir(3)*ddsc5(3)
c            t1zzp = t1zzp + fact2*qir(3)*ddsc5(3)

            t2xxd = t2xxd - fact1*qkr(1)*ddsc5(1)
c            t2xxp = t2xxp - fact2*qkr(1)*ddsc5(1)
            t2xyd = t2xyd - fact1*qkr(2)*ddsc5(1)
c            t2xyp = t2xyp - fact2*qkr(2)*ddsc5(1)
            t2xzd = t2xzd - fact1*qkr(3)*ddsc5(1)
c            t2xzp = t2xzp - fact2*qkr(3)*ddsc5(1)

            t2yxd = t2yxd - fact1*qkr(1)*ddsc5(2)
c            t2yxp = t2yxp - fact2*qkr(1)*ddsc5(2)
            t2yyd = t2yyd - fact1*qkr(2)*ddsc5(2)
c            t2yyp = t2yyp - fact2*qkr(2)*ddsc5(2)
            t2yzd = t2yzd - fact1*qkr(3)*ddsc5(2)
c            t2yzp = t2yzp - fact2*qkr(3)*ddsc5(2)

            t2zxd = t2zxd - fact1*qkr(1)*ddsc5(3)
c            t2zxp = t2zxp - fact2*qkr(1)*ddsc5(3)
            t2zyd = t2zyd - fact1*qkr(2)*ddsc5(3)
c            t2zyp = t2zyp - fact2*qkr(2)*ddsc5(3)
            t2zzd = t2zzd - fact1*qkr(3)*ddsc5(3)
c            t2zzp = t2zzp - fact2*qkr(3)*ddsc5(3)

            fact1=-0.5d0*rr7*dscale(kk)*sc(6)
c            fact2=-0.5d0*rr7*pscale(kk)*sc(6)
            t2xxd = t2xxd + fact1*xr*ddsc7(1)
c            t2xxp = t2xxp + fact2*xr*ddsc7(1)
            t2xyd = t2xyd + fact1*yr*ddsc7(1)
c            t2xyp = t2xyp + fact2*yr*ddsc7(1)
            t2xzd = t2xzd + fact1*zr*ddsc7(1)
c            t2xzp = t2xzp + fact2*zr*ddsc7(1)

            t2yxd = t2yxd + fact1*xr*ddsc7(2)
c            t2yxp = t2yxp + fact2*xr*ddsc7(2)
            t2yyd = t2yyd + fact1*yr*ddsc7(2)
c            t2yyp = t2yyp + fact2*yr*ddsc7(2)
            t2yzd = t2yzd + fact1*zr*ddsc7(2)
c            t2yzp = t2yzp + fact2*zr*ddsc7(2)

            t2zxd = t2zxd + fact1*xr*ddsc7(3)
c            t2zxp = t2zxp + fact2*xr*ddsc7(3)
            t2zyd = t2zyd + fact1*yr*ddsc7(3)
c            t2zyp = t2zyp + fact2*yr*ddsc7(3)
            t2zzd = t2zzd + fact1*zr*ddsc7(3)
c            t2zzp = t2zzp + fact2*zr*ddsc7(3)

            fact1=0.5d0*rr7*dscale(kk)*sc(5)
c            fact2=0.5d0*rr7*pscale(kk)*sc(5)
            t1xxd = t1xxd + fact1*xr*ddsc7(1)
c            t1xxp = t1xxp + fact2*xr*ddsc7(1)
            t1xyd = t1xyd + fact1*yr*ddsc7(1)
c            t1xyp = t1xyp + fact2*yr*ddsc7(1)
            t1xzd = t1xzd + fact1*zr*ddsc7(1)
c            t1xzp = t1xzp + fact2*zr*ddsc7(1)

            t1yxd = t1yxd + fact1*xr*ddsc7(2)
c            t1yxp = t1yxp + fact2*xr*ddsc7(2)
            t1yyd = t1yyd + fact1*yr*ddsc7(2)
c            t1yyp = t1yyp + fact2*yr*ddsc7(2)
            t1yzd = t1yzd + fact1*zr*ddsc7(2)
c            t1yzp = t1yzp + fact2*zr*ddsc7(2)

            t1zxd = t1zxd + fact1*xr*ddsc7(3)
c            t1zxp = t1zxp + fact2*xr*ddsc7(3)
            t1zyd = t1zyd + fact1*yr*ddsc7(3)
c            t1zyp = t1zyp + fact2*yr*ddsc7(3)
            t1zzd = t1zzd + fact1*zr*ddsc7(3)
c            t1zzp = t1zzp + fact2*zr*ddsc7(3)
c
...
c      tau1p(1)=tau1p(1)+ (0.5d0*rr5*psc5*dixr(1)*xr
c     &        +0.5d0*2.0d0*rr5*psc5*(yr*qi(3)-zr*qi(2))
c     &         - rr7*psc7*rxqir(1)*xr)!*(-1.0d0)
c      tau1p(2)=tau1p(2)+(-0.5d0*rr3*psc3*(-di(3))
c     &             +0.5d0*rr5*psc5*dixr(1)*yr
c     &          + 0.5d0*2.0d0*rr5*psc5*(qir(3) + yr*qi(6)-zr*qi(5))
c     &          -rr7*psc7*rxqir(1)*yr)!*(-1.0d0)
c      tau1p(3)=tau1p(3)+ (-0.5d0*rr3*psc3*di(2)
c     &           + 0.5d0*rr5*psc5*dixr(1)*zr
c     &          + 0.5d0*2.0d0*rr5*psc5*(-qir(2) + yr*qi(9)-zr*qi(8))
c     &          -rr7*psc7*rxqir(1)*zr)!*(-1.0d0)
c
c
c      tau1p(4)= tau1p(4)+ (-0.5d0*rr3*psc3*di(3)
c     &             + 0.5d0*rr5*psc5*dixr(2)*xr
c     &          +0.5d0*2.0d0*rr5*psc5*( -qir(3) + zr*qi(1) - xr*qi(3))
c     &          -rr7*psc7*rxqir(2)*xr)!*(-1.0d0)
c      tau1p(5)= tau1p(5)+ (0.5d0*rr5*psc5*dixr(2)*yr
c     &         +0.5d0*2.0d0*rr5*psc5*( zr*qi(4)-xr*qi(6))
c     &         -rr7*psc7*rxqir(2)*yr)!*(-1.0d0)
c      tau1p(6)=tau1p(6)+ (-0.5d0*rr3*psc3*(-di(1))
c     &               +0.5d0*rr5*psc5*dixr(2)*zr
c     &         +0.5d0*2.0d0*rr5*psc5*( qir(1) + zr*qi(7)-xr*qi(9) )
c     &         -rr7*psc7*rxqir(2)*zr)!*(-1.0d0)
c
c      tau1p(7)=tau1p(7)+ (-0.5d0*rr3*psc3*(-di(2))
c     &         + 0.5d0*rr5*psc5*dixr(3)*xr
c     &         +0.5d0*2.0d0*rr5*psc5*(qir(2) + xr*qi(2)-yr*qi(1) )
c     &         -rr7*psc7*rxqir(3)*xr)!*(-1.0d0)
c      tau1p(8)=tau1p(8)+ (-0.5d0*rr3*psc3*di(1)
c     &         + 0.5d0*rr5*psc5*dixr(3)*yr
c     &        +0.5d0*2.0d0*rr5*psc5*(-qir(1) + xr*qi(5)-yr*qi(4) )
c     &        -rr7*psc7*rxqir(3)*yr)!*(-1.0d0)
c      tau1p(9)=tau1p(9)+ (0.5d0*rr5*psc5*dixr(3)*zr
c     &        +0.5d0*2.0d0*rr5*psc5*(xr*qi(8)-yr*qi(7))
c     &        -rr7*psc7*rxqir(3)*zr)!*(-1.0d0)
c
c      tau2p(1)=tau2p(1) + (0.5d0*rr5*psc5*dkxr(1)*xr
c     &        -0.5d0*2.0d0*rr5*psc5*(yr*qk(3)-zr*qk(2))
c     &        -(-rr7*psc7)*rxqkr(1)*xr)!*(-1.0d0)
c      tau2p(2)=tau2p(2)+ (-0.5d0*rr3*psc3*(-dk(3))
c     &           +0.5d0*rr5*psc5*dkxr(1)*yr
c     &       -0.5d0*2.0d0*rr5*psc5*(qkr(3) + yr*qk(6)-zr*qk(5))
c     &       -(-rr7*psc7)*rxqkr(1)*yr)!*(-1.0d0)
c      tau2p(3)=tau2p(3)+ (-0.5d0*rr3*psc3*dk(2)
c     &          + 0.5d0*rr5*psc5*dkxr(1)*zr
c     &        -0.5d0*2.0d0*rr5*psc5*(-qkr(2) +yr*qk(9)-zr*qk(8))
c     &        -(-rr7*psc7)*rxqkr(1)*zr)!*(-1.0d0)
c
c
c      tau2p(4)= tau2p(4)+ (-0.5d0*rr3*psc3*dk(3)
c     &                 + 0.5d0*rr5*psc5*dkxr(2)*xr
c     &          -0.5d0*2.0d0*rr5*psc5*( -qkr(3) + zr*qk(1) - xr*qk(3))
c     &          -(-rr7*psc7)*rxqkr(2)*xr)!*(-1.0d0)
c      tau2p(5)= tau2p(5)+ (0.5d0*rr5*psc5*dkxr(2)*yr
c     &         -0.5d0*2.0d0*rr5*psc5*( zr*qk(4)-xr*qk(6))
c     &         -(-rr7*psc7)*rxqkr(2)*yr)!*(-1.0d0)
c      tau2p(6)=tau2p(6)+ (-0.5d0*rr3*psc3*(-dk(1))
c     &                  +0.5d0*rr5*psc5*dkxr(2)*zr
c     &         -0.5d0*2.0d0*rr5*psc5*( qkr(1) + zr*qk(7)-xr*qk(9) )
c     &         -(-rr7*psc7)*rxqkr(2)*zr)!*(-1.0d0)
c
c      tau2p(7)=tau2p(7)+ (-0.5d0*rr3*psc3*(-dk(2))
c     &                     +0.5d0*rr5*psc5*dkxr(3)*xr
c     &         -0.5d0*2.0d0*rr5*psc5*(qkr(2) + xr*qk(2)-yr*qk(1) )
c     &         -(-rr7*psc7)*rxqkr(3)*xr)!*(-1.0d0)
c      tau2p(8)=tau2p(8)+ (-0.5d0*rr3*psc3*dk(1)
c     &                     + 0.5d0*rr5*psc5*dkxr(3)*yr
c     &        -0.5d0*2.0d0*rr5*psc5*(-qkr(1) + xr*qk(5)-yr*qk(4) )
c     &        -(-rr7*psc7)*rxqkr(3)*yr)!*(-1.0d0)
c      tau2p(9)=tau2p(9)+(0.5d0*rr5*psc5*dkxr(3)*zr
c     &        -0.5d0*2.0d0*rr5*psc5*(xr*qk(8)-yr*qk(7))
c     &        -(-rr7*psc7)*rxqkr(3)*zr)!*(-1.0d0)
...
c!$OMP CRITICAL
             !nlocal = nlocal + 1
             !elstgrad(kkk,i) = nlocal
             
             dEd1_3(1,kkk,i)=t1xxd
             dEd1_3(2,kkk,i)=t1xyd
             dEd1_3(3,kkk,i)=t1xzd
             dEd1_3(4,kkk,i)=t1yxd
             dEd1_3(5,kkk,i)=t1yyd
             dEd1_3(6,kkk,i)=t1yzd
             dEd1_3(7,kkk,i)=t1zxd
             dEd1_3(8,kkk,i)=t1zyd
             dEd1_3(9,kkk,i)=t1zzd

             dEd2_3(1,kkk,i)=t2xxd
             dEd2_3(2,kkk,i)=t2xyd
             dEd2_3(3,kkk,i)=t2xzd
             dEd2_3(4,kkk,i)=t2yxd
             dEd2_3(5,kkk,i)=t2yyd
             dEd2_3(6,kkk,i)=t2yzd
             dEd2_3(7,kkk,i)=t2zxd
             dEd2_3(8,kkk,i)=t2zyd
             dEd2_3(9,kkk,i)=t2zzd

c             dEp1_3(1,kkk,i)=t1xxp
c             dEp1_3(2,kkk,i)=t1xyp
c             dEp1_3(3,kkk,i)=t1xzp
c             dEp1_3(4,kkk,i)=t1yxp
c             dEp1_3(5,kkk,i)=t1yyp
c             dEp1_3(6,kkk,i)=t1yzp
c             dEp1_3(7,kkk,i)=t1zxp
c             dEp1_3(8,kkk,i)=t1zyp
c             dEp1_3(9,kkk,i)=t1zzp
c
c             dEp2_3(1,kkk,i)=t2xxp
c             dEp2_3(2,kkk,i)=t2xyp
c             dEp2_3(3,kkk,i)=t2xzp
c             dEp2_3(4,kkk,i)=t2yxp
c             dEp2_3(5,kkk,i)=t2yyp
c             dEp2_3(6,kkk,i)=t2yzp
c             dEp2_3(7,kkk,i)=t2zxp
c             dEp2_3(8,kkk,i)=t2zyp
c             dEp2_3(9,kkk,i)=t2zzp


c           call torque3_dEtensor_72015_taunew3_pt1_vir_notenssends(i,
c     &  ttm2i,tau1d,tau1p,nlocaltau1,
c     &  nlocal,
c     &  k,kkk)
c           call torque3_dEtensor_72015_taunew3_pt1_vir_notenssends2(i,
c     &  ttm2i,tau1d,tau1p,
c     &  k,kkk)

           call torque3_dEtensor_72015_taunew4_pt1_vir_notenssends2(i,
     &  ttm2i,tau1d,
     &  k,kkk)
...
c           call torque3_dEtensor_72015_taunew3_pt2_vir_notenssends(i,
c     &  ttm3i,tau2d,tau2p,nlocaltau2,
c     &  nlocal,
c     &  k,kkk)

c           call torque3_dEtensor_72015_taunew3_pt2_vir_notenssends2(i,
c     &  ttm3i,tau2d,tau2p,
c     &  k,kkk)

           call torque3_dEtensor_72015_taunew4_pt2_vir_notenssends2(i,
     &  ttm3i,tau2d,
     &  k,kkk)
...
c
         ...
         ntpair_dE = nlocal
         ntpair_tau1 = nlocaltau1
         ntpair_tau2 = nlocaltau2
