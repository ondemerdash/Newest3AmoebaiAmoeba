...
c
c     This is modified from the original prtdyn to be able to
c     write out multiple dyn files at select intervals, as 
c     opposed to the original where the dyn file is overwritten.
c
c
      subroutine prtdyn_writeDyns(ext,lext)
      use sizes
      use atoms
      use boxes
      use files
...
c
c     Added variables for writeDyns
c
      integer lext
      character*7 ext
c
...
c
      idyn = freeunit ()
ccc      dynfile = filename(1:leng)//'.dyn' ! Original version
      dynfile = filename(1:leng)//'.dyn.'//ext(1:lext) ! Modified verison for writeDyns
