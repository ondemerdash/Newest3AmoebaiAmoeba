...
c
c
      subroutine induce0a_3b_scf_totfieldnpole(npole3b,pnum,uind,uinp)
      use sizes
      use atoms
      use inform
      use iounit
      ...
      use units
      use uprior
      use pme, only: nfft1, nfft2, nfft3
      use totfield
      implicit none
      integer i,j,k,iter
      integer maxiter
...
c      real*8 uind(3,npole3b),uinp(3,npole3b)
      real*8 uind(3,npole),uinp(3,npole)
      real*8, allocatable :: minv(:)
      real*8 qgrid(2,nfft1,nfft2,nfft3)
      real*8 qfac(nfft1,nfft2,nfft3)
...
c      allocate (udir(3,npole3b))
c      allocate (udirp(3,npole3b))
c      allocate (minv(6*maxulst*npole3b))
      allocate (minv(6*maxulst*npole))
...
c      if (use_ewald) then
c         call dfield0c_3b (npole3b,pnum,field,fieldp,qgrid,qfac)
c      else if (use_mlist) then
c         call dfield0b (field,fieldp)
c      else
c         call dfield0a_3b2 (npole3b,pnum,field,fieldp)
c      end if
...
c            udirp(j,i) = polarity(i) * fieldp(j,i)
            udir(j,i) = polarity(i) * fieldnpole(j,i)
            udirp(j,i) = polarity(i) * fieldpnpole(j,i)
            uind(j,i) = udir(j,i)
            uinp(j,i) = udirp(j,i)
c            udir(j,l1) = polarity(i) * field(j,l1)
c            udirp(j,l1) = polarity(i) * fieldp(j,l1)
c            uind(j,l1) = udir(j,l1)
c            uinp(j,l1) = udirp(j,l1)
         end do
      end do
c
...
c            call ufield0b (field,fieldp)
            ...
            call ufield0a_3b2 (npole3b,pnum,field,fieldp,uind,uinp)
         end if
c
...
c         do i = 1, npole
         do l1= 1,npole3b
            i=pnum(l1)
c            poli(l1) = max(polmin,polarity(i))
...
c     &                       + fieldp(j,i)
c               rsd(j,l1) = (udir(j,l1)-uind(j,l1))/poli(l1)
c     &                       + field(j,l1)
c               rsdp(j,l1) = (udirp(j,l1)-uinp(j,l1))/poli(l1)
c     &                       + fieldp(j,l1)
...
c            call uscale0b (mode,rsd,rsdp,zrsd,zrsdp)
c         else
            call uscale0a_3b2 (npole3b,pnum,mode,rsd,rsdp,zrsd,zrsdp,
     &              minv)
            mode = 'APPLY'
            call uscale0a_3b2 (npole3b,pnum,mode,rsd,rsdp,zrsd,zrsdp,
     &              minv)
...
c         do i = 1, npole
         do l1 =1,npole3b
            ...
            do j = 1, 3
               conj(j,i) = zrsd(j,i)
               conjp(j,i) = zrsdp(j,i)
c               conj(j,l1) = zrsd(j,l1)
c               conjp(j,l1) = zrsdp(j,l1)
            end do
         end do
c
...
c                  vec(j,l1) = uind(j,l1)
c                  vecp(j,l1) = uinp(j,l1)
c                  uind(j,l1) = conj(j,l1)
c                  uinp(j,l1) = conjp(j,l1)
               end do
            end do
            if (use_ewald) then
               call ufield0c_3b (npole3b,pnum,field,fieldp,uind,uinp,
...
c            do i = 1, npole
            do l1 =1,npole3b
               ...
               do j = 1, 3
                  uind(j,i) = vec(j,i)
                  uinp(j,i) = vecp(j,i)
                  vec(j,i) = conj(j,i)/poli(i) - field(j,l1)
                  vecp(j,i) = conjp(j,i)/poli(i) - fieldp(j,l1)
c                  uind(j,l1) = vec(j,l1)
c                  uinp(j,l1) = vecp(j,l1)
c                  vec(j,l1) = conj(j,l1)/poli(l1) - field(j,l1)
c                  vecp(j,l1) = conjp(j,l1)/poli(l1) - fieldp(j,l1)
...
c                  a = a + conj(j,l1)*vec(j,l1)
c                  ap = ap + conjp(j,l1)*vecp(j,l1)
c                  sum = sum + rsd(j,l1)*zrsd(j,l1)
c                  sump = sump + rsdp(j,l1)*zrsdp(j,l1)
...
c                  uind(j,l1) = uind(j,l1) + a*conj(j,l1)
c                  uinp(j,l1) = uinp(j,l1) + ap*conjp(j,l1)
c                  rsd(j,l1) = rsd(j,l1) - a*vec(j,l1)
c                  rsdp(j,l1) = rsdp(j,l1) - ap*vecp(j,l1)
...
c                  b = b + rsd(j,l1)*zrsd(j,l1)
c                  bp = bp + rsdp(j,l1)*zrsdp(j,l1)
               end do
            end do
...
c                  conj(j,l1) = zrsd(j,l1) + b*conj(j,l1)
c                  conjp(j,l1) = zrsdp(j,l1) + bp*conjp(j,l1)
c                  epsd = epsd + rsd(j,l1)*rsd(j,l1)
c                  epsp = epsp + rsdp(j,l1)*rsdp(j,l1)
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine dfield0a_3b2  --  direct induction via double loop  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "dfield0a_3b2" computes the direct electrostatic field due to
c     permanent multipole moments via a double loop
c
c
      subroutine dfield0a_3b2 (npole3b,pnum,field,fieldp)
      use sizes
      use atoms
      use bound
      use cell
...
c      print*,"off2 in dfield0a_3b2",off2 
c
c     perform dynamic allocation of some local arrays
c
      allocate (dscale(n))
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine ufield0a_3b2  --  mutual induction via double loop  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "ufield0a_3b2" computes the mutual electrostatic field due to
c     induced dipole moments via a double loop
c
c
      subroutine ufield0a_3b2 (npole3b,pnum,field,fieldp,uind,uinp)
      use sizes
      use atoms
      use bound
      use cell
...
c      print*,"off2 in ufield0a_3b2",off2
c
c     perform dynamic allocation of some local arrays
c
      allocate (dscale(n))
...
c
c
      subroutine dfield0c_3b (npole3b,pnum,field,fieldp,qgrid,qfac)
      use sizes
      use atoms
      use boxes
      use ewald
...
c
      call udirect1_3b (npole3b,pnum,field,qgrid,qfac)
c      do i = 1, npole
      do l1 =1,npole3b
         do j = 1, 3
...
c
c
      subroutine udirect1_3b (npole3b,pnum,field,qgrid,qfac)
      use sizes
      use bound
      use boxes
      use ewald
      use math
      use mpole
      use pme, only: nfft1,nfft2,nfft3,maxorder,bsorder,igrid,thetai1,
     & thetai2,thetai3,bsmod1,bsmod2,bsmod3
      implicit none
      integer i,j,k,ntot
      integer k1,k2,k3
      integer m1,m2,m3
...
c
      call fftback3b(qgrid)
      call fphi_mpole3b (npole3b,pnum,fphi,qgrid)
c
...
c     assign PME grid and perform 3-D FFT forward transform
c
      call grid_uind3b (npole3b,pnum,fuind,fuinp,qgrid)
      call fftfront3b(qgrid)
c
c     complete the transformation of the PME grid
c
...
c
c
      subroutine uscale0a_3b2 (npole3b,pnum,mode,rsd,rsdp,zrsd,zrsdp,
     &  minv)
      use sizes
      use atoms
      use limits
...
c               zrsd(j,l1) = poli * rsd(j,l1)
c               zrsdp(j,l1) = poli * rsdp(j,l1)
            end do
         end do
c
...
c            do k = i+1, npole
     ...
     &                            + m6*rsdp(3,i)
c                  zrsd(1,l1) = zrsd(1,l1) + m1*rsd(1,l3) + m2*rsd(2,l3)
c     &                           + m3*rsd(3,l3)
c                  zrsd(2,l1) = zrsd(2,l1) + m2*rsd(1,l3) + m4*rsd(2,l3)
c     &                           + m5*rsd(3,l3)
c                  zrsd(3,l1) = zrsd(3,l1) + m3*rsd(1,l3) + m5*rsd(2,l3)
c     &                           + m6*rsd(3,l3)
c                  zrsd(1,l3) = zrsd(1,l3) + m1*rsd(1,l1) + m2*rsd(2,l1)
c     &                           + m3*rsd(3,l1)
c                  zrsd(2,l3) = zrsd(2,l3) + m2*rsd(1,l1) + m4*rsd(2,l1)
c     &                           + m5*rsd(3,l1)
c                  zrsd(3,l3) = zrsd(3,l3) + m3*rsd(1,l1) + m5*rsd(2,l1)
c     &                           + m6*rsd(3,l1)
c                  zrsdp(1,l1)= zrsdp(1,l1) + m1*rsdp(1,l3)+m2*rsdp(2,l3)
c     &                            + m3*rsdp(3,l3)
c                  zrsdp(2,l1)= zrsdp(2,l1) + m2*rsdp(1,l3)+m4*rsdp(2,l3)
c     &                            + m5*rsdp(3,l3)
c                  zrsdp(3,l1)= zrsdp(3,l1) + m3*rsdp(1,l3)+m5*rsdp(2,l3)
c     &                            + m6*rsdp(3,l3)
c                 zrsdp(1,l3)= zrsdp(1,l3) + m1*rsdp(1,l1) +m2*rsdp(2,l1)
c     &                            + m3*rsdp(3,l1)
c                zrsdp(2,l3) = zrsdp(2,l3) + m2*rsdp(1,l1) +m4*rsdp(2,l1)
c     &                            + m5*rsdp(3,l1)
c                zrsdp(3,l3) = zrsdp(3,l3) + m3*rsdp(1,l1) +m5*rsdp(2,l1)
c     &                            + m6*rsdp(3,l1)
