...
c
c
      subroutine induce0a_3b_scf_vacomp1bclust(npole3b,pnum,uind,uinp,
     & cnt)
      use sizes
      use atoms
      use inform
...
c
         !print*,"use_ewaldclust=",use_ewaldclust
         if(use_ewaldclust) then
            call dfield0c_3b_vacomp1b(npole3b,pnum,field,fieldp,cnt)
          !print*,"Did dfield0c_3b_vacomp1b"
         else
         call dfield0b_3b_vacomp1b(npole3b,pnum,field,fieldp,cnt)
         end if
...
c
      ...
      !print*,"maxualt=",maxualt
      !if(use_pred) then
      ! print*,"task=",taskid,"TILT! USE_PRED IS FALSE"
      !end if
         if (use_pred) then
           if(nualt1b(cnt).eq.maxualt) then
            call ulspred1b(npole3b,cnt)
            !do i = 1, npole
            do l1 =1, npole3b
               do j = 1, 3
                  udsum = 0.0d0
                  ...
                  do k = 1, nualt1b(cnt)-1
                   udsum = udsum + bpred1b(k,cnt)*udalt1b(k,j,l1,cnt)
                   upsum = upsum + bpredp1b(k,cnt)*upalt1b(k,j,l1,cnt)
                  end do
                  uind(j,l1) = udsum
                  uinp(j,l1) = upsum
               end do
...
c
          if(use_ewaldclust) then
           if(useboxclust) then
           !call ufield0c_3b_vacompbox1bclust(npole3b,pnum,field,fieldp,
     &     !    uind,uinp,cnt)
           else
           call ufield0c_3b_vacomp (npole3b,pnum,field,fieldp,
     &         uind,uinp)
           end if
           !print*,"After ufield0c_3b_vacomp in 1bclust"
          else
...
c         else
          if(mlistclust) then
           if(useboxclust) then
          !call uscale0b_3b_vacomp1btestbox1bclust(npole3b,pnum,mode,rsd,
     &    !      rsdp,zrsd,zrsdp,cnt)
          !  mode = 'APPLY'
          !call uscale0b_3b_vacomp1btestbox1bclust(npole3b,pnum,mode,rsd,
     &    !      rsdp,zrsd,zrsdp,cnt)
           else
          call uscale0b_3b_vacomp1btest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp,cnt)
            mode = 'APPLY'
          call uscale0b_3b_vacomp1btest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp,cnt)
          ...
          call uscale0a_3b_vacomptest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp)
            mode = 'APPLY'
          call uscale0a_3b_vacomptest(npole3b,pnum,mode,rsd,rsdp,
     &          zrsd,zrsdp)
...
c
      if (use_pred) then
         nualt1b(cnt) = min(nualt1b(cnt)+1,maxualt)
         !do i = 1, npole
         do l1 = 1, npole3b
            i=pnum(l1)
            do j = 1, 3
               do k = nualt1b(cnt), 2, -1
                  udalt1b(k,j,l1,cnt) = udalt1b(k-1,j,l1,cnt)
                  upalt1b(k,j,l1,cnt) = upalt1b(k-1,j,l1,cnt)
               end do
               udalt1b(1,j,l1,cnt) = uind(j,l1)
               upalt1b(1,j,l1,cnt) = uinp(j,l1)
            end do
         end do
      end if
...
c
c
      subroutine dfield0b_3b_vacomp1b(npole3b,pnum,field,fieldp,
     &  cnt)
      use sizes
      use atoms
      use bound
...
!$OMP& n14,i14,n15,i15,np11,ip11,np12,ip12,np13,ip13,np14,ip14,nelst1b,
!$OMP& elst1b,cut2,poltyp,ntpair,tindex,tdipdip,
...
c
c
      subroutine uscale0b_3b_vacomp1b(npole3b,pnum,mode,rsd,rsdp,zrsd,
     &  zrsdp,cnt)
      use sizes
      use atoms
      use limits
...
c
         ...
         !print*,"usolvcut in uscale0b",usolvcut
         j = 0
c         do i = 1, npole-1
!$OMP PARALLEL default(private) shared(npole,mindex,minv,nulst1b,ulst1b,
...
c            do k = i+1, npole
            m = mindex(l1)
            !do l3 = l1+1, npole3b
            do kkk=1,nulst1b(l1,cnt)
               l3=ulst1b(kkk,l1,cnt)
            !   k=pnum(l3) 
            !   kk = ipole(k)
            !   xr = x(kk) - x(ii)
            !   yr = y(kk) - y(ii)
...
!$OMP& np12,ip12,np13,ip13,np14,ip14,nulst1b,ulst1b,mindex,minv,
...
c
c
      subroutine uscale0b_3b_vacomp1btest(npole3b,pnum,mode,rsd,rsdp,
     &  zrsd,zrsdp,cnt)
      use sizes
      use atoms
      use limits
