      subroutine polar_load_balance7_half
      use mpidat
      use neigh3b
      use molcul
      implicit none
...
c      integer, allocatable :: molnew(:)      
     ...
     &          allocate (start_polar(0:numtasks_polz-1))
         if (.not. allocated(last_polar)) 
     &          allocate (last_polar(0:numtasks_polz-1))
...
c      print*,"nmol_nonzero",nmol_nonzero
      ...
      avg_load=nmollst_tot/numtasks_polz
c      print*,"avg_load=",avg_load

      if(mod(nmollst_tot,numtasks_polz).gt.0) then
        avg_load=avg_load+1
...
c      end if
      ...
      subroutine bcast_polar_load_balance_half
      use mpidat
      use molcul
      implicit none
      include 'mpif.h'
      ...
      if (first.and.(taskid.ne.numtasks_emreal+2)) then
         first = .false.
         if (.not. allocated(start_polar))
     &          allocate (start_polar(0:numtasks_polz-1))
         if (.not. allocated(last_polar))
...
c         print*,"numtasks_polar in bcast",numtasks_polar
         call mpi_bcast(numtasks_polar,1,mpi_integer,numtasks_emreal+2,
     &   mpi_comm_world,ierr)
      call mpi_bcast(start_polar,numtasks_polz,mpi_integer,
     &   numtasks_emreal+2,mpi_comm_world,ierr)
       call mpi_bcast(last_polar,numtasks_polz,mpi_integer,
     &   numtasks_emreal+2,mpi_comm_world,ierr)
         call mpi_bcast(molnew,nmol,mpi_integer,numtasks_emreal+2,
     &   mpi_comm_world,ierr)
