...
c      real*8 dEd1tmp(9,*),dEd2tmp(9,*),dEp1tmp(9,*),dEp2tmp(9,*)
      ...
      real*8 rr3_field,dir,qix,qiy,qiz,dkr,qkx,qky,qkz,fid(3),fkd(3)
      real*8 qkr2,qir2
c
...
c            bcn(1) = bn(1)-(1.0d0-scale3*dscale(kk))*rr3
c            bcn(2) = bn(2)-3.0d0*(1.0d0-scale5*dscale(kk))*rr5_field
c            bcn(3) = bn(3)-15.0d0*(1.0d0-scale7*dscale(kk))*rr7_field
c            fimd(1) = -xr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(1) + 2.0d0*bcn(2)*qkr(1)
c            fimd(2) = -yr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(2) + 2.0d0*bcn(2)*qkr(2)
c            fimd(3) = -zr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(3) + 2.0d0*bcn(2)*qkr(3)
c            fkmd(1) = xr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(1) - 2.0d0*bcn(2)*qir(1)
c            fkmd(2) = yr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(2) - 2.0d0*bcn(2)*qir(2)
c            fkmd(3) = zr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(3) - 2.0d0*bcn(2)*qir(3)
c            bcn(1) = bn(1)-(1.0d0-scale3*pscale(kk))*rr3
c            bcn(2) = bn(2)-3.0d0*(1.0d0-scale5*pscale(kk))*rr5_field
c            bcn(3) = bn(3)-15.0d0*(1.0d0-scale7*pscale(kk))*rr7_field
c            fimp(1) = -xr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(1) + 2.0d0*bcn(2)*qkr(1)
c            fimp(2) = -yr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(2) + 2.0d0*bcn(2)*qkr(2)
c            fimp(3) = -zr*(bcn(1)*ck-bcn(2)*sc(4)+bcn(3)*sc(6))
c     &                     - bcn(1)*dk(3) + 2.0d0*bcn(2)*qkr(3)
c            fkmp(1) = xr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(1) - 2.0d0*bcn(2)*qir(1)
c            fkmp(2) = yr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(2) - 2.0d0*bcn(2)*qir(2)
c            fkmp(3) = zr*(bcn(1)*ci+bcn(2)*sc(3)+bcn(3)*sc(5))
c     &                     - bcn(1)*di(3) - 2.0d0*bcn(2)*qir(3)
c            do j = 1, 3
c              fieldt(j,i) = fieldt(j,i) + fimd(j)
c              fieldt(j,k) = fieldt(j,k) + fkmd(j)
c              fieldtp(j,i) = fieldtp(j,i) + fimp(j)
c              fieldtp(j,k) = fieldtp(j,k) + fkmp(j)
c            end do

C   HERE WE'RE CALCULATING A FIELD W/OUT EWALD SCREENING
          !  ck = rpole(1,k)
          !  dk(1) = rpole(2,k)
          !  dk(2) = rpole(3,k)
          !  dk(3) = rpole(4,k)
          ...
          !  qk(9) = rpole(13,k) ! qkzz
                  ...
                  dir = di(1)*xr + di(2)*yr + di(3)*zr !dix*xr + diy*yr + diz*zr
                  qix = qi(1)*xr + qi(2)*yr + qi(3)*zr !qixx*xr + qixy*yr + qixz*zr
                  qiy = qi(2)*xr + qi(5)*yr + qi(6)*zr !qixy*xr + qiyy*yr + qiyz*zr
                  qiz = qi(3)*xr + qi(6)*yr + qi(9)*zr !qixz*xr + qiyz*yr + qizz*zr
                  qir2 = qix*xr + qiy*yr + qiz*zr
                  dkr = dk(1)*xr + dk(2)*yr + dk(3)*zr !dkx*xr + dky*yr + dkz*zr
                  qkx = qk(1)*xr + qk(2)*yr + qk(3)*zr !qkxx*xr + qkxy*yr + qkxz*zr
                  qky = qk(2)*xr + qk(5)*yr + qk(6)*zr !qkxy*xr + qkyy*yr + qkyz*zr
                  qkz = qk(3)*xr + qk(6)*yr + qk(9)*zr !qkxz*xr + qkyz*yr + qkzz*zr
                  qkr2 = qkx*xr + qky*yr + qkz*zr
                 fid(1)= -xr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr2)
     &                   - rr3_field*dk(1) + 2.0d0*rr5_field*qkx !- rr3_field*dkx + 2.0d0*rr5_field*qkx
                 fid(2)= -yr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr2)
     &                   - rr3_field*dk(2) + 2.0d0*rr5_field*qky  !- rr3_field*dky + 2.0d0*rr5_field*qky
                 fid(3)= -zr*(rr3_field*ck-rr5_field*dkr+rr7_field*qkr2)
     &                   - rr3_field*dk(3) + 2.0d0*rr5_field*qkz ! - rr3_field*dkz + 2.0d0*rr5_field*qkz
                 fkd(1) = xr*(rr3_field*ci+rr5_field*dir+rr7_field*qir2)
     &                   - rr3_field*di(1) - 2.0d0*rr5_field*qix !- rr3_field*dix - 2.0d0*rr5_field*qix
                 fkd(2) = yr*(rr3_field*ci+rr5_field*dir+rr7_field*qir2)
     &                   - rr3_field*di(2) - 2.0d0*rr5_field*qiy ! - rr3_field*diy - 2.0d0*rr5_field*qiy
                 fkd(3) = zr*(rr3_field*ci+rr5_field*dir+rr7_field*qir2)
     &                   - rr3_field*di(3) - 2.0d0*rr5_field*qiz ! - rr3_field*diz - 2.0d0*rr5_field*qiz
