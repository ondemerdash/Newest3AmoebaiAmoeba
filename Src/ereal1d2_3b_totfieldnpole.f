...
c     &   eptemp,deptemp,virtemp)
      subroutine ereal1d2_3b_totfieldnpole(npole3b,pnum,uind,uinp,
     &  eptemp,deptemp,virtemp,nembedlst,embedlst)
      use sizes
      use atoms
      use bound
      use boxes
      ...
      character*7 mode
      logical flag,doi,dok
      real*8, allocatable :: dept1(:,:)
      real*8, allocatable :: dept2(:,:)
      real*8 virt(3,3),eptt
      integer kkk
      integer nembedlst(npole3b),embedlst(maxelst,npole3b)
      logical dorli
      external erfc
...
c      ewaldcut3b_2=ewaldcut3b*ewaldcut3b
      ewaldcut3b_2= off2     
      aewald3b=aewald
      !print*,"off2=",off2,"aewald3b=",aewald3b
c      do l1 = 1, npole3b-1
c         i = pnum(l1)
      !do i=1,npole
      !   doi=.false.
...
!$OMP& gfr,gfri,gti,gtri,dorli,dok,k1,l1,l2)
...
c               scip(3) = 0.0d0
c               scip(4) = uinp(1,l2)*xr + uinp(2,l2)*yr + uinp(3,l2)*zr
...
c     DON'T FORGET TO UNCOMMENT FOR ACTUAL RUN.  THE FOLLOWING
C     TERM IS COMMENTED OUT FOR DEBUGGING PURPOSES.
...
c
               do j = 1, 3
                  ...
                  ! THE FOLLOWING 2 LINES ARE COMMENTED OUT FOR DEBUGGING  ONLY!
