...
C BROADCAST PME ENERGY,GRADIENT,VIRIAL
     ...
     &       mpi_comm_world,ierr)
           call mpi_bcast(viremrecip,3*3,mpi_real8,master1,
     &       mpi_comm_world,ierr)
c            call mpi_bcast(ep,1,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
c           call mpi_bcast(dep,3*n,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
c           call mpi_bcast(virpol,3*3,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
...
C GET TOTAL ENERGY
     ...
     &                   + ev + em+ep3b+ep
               ...
               print*,"ep in verlet/nose/beeman setup",ep
C GET TOTAL GRADIENT
                do i = 1, n
                  do j = 1, 3
               derivs(j,i) =deb(j,i) + dea(j,i) + deba(j,i) +deub(j,i)
     &                   + deopb(j,i) + det(j,i) + dept(j,i) + dett(j,i)
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)+dep(j,i)
                  end do
                 print*,"dep3b_x",i,dep(1,i)!+dep3bmut(1,i)+dep3b1(1,i)
     &         !          +dep3b_recip(1,i)
                 print*,"dep3b_y",i,dep(2,i)!+dep3bmut(2,i)+dep3b1(2,i)
     &          !         +dep3b_recip(2,i)
                 print*,"dep3b_z",i,dep(3,i)!+dep3bmut(3,i)+dep3b1(3,i)
...
C VIA BROADCAST
           !  t1=mpi_Wtime()
             call gradient_emrecip
           !  t2=mpi_Wtime()
           !  print*,"PermElec PME time, taskid",t2-t1,taskid
            end if
...
C ALL TASKS CALCULATE A PORTION OF ENERGY/GRADIENT/VIRIAL OF POLARIZATION. 
             ...
             !call smallsmooth2_gradient_polar_load_balanced 
             !t6=mpi_Wtime()
             !print*,"Polz time,taskid",t6-t5,taskid
            if(taskid.eq.master) then
              call empole1a_PolarStripped
            end if
...
C CALCULATE ACCELERATIONS AND VELOCITIES
       ...
       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b,ep
...
c     &       mpi_comm_world,ierr)
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)+dep(j,i)
       ...
       print*,"Noseistep Vdw,PermElec,Polz",istep,ev,em,ep3b,ep
