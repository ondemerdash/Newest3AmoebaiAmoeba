...
c      do j=0,numtasks_emreal-1
c        taskidemreal(j)=j
c      end do
c
c      do j=numtasks_emreal,numtasks-1
c         k=j-numtasks_emreal
c        taskidother(k)=j
c      end do
c
c      call MPI_COMM_GROUP(MPI_COMM_WORLD, orig_group, ierr)
c
c      if(taskid.lt.numtasks_emreal) then
c        call mpi_group_incl(orig_group,numtasks_emreal,taskidemreal,
c     &        emreal_group,ierr)
c      else
c        call mpi_group_incl(orig_group,numtasks-numtasks_emreal,
c     &       taskidother,emreal_group,ierr)
c      end if

c        call MPI_COMM_CREATE(MPI_COMM_WORLD, emreal_group,
c     &            emreal_comm,ierr)
...
c
        ...
        diffatom = 6
         ...
         if (integrate .eq. 'VERLET') then
          do diffatom_step = 1, 3001, 300
          if(diffatom_step.eq.3001) then
             diffatom = 3000
          else
             diffatom = diffatom_step
          end if
          do istep = 1, 3

            if(taskid.eq.master) then
             z(diffatom) = z(diffatom) + delx
...
c            end if
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)
       ...
       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b
              ...
              analyticgx(istep)=derivs(1,diffatom)
              analyticg_dep3bx(istep)=dep3b(1,diffatom)
              analyticgy(istep)=derivs(2,diffatom)
              analyticg_dep3by(istep)=dep3b(2,diffatom)
              analyticgz(istep)=derivs(3,diffatom)
              analyticg_dep3bz(istep)=dep3b(3,diffatom)
       
              deallocate (derivs)
c              call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
                print*,"Atom Number", diffatom
     ...
     & (efindif_ep3b(3)-efindif_ep3b(1))/2.0d0/delx,analyticg_dep3bz(2)
         ...
         if (integrate .eq. 'NOSE-HOOVER') then
          do istep = 1, nstep
            if(taskid.eq.master) then
             z(diffatom) = z(diffatom) + delx
c             call nose_pt1(istep,dt,press)
             call gradient_setup
             !call alloc3b_vdwlist_erecip
             call alloc3b_vdwlist
             if(listbcast) then
...
c              call nose_pt2(istep,dt,press,energy) 
            end if
          end do
