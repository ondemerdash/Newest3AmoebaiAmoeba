...
c
c
      subroutine kewald_eastman_list
      use sizes
      use atoms
      use bound
      use boxes
...
c
      ...
      aewald=(sqrt(-log(2.0d0*0.0001d0)))/ewaldcut
c
...
c
      ...
      if (.not. allocated(numpolechunk)) allocate (numpolechunk(nchunk))
      if (.not. allocated(chunksitelist)) 
     &       allocate (chunksitelist(n,nchunk))
c
