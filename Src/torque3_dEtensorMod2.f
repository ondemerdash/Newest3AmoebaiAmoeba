...
c
c
      subroutine torque3_dEtensorMod2(i,trq1,trq2,frcx,frcy,frcz,demi,
...
c         end do
         ...
         duxy_d = uv(2)*dphidvx_d/(usiz*uvsin) + uw(2)*dphidwx_d/usiz
         duxz_d = uv(3)*dphidvx_d/(usiz*uvsin) + uw(3)*dphidwx_d/usiz

         duyx_d = uv(1)*dphidvy_d/(usiz*uvsin) + uw(1)*dphidwy_d/usiz
         duyy_d = uv(2)*dphidvy_d/(usiz*uvsin) + uw(2)*dphidwy_d/usiz
         duyz_d = uv(3)*dphidvy_d/(usiz*uvsin) + uw(3)*dphidwy_d/usiz

         duzx_d = uv(1)*dphidvz_d/(usiz*uvsin) + uw(1)*dphidwz_d/usiz
         duzy_d = uv(2)*dphidvz_d/(usiz*uvsin) + uw(2)*dphidwz_d/usiz
         duzz_d = uv(3)*dphidvz_d/(usiz*uvsin) + uw(3)*dphidwz_d/usiz

         duxx_p = uv(1)*dphidvx_p/(usiz*uvsin) + uw(1)*dphidwx_p/usiz
         duxy_p = uv(2)*dphidvx_p/(usiz*uvsin) + uw(2)*dphidwx_p/usiz
         duxz_p = uv(3)*dphidvx_p/(usiz*uvsin) + uw(3)*dphidwx_p/usiz

         duyx_p = uv(1)*dphidvy_p/(usiz*uvsin) + uw(1)*dphidwy_p/usiz
         duyy_p = uv(2)*dphidvy_p/(usiz*uvsin) + uw(2)*dphidwy_p/usiz
         duyz_p = uv(3)*dphidvy_p/(usiz*uvsin) + uw(3)*dphidwy_p/usiz

         duzx_p = uv(1)*dphidvz_p/(usiz*uvsin) + uw(1)*dphidwz_p/usiz
         duzy_p = uv(2)*dphidvz_p/(usiz*uvsin) + uw(2)*dphidwz_p/usiz
         duzz_p = uv(3)*dphidvz_p/(usiz*uvsin) + uw(3)*dphidwz_p/usiz

         dvxx_d = -uv(1)*dphidux_d/(vsiz*uvsin)
         dvxy_d = -uv(2)*dphidux_d/(vsiz*uvsin)
         dvxz_d = -uv(3)*dphidux_d/(vsiz*uvsin)

         dvyx_d = -uv(1)*dphiduy_d/(vsiz*uvsin) 
         dvyy_d = -uv(2)*dphiduy_d/(vsiz*uvsin)
         dvyz_d = -uv(3)*dphiduy_d/(vsiz*uvsin)

         dvzx_d = -uv(1)*dphiduz_d/(vsiz*uvsin)
         dvzy_d = -uv(2)*dphiduz_d/(vsiz*uvsin)
         dvzz_d = -uv(3)*dphiduz_d/(vsiz*uvsin)

         dvxx_p = -uv(1)*dphidux_p/(vsiz*uvsin) 
         dvxy_p = -uv(2)*dphidux_p/(vsiz*uvsin)
         dvxz_p = -uv(3)*dphidux_p/(vsiz*uvsin)

         dvyx_p = -uv(1)*dphiduy_p/(vsiz*uvsin)
         dvyy_p = -uv(2)*dphiduy_p/(vsiz*uvsin)
         dvyz_p = -uv(3)*dphiduy_p/(vsiz*uvsin)

         dvzx_p = -uv(1)*dphiduz_p/(vsiz*uvsin)
         dvzy_p = -uv(2)*dphiduz_p/(vsiz*uvsin)
...
c         end do
        ...
        duxy_d=uv(2)*dphidvx_d/(usiz*uvsin) + 0.5d0*uw(2)*dphidwx_d/usiz
        duxz_d=uv(3)*dphidvx_d/(usiz*uvsin) + 0.5d0*uw(3)*dphidwx_d/usiz

        duyx_d=uv(1)*dphidvy_d/(usiz*uvsin) + 0.5d0*uw(1)*dphidwy_d/usiz
        duyy_d=uv(2)*dphidvy_d/(usiz*uvsin) + 0.5d0*uw(2)*dphidwy_d/usiz
        duyz_d=uv(3)*dphidvy_d/(usiz*uvsin) + 0.5d0*uw(3)*dphidwy_d/usiz

        duzx_d=uv(1)*dphidvz_d/(usiz*uvsin) + 0.5d0*uw(1)*dphidwz_d/usiz
        duzy_d=uv(2)*dphidvz_d/(usiz*uvsin) + 0.5d0*uw(2)*dphidwz_d/usiz
        duzz_d=uv(3)*dphidvz_d/(usiz*uvsin) + 0.5d0*uw(3)*dphidwz_d/usiz

        duxx_p=uv(1)*dphidvx_p/(usiz*uvsin) + 0.5d0*uw(1)*dphidwx_p/usiz
        duxy_p=uv(2)*dphidvx_p/(usiz*uvsin) + 0.5d0*uw(2)*dphidwx_p/usiz
        duxz_p=uv(3)*dphidvx_p/(usiz*uvsin) + 0.5d0*uw(3)*dphidwx_p/usiz

        duyx_p=uv(1)*dphidvy_p/(usiz*uvsin) + 0.5d0*uw(1)*dphidwy_p/usiz
        duyy_p=uv(2)*dphidvy_p/(usiz*uvsin) + 0.5d0*uw(2)*dphidwy_p/usiz
        duyz_p=uv(3)*dphidvy_p/(usiz*uvsin) + 0.5d0*uw(3)*dphidwy_p/usiz

        duzx_p=uv(1)*dphidvz_p/(usiz*uvsin) + 0.5d0*uw(1)*dphidwz_p/usiz
        duzy_p=uv(2)*dphidvz_p/(usiz*uvsin) + 0.5d0*uw(2)*dphidwz_p/usiz
        duzz_p=uv(3)*dphidvz_p/(usiz*uvsin) + 0.5d0*uw(3)*dphidwz_p/usiz

        dvxx_d=-uv(1)*dphidux_d/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwx_d/vsiz
        dvxy_d=-uv(2)*dphidux_d/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwx_d/vsiz
        dvxz_d=-uv(3)*dphidux_d/(vsiz*uvsin)+ 0.5d0*vw(3)*dphidwx_d/vsiz

        dvyx_d=-uv(1)*dphiduy_d/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwy_d/vsiz
        dvyy_d=-uv(2)*dphiduy_d/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwy_d/vsiz
        dvyz_d=-uv(3)*dphiduy_d/(vsiz*uvsin)+ 0.5d0*vw(3)*dphidwy_d/vsiz

        dvzx_d=-uv(1)*dphiduz_d/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwz_d/vsiz
        dvzy_d=-uv(2)*dphiduz_d/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwz_d/vsiz
        dvzz_d=-uv(3)*dphiduz_d/(vsiz*uvsin)+ 0.5d0*vw(3)*dphidwz_d/vsiz

        dvxx_p=-uv(1)*dphidux_p/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwx_p/vsiz
        dvxy_p=-uv(2)*dphidux_p/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwx_p/vsiz
        dvxz_p=-uv(3)*dphidux_p/(vsiz*uvsin)+ 0.5d0*vw(3)*dphidwx_p/vsiz

        dvyx_p=-uv(1)*dphiduy_p/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwy_p/vsiz
        dvyy_p=-uv(2)*dphiduy_p/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwy_p/vsiz
        dvyz_p=-uv(3)*dphiduy_p/(vsiz*uvsin)+ 0.5d0*vw(3)*dphidwy_p/vsiz

        dvzx_p=-uv(1)*dphiduz_p/(vsiz*uvsin)+ 0.5d0*vw(1)*dphidwz_p/vsiz
        dvzy_p=-uv(2)*dphiduz_p/(vsiz*uvsin)+ 0.5d0*vw(2)*dphidwz_p/vsiz
