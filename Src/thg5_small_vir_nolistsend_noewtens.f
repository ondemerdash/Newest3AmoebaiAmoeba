...
c     &  ntpair_dEtmp,dEindextmp,dEd1tmp,dEd2tmp,dEp1tmp,dEp2tmp)
      subroutine thg5_small_vir_nolistsend_noewtens 
...
!$OMP& nelst,elst,xaxis,yaxis,zaxis,f,aewald,aewald3b,
...
c
            ...
            ralpha = aewald3b * r
            bn_noew(0) = erfc(ralpha) / r
            alsq2 = 2.0d0 * aewald3b**2
            alsq2n = 0.0d0
            if (aewald3b .gt. 0.0d0)  alsq2n = 1.0d0 / (sqrtpi*aewald3b)
            exp2a = exp(-ralpha**2)
            do j = 1, 5
              bfac = dble(2*j-1)
              alsq2n = alsq2 * alsq2n
              bn_noew(j) = (bfac*bn_noew(j-1)+alsq2n*exp2a) / r2
            end do
...
c
     ...
     &               qi(7)*qk(7) + qi(8)*qk(8) + qi(9)*qk(9)
           ...
           bcn(1) = bn_noew(1)-(1.0d0-scale3*dscale(kk))*rr3
           bcn(2) = bn_noew(2)-3.0d0*(1.0d0-scale5*dscale(kk))*rr5_field
          bcn(3) = bn_noew(3)-15.0d0*(1.0d0-scale7*dscale(kk))*rr7_field
     ...
     &                     - bcn(1)*di(3) - 2.0d0*bcn(2)*qir(3)
            bcn(1) = bn_noew(1)-(1.0d0-scale3*pscale(kk))*rr3
           bcn(2) = bn_noew(2)-3.0d0*(1.0d0-scale5*pscale(kk))*rr5_field
          bcn(3) = bn_noew(3)-15.0d0*(1.0d0-scale7*pscale(kk))*rr7_field
...
c
            fact1=0.5d0*bn_noew(2)
            fact2=0.5d0*bn_noew(3)
            fact3=fact1*ck-fact2*sc(4)+0.5d0*bn_noew(4)*sc(6)

            t2xxd = t2xxd + xr*(xr*(fact1*ck-fact2*sc(4)
     &          +0.5d0*bn_noew(4)*sc(6)) + fact1*dk(1))
            t2xyd = t2xyd + xr*(yr*(fact1*ck-fact2*sc(4)
     &        +0.5d0*bn_noew(4)*sc(6)) + fact1*dk(2))
            t2xzd = t2xzd + xr*(zr*(fact1*ck-fact2*sc(4)
     &         +0.5d0*bn_noew(4)*sc(6)) + fact1*dk(3))

            t2yxd = t2yxd + yr*(xr*(fact1*ck -fact2*sc(4)
     &         +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(1))
            t2yyd = t2yyd + yr*(yr*(fact1*ck -fact2*sc(4)
     &         +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(2))
            t2yzd = t2yzd + yr*(zr*(fact1*ck -fact2*sc(4)
     &          +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(3))

            t2zxd = t2zxd + zr*(xr*(fact1*ck -fact2*sc(4)
     &        +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(1))
            t2zyd = t2zyd + zr*(yr*(fact1*ck -fact2*sc(4)
     &         +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(2))
            t2zzd = t2zzd + zr*(zr*(fact1*ck -fact2*sc(4)
     &          +0.5d0*bn_noew(4)*sc(6))+ fact1*dk(3))

            fact3=fact1*ci+fact2*sc(3)+0.5d0*bn_noew(4)*sc(5)
            t1xxd = t1xxd - xr*(xr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5)) - fact1*di(1))
            t1xyd = t1xyd - xr*(yr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5)) - fact1*di(2))
            t1xzd = t1xzd - xr*(zr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5)) - fact1*di(3))

            t1yxd = t1yxd - yr*(xr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(1))
            t1yyd = t1yyd - yr*(yr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(2))
            t1yzd = t1yzd - yr*(zr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(3))

            t1zxd = t1zxd - zr*(xr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(1))
            t1zyd = t1zyd - zr*(yr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(2))
            t1zzd = t1zzd - zr*(zr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn_noew(4)*sc(5))- fact1*di(3))

            t1xxd = t1xxd + bn_noew(3)*qir(1)*xr
            t1xyd = t1xyd + bn_noew(3)*qir(2)*xr
            t1xzd = t1xzd + bn_noew(3)*qir(3)*xr
            t1yxd = t1yxd + bn_noew(3)*qir(1)*yr
            t1yyd = t1yyd + bn_noew(3)*qir(2)*yr
            t1yzd = t1yzd + bn_noew(3)*qir(3)*yr
            t1zxd = t1zxd + bn_noew(3)*qir(1)*zr
            t1zyd = t1zyd + bn_noew(3)*qir(2)*zr
            t1zzd = t1zzd + bn_noew(3)*qir(3)*zr

            t2xxd = t2xxd - bn_noew(3)*qkr(1)*xr
            t2xyd = t2xyd - bn_noew(3)*qkr(2)*xr
            t2xzd = t2xzd - bn_noew(3)*qkr(3)*xr
            t2yxd = t2yxd - bn_noew(3)*qkr(1)*yr
            t2yyd = t2yyd - bn_noew(3)*qkr(2)*yr
            t2yzd = t2yzd - bn_noew(3)*qkr(3)*yr
            t2zxd = t2zxd - bn_noew(3)*qkr(1)*zr
            t2zyd = t2zyd - bn_noew(3)*qkr(2)*zr
            t2zzd = t2zzd - bn_noew(3)*qkr(3)*zr

c
C 2ND TERM W SCREENING
c
            gfi(2) = -ck*bn_noew(1)+ sc(4)*bn_noew(2) - sc(6)*bn_noew(3)
...
c
            gfi(3) = ci*bn_noew(1) + sc(3)*bn_noew(2) + sc(5)*bn_noew(3)
...
c
            t1xxd = t1xxd + 0.5d0*bn_noew(2)*di(1)*xr
            t1xyd = t1xyd + 0.5d0*bn_noew(2)*di(1)*yr
            t1xzd = t1xzd + 0.5d0*bn_noew(2)*di(1)*zr

            t1yxd = t1yxd + 0.5d0*bn_noew(2)*di(2)*xr
            t1yyd = t1yyd + 0.5d0*bn_noew(2)*di(2)*yr
            t1yzd = t1yzd + 0.5d0*bn_noew(2)*di(2)*zr

            t1zxd = t1zxd + 0.5d0*bn_noew(2)*di(3)*xr
            t1zyd = t1zyd + 0.5d0*bn_noew(2)*di(3)*yr
            t1zzd = t1zzd + 0.5d0*bn_noew(2)*di(3)*zr
c
C 5TH TERM W SCREENING
c
            t2xxd = t2xxd + 0.5d0*bn_noew(2)*dk(1)*xr
            t2xyd = t2xyd + 0.5d0*bn_noew(2)*dk(1)*yr
            t2xzd = t2xzd + 0.5d0*bn_noew(2)*dk(1)*zr

            t2yxd = t2yxd + 0.5d0*bn_noew(2)*dk(2)*xr
            t2yyd = t2yyd + 0.5d0*bn_noew(2)*dk(2)*yr
            t2yzd = t2yzd + 0.5d0*bn_noew(2)*dk(2)*zr

            t2zxd = t2zxd + 0.5d0*bn_noew(2)*dk(3)*xr
            t2zyd = t2zyd + 0.5d0*bn_noew(2)*dk(3)*yr
            t2zzd = t2zzd + 0.5d0*bn_noew(2)*dk(3)*zr
c
C 6TH TERM W SCREENING
c
            t2xxd = t2xxd + bn_noew(2)*qk(1)
            t2xyd = t2xyd + bn_noew(2)*qk(4)
            t2xzd = t2xzd + bn_noew(2)*qk(7)

            t2yxd = t2yxd + bn_noew(2)*qk(2)
            t2yyd = t2yyd + bn_noew(2)*qk(5)
            t2yzd = t2yzd + bn_noew(2)*qk(8)

            t2zxd = t2zxd + bn_noew(2)*qk(3)
            t2zyd = t2zyd + bn_noew(2)*qk(6)
            t2zzd = t2zzd + bn_noew(2)*qk(9)

            t1xxd = t1xxd - bn_noew(2)*qi(1)
            t1xyd = t1xyd - bn_noew(2)*qi(4)
            t1xzd = t1xzd - bn_noew(2)*qi(7)

            t1yxd = t1yxd - bn_noew(2)*qi(2)
            t1yyd = t1yyd - bn_noew(2)*qi(5)
            t1yzd = t1yzd - bn_noew(2)*qi(8)

            t1zxd = t1zxd - bn_noew(2)*qi(3)
            t1zyd = t1zyd - bn_noew(2)*qi(6)
            t1zzd = t1zzd - bn_noew(2)*qi(9)
c
C 7TH TERM W SCREENING
c
            t1xxd = t1xxd + bn_noew(3)*qir(1)*xr
            t1xyd = t1xyd + bn_noew(3)*qir(1)*yr
            t1xzd = t1xzd + bn_noew(3)*qir(1)*zr

            t1yxd = t1yxd + bn_noew(3)*qir(2)*xr
            t1yyd = t1yyd + bn_noew(3)*qir(2)*yr
            t1yzd = t1yzd + bn_noew(3)*qir(2)*zr

            t1zxd = t1zxd + bn_noew(3)*qir(3)*xr
            t1zyd = t1zyd + bn_noew(3)*qir(3)*yr
            t1zzd = t1zzd + bn_noew(3)*qir(3)*zr
c
C 8TH TERM W SCREENING
c
            t2xxd = t2xxd - bn_noew(3)*qkr(1)*xr
            t2xyd = t2xyd - bn_noew(3)*qkr(1)*yr
            t2xzd = t2xzd - bn_noew(3)*qkr(1)*zr

            t2yxd = t2yxd - bn_noew(3)*qkr(2)*xr
            t2yyd = t2yyd - bn_noew(3)*qkr(2)*yr
            t2yzd = t2yzd - bn_noew(3)*qkr(2)*zr

            t2zxd = t2zxd - bn_noew(3)*qkr(3)*xr
            t2zyd = t2zyd - bn_noew(3)*qkr(3)*yr
            t2zzd = t2zzd - bn_noew(3)*qkr(3)*zr
...
C   FIRST, TORQUE TERMS W/ EWALD SCREENING
      ...
      tau1d(1)= tau1d(1)+ 0.5d0*bn_noew(2)*dixr(1)*xr
     &        +0.5d0*2.0d0*bn_noew(2)*(yr*qi(3)-zr*qi(2))
     &         - bn_noew(3)*rxqir(1)*xr
      tau1p(1)=tau1d(1)
      tau1d(2)= tau1d(2) - 0.5d0*bn_noew(1)*(-di(3)) 
     &         + 0.5d0*bn_noew(2)*dixr(1)*yr
     &          + 0.5d0*2.0d0*bn_noew(2)*(qir(3) + yr*qi(6)-zr*qi(5))
     &          -bn_noew(3)*rxqir(1)*yr
      tau1p(2)=tau1d(2)
      tau1d(3)=tau1d(3) -0.5d0*bn_noew(1)*di(2) 
     &          + 0.5d0*bn_noew(2)*dixr(1)*zr
     &          + 0.5d0*2.0d0*bn_noew(2)*(-qir(2) + yr*qi(9)-zr*qi(8))
     &          -bn_noew(3)*rxqir(1)*zr
      tau1p(3)=tau1d(3)

      tau1d(4)= tau1d(4) -0.5d0*bn_noew(1)*di(3) 
     &          + 0.5d0*bn_noew(2)*dixr(2)*xr
     &          +0.5d0*2.0d0*bn_noew(2)*( -qir(3) + zr*qi(1) - xr*qi(3))
     &          -bn_noew(3)*rxqir(2)*xr
      tau1p(4)=tau1d(4)
      tau1d(5)= tau1d(5)+0.5d0*bn_noew(2)*dixr(2)*yr
     &         +0.5d0*2.0d0*bn_noew(2)*( zr*qi(4)-xr*qi(6))
     &         -bn_noew(3)*rxqir(2)*yr
      tau1p(5)=tau1d(5)
      tau1d(6)=tau1d(6) -0.5d0*bn_noew(1)*(-di(1))
     &         +0.5d0*bn_noew(2)*dixr(2)*zr
     &         +0.5d0*2.0d0*bn_noew(2)*( qir(1) + zr*qi(7)-xr*qi(9) )
     &         -bn_noew(3)*rxqir(2)*zr
      tau1p(6)=tau1d(6)

      tau1d(7)=tau1d(7) -0.5d0*bn_noew(1)*(-di(2)) 
     &         + 0.5d0*bn_noew(2)*dixr(3)*xr
     &         +0.5d0*2.0d0*bn_noew(2)*(qir(2) + xr*qi(2)-yr*qi(1) )
     &         -bn_noew(3)*rxqir(3)*xr
      tau1p(7)=tau1d(7)
      tau1d(8)=tau1d(8) -0.5d0*bn_noew(1)*di(1) 
     &        + 0.5d0*bn_noew(2)*dixr(3)*yr
     &        +0.5d0*2.0d0*bn_noew(2)*(-qir(1) + xr*qi(5)-yr*qi(4) )
     &        -bn_noew(3)*rxqir(3)*yr
      tau1p(8)=tau1d(8)
      tau1d(9)=tau1d(9)+0.5d0*bn_noew(2)*dixr(3)*zr
     &        +0.5d0*2.0d0*bn_noew(2)*(xr*qi(8)-yr*qi(7))
     &        -bn_noew(3)*rxqir(3)*zr
      tau1p(9)=tau1d(9)

      tau2d(1)=tau2d(1) + 0.5d0*bn_noew(2)*dkxr(1)*xr
     &        -0.5d0*2.0d0*bn_noew(2)*(yr*qk(3)-zr*qk(2))
     &        -(-bn_noew(3))*rxqkr(1)*xr
      tau2p(1)=tau2d(1)
      tau2d(2)=tau2d(2) -0.5d0*bn_noew(1)*(-dk(3)) 
     &       + 0.5d0*bn_noew(2)*dkxr(1)*yr
     &       -0.5d0*2.0d0*bn_noew(2)*(qkr(3) + yr*qk(6)-zr*qk(5))
     &       -(-bn_noew(3))*rxqkr(1)*yr
      tau2p(2)=tau2d(2)
      tau2d(3)=tau2d(3) -0.5d0*bn_noew(1)*dk(2) 
     &        + 0.5d0*bn_noew(2)*dkxr(1)*zr
     &        -0.5d0*2.0d0*bn_noew(2)*(-qkr(2) +yr*qk(9)-zr*qk(8))
     &        -(-bn_noew(3))*rxqkr(1)*zr
      tau2p(3)=tau2d(3)


      tau2d(4)= tau2d(4) -0.5d0*bn_noew(1)*dk(3) 
     &          + 0.5d0*bn_noew(2)*dkxr(2)*xr
     &          -0.5d0*2.0d0*bn_noew(2)*( -qkr(3) + zr*qk(1) - xr*qk(3))
     &          -(-bn_noew(3))*rxqkr(2)*xr
      tau2p(4)=tau2d(4)
      tau2d(5)= tau2d(5)+0.5d0*bn_noew(2)*dkxr(2)*yr
     &         -0.5d0*2.0d0*bn_noew(2)*( zr*qk(4)-xr*qk(6))
     &         -(-bn_noew(3))*rxqkr(2)*yr
      tau2p(5)=tau2d(5)
      tau2d(6)=tau2d(6) -0.5d0*bn_noew(1)*(-dk(1))
     &         +0.5d0*bn_noew(2)*dkxr(2)*zr
     &         -0.5d0*2.0d0*bn_noew(2)*( qkr(1) + zr*qk(7)-xr*qk(9) )
     &         -(-bn_noew(3))*rxqkr(2)*zr
      tau2p(6)=tau2d(6)

      tau2d(7)=tau2d(7) -0.5d0*bn_noew(1)*(-dk(2)) 
     &         + 0.5d0*bn_noew(2)*dkxr(3)*xr
     &         -0.5d0*2.0d0*bn_noew(2)*(qkr(2) + xr*qk(2)-yr*qk(1) )
     &         -(-bn_noew(3))*rxqkr(3)*xr
      tau2p(7)=tau2d(7)
      tau2d(8)=tau2d(8) -0.5d0*bn_noew(1)*dk(1) 
     &        + 0.5d0*bn_noew(2)*dkxr(3)*yr
     &        -0.5d0*2.0d0*bn_noew(2)*(-qkr(1) + xr*qk(5)-yr*qk(4) )
     &        -(-bn_noew(3))*rxqkr(3)*yr
      tau2p(8)=tau2d(8)
      tau2d(9)=tau2d(9)+0.5d0*bn_noew(2)*dkxr(3)*zr
     &        -0.5d0*2.0d0*bn_noew(2)*(xr*qk(8)-yr*qk(7))
     &        -(-bn_noew(3))*rxqkr(3)*zr
      tau2p(9)=tau2d(9)
...
c      print*,"taskid",taskid,"thrdnum=",tid,"maxlocal=",maxlocal
c      print*,"taskid",taskid,"thrdnum=",tid,"nlocal=",nlocal
c      print*,"taskid",taskid,"thrdnum=",tid,"maxlocaltau=",maxlocaltau
c      print*,"taskid",taskid,"thrdnum=",tid,"nlocaltau1=",nlocaltau1
...
!$OMP END CRITICAL
         ...
         do i = 1, nlocal
          ...
          frcztau1d(j,m)= frcztaulocal1d(j,i)
          frcytau1d(j,m)= frcytaulocal1d(j,i)
          frcxtau1d(j,m)= frcxtaulocal1d(j,i)

          frcztau1p(j,m)= frcztaulocal1p(j,i)
          frcytau1p(j,m)= frcytaulocal1p(j,i)
          frcxtau1p(j,m)= frcxtaulocal1p(j,i)

          frcztau2d(j,m)= frcztaulocal2d(j,i)
          frcytau2d(j,m)= frcytaulocal2d(j,i)
          frcxtau2d(j,m)= frcxtaulocal2d(j,i)

          frcztau2p(j,m)= frcztaulocal2p(j,i)
          frcytau2p(j,m)= frcytaulocal2p(j,i)
          frcxtau2p(j,m)= frcxtaulocal2p(j,i)
         ...
         deallocate (frcztaulocal1d)
         deallocate (frcytaulocal1d)
         deallocate (frcxtaulocal1d)
         deallocate (frcztaulocal1p)
         deallocate (frcytaulocal1p)
         deallocate (frcxtaulocal1p)
         deallocate (frcztaulocal2d)
         deallocate (frcytaulocal2d)
         deallocate (frcxtaulocal2d)
         deallocate (frcztaulocal2p)
         deallocate (frcytaulocal2p)
         deallocate (frcxtaulocal2p)
