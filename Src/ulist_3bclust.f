...
c
c
      subroutine ulist_3bclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use boxes
...
c
      ...
      if (doulstclust3b(counter)) then
         doulstclust3b(counter) = .false.
         if (octahedron) then
            call ubuild_3bclust(npole3b,pnum,counter)
         else
            call ulight_3bclust(npole3b,pnum,counter)
         end if
         return
      end if
c
...
!$OMP DO schedule(guided)
      do l1=1,npole3b
         ...
         yi = y(ii)
         zi = z(ii)
         xr = xi - xuold3b(l1,counter)
         yr = yi - yuold3b(l1,counter)
         zr = zi - zuold3b(l1,counter)
...
c            listsend_mpole =.true.
            update(l1) = .true.
            xuold3b(l1,counter) = xi
            yuold3b(l1,counter) = yi
            zuold3b(l1,counter) = zi
            nulst3b(l1,counter) = 0
           ! do k = i+1, npole
            do l3=l1+1,npole3b
               k=pnum(l3)
               kk = ipole(k)
               ...
               r2 = xr*xr + yr*yr + zr*zr
               if (r2 .le. ubuf2) then
                  nulst3b(l1,counter) = nulst3b(l1,counter) + 1
                  ulst3b(nulst3b(l1,counter),l1,counter) = l3
               end if
            end do
         end if
      end do
...
c            do k = 1, i-1
            do l3 = 1, l1-1
               if (.not. update(l3)) then
                  xr = xi - xuold3b(l3,counter)
                  yr = yi - yuold3b(l3,counter)
                  zr = zi - zuold3b(l3,counter)
                  call imagen (xr,yr,zr)
                  r2 = xr*xr + yr*yr + zr*zr
                  if (r2 .le. ubuf2) then
!$OMP CRITICAL
                     do j = 1, nulst3b(l3,counter)
                        if (ulst3b(j,l3,counter) .eq. l1)  goto 20
                     end do
                     nulst3b(l3,counter) = nulst3b(l3,counter) + 1
                     ulst3b(nulst3b(l3,counter),l3,counter) = l1
   20                continue
!$OMP END CRITICAL
                  else if (r2 .le. ubufx) then
!$OMP CRITICAL
                     do j = 1, nulst3b(l3,counter)
                        if (ulst3b(j,l3,counter) .eq. l1) then
                           ulst3b(j,l3,counter) = 
     &                       ulst3b(nulst3b(l3,counter),l3,counter)
                           nulst3b(l3,counter) = nulst3b(l3,counter) - 1
                           goto 30
                        end if
                     end do
...
!$OMP DO schedule(guided)
      do i = 1, npole3b
         if (nulst3b(i,counter) .ge. maxulst2) then
            write (iout,40)
...
c
c
      subroutine ubuild_3bclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use iounit
...
c
c
      subroutine ulight_3bclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use cell
...
c
      do l1 = 1, npole3b
         ...
         nulst3b(l1,counter) = 0
         ii = ipole(i)
         xuold3b(l1,counter) = x(ii)
         yuold3b(l1,counter) = y(ii)
         zuold3b(l1,counter) = z(ii)
         xsort(l1) = x(ii)
         ysort(l1) = y(ii)
         zsort(l1) = z(ii)
      end do
...
c
      off = sqrt(ubuf2)
      call lightn (off,npole3b,xsort,ysort,zsort)
c
c     perform deallocation of some local arrays
c
      deallocate (xsort)
...
c      do i = 1, npole
   ...
   10    continue
         do j = start, stop
            ...
            if (r2 .le. ubuf2) then
               nulst3b(l1,counter) = nulst3b(l1,counter) + 1
               ulst3b(nulst3b(l1,counter),l1,counter) = k
            end if
   20       continue
         end do
         if (repeat) then
...
c     check to see if the neighbor list is too long
c
         if (nulst3b(l1,counter) .ge. maxulst2) then
            write (iout,30) nulst3b(l1,counter)
   30       format (/,' ULIGHT2B  --  Too many Neighbors;',
     &                 ' Increase MAXULST',i6)
            call fatal
         end if
      end do
