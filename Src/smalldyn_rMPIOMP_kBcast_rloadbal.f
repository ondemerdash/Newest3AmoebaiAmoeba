...
c      start=taskid*offset+1
      ...
      offset_emreal=int(n/numtasks_emreal)
      remainder_emreal=mod(n,numtasks_emreal)

      if(taskid.le.remainder_emreal-1) then
        atomind=numtasks_emreal*offset_emreal+taskid+1
      else if(taskid.gt.remainder_emreal-1) then
        atomind=0
      end if

      if(taskid.le.numtasks_emreal-1) then
       start_emreal=taskid*offset_emreal+1
       print*,"Taskid start",taskid,start_emreal
      end if
...
c            call mpi_barrier(mpi_comm_world,ierr)
     ...
     &       mpi_comm_world,ierr)
            ...
            if(listbcast) then
c             if(firstload_polz_bcast) then
c               firstload_polz_bcast = .false.
