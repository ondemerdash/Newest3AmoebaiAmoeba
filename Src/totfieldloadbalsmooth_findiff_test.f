...
c
      ...
      real*8 xr,yr,zr,r,r2,fnorm,fnorm2,sumdepx,sumdepy,sumdepz
...
c         if (.not.allocated(elst))  allocate (elst(maxelst,n))      
         if(printforcemat) then
           if(.not.allocated(depmat3b))
     &          allocate(depmat3b(3,npole,npole))
         end if
            ...
            if(taskid.eq.master) then
             ...
             !   call polar_load_balance
              ...
              if(printforcemat) then
                do i=1,npole
                   do k=1,npole
                   depmat3b(1,k,i)=0.0d0
                   depmat3b(2,k,i)=0.0d0
                   depmat3b(3,k,i)=0.0d0
                   end do
                end do
              end if
...
c               call mlist
     ...
     &       mpi_comm_world,ierr)
            ...
            !  call dfield0b_totfield
              ...
              print*,"After dfield0a_totfield"
            end if           
            call mpi_bcast(fieldnpole,3*n,mpi_real8,master,
     &       mpi_comm_world,ierr)
            call mpi_bcast(fieldpnpole,3*n,mpi_real8,master,
     &       mpi_comm_world,ierr)
            if(listbcast) then
...
c            if(taskid.eq.master) then
            call totfieldsmallsmooth2_gradient_polar_load_balanced(
                ...
                if(printforcemat) then
                  do i=1,npole
                     do k=1,npole
                      ...
                      r = sqrt(r2)
             fnorm2=(depmat3b(1,k,i)*depmat3b(1,k,i) +
     &        depmat3b(2,k,i)*depmat3b(2,k,i)
     &        + depmat3b(3,k,i)*depmat3b(3,k,i))

              fnorm=sqrt(fnorm2)

             !print*,"i=",i,"k=",k,"rij=",r,"depmat3b_x=",depmat3b(1,k,i)
             !print*,"i=",i,"k=",k,"rij=",r,"depmat3b_y=",depmat3b(2,k,i)
             !print*,"i=",i,"k=",k,"rij=",r,"depmat3b_z=",depmat3b(3,k,i)

             print*,"i=",i,"k=",k,"rij=",r,"depmat3bnorm=",fnorm
                     end do
                  end do
                  do i = 1,npole
                     sumdepx=0.0d0
                     ...
                     sumdepz=0.0d0
                     do k=1,npole
                        sumdepx=sumdepx+depmat3b(1,k,i)
                        sumdepy=sumdepy+depmat3b(2,k,i)
                        sumdepz=sumdepz+depmat3b(3,k,i)
                     end do
                     print*,"sumdep_x=",i,sumdepx
                     print*,"sumdep_y=",i,sumdepy
                     print*,"sumdep_z=",i,sumdepz
                  end do
                end if
                 deallocate (derivs)
                 if (nuse .eq. n)  call mdrest (0)
...
c     &       mpi_comm_world,ierr)
            ...
            if(taskid.eq.master) then
                 ...
                 do i=1,3
                   do j=1,3
                    vir(i,j)=vir(i,j)+viremreal_tmp(i,j)
     &                       + viremrecip(i,j)
                   end do
                 end do
...
c              analyticgz(istep)=derivs(3,6)
              ...
              deallocate (derivs)
cccc              call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
c                print*,"Finite Diff TotEng, TotGradz",
c     & (efindif(3)-efindif(1))/2.0d0/delx,analyticgy(2)
                print*,"Finite Diff PolEng, PolGradz",
     & (efindif_ep3b(3)-efindif_ep3b(1))/2.0d0/delx,analyticg_dep3by(2)
              end if
