...
c
c
c      subroutine emrecip1_1and2_3b_Polar_totfield_parl(npole3b,pnum,
c     & uind,uinp,eptemp,deptemp,virtemp,qgrid3b,qfac3b,thetai1_3b,
c     & thetai2_3b,thetai3_3b,pmetable3b,igrid3b,planf3b,planb3b,
c     &   iprime3b,ffttable3b)
      subroutine emrecip1_1and2_3b_Polar_totfield_parl_513(npole3b,pnum,
...
c         print*,"After first call to grid_mpole"
     ...
     &  !   thetai2_3b,thetai3_3b,pmetable3b,igrid3b)
         call grid_mpole3b_totfield_513(fmp,qgrid3b)
...
c
     ...
     &                  + qgrid3b(2,k1,k2,k3)*qgrip(2,k1,k2,k3)
        ...
        ! qfac3b(k1,k2,k3) = expterm
         !print*,"qfac3b(k1,k2,k3)=",qfac3b(k1,k2,k3)
      end do
c
...
c     assign just the Permanent multipoles to PME grid
c     and perform the 3-D FFT forward transformation
c
...
c
c      qfac3b(1,1,1) = 0.0d0
c      if (.not. use_bounds) then
c         expterm = 0.5d0 * pi / xbox
c         struc2 = qgrid3b(1,1,1,1)**2 + qgrid3b(2,1,1,1)**2
c         e = 0.5d0 * expterm * struc2
c         qfac3b(1,1,1) = expterm
c      end if
c
...
c
      ...
      !call fphi_mpole3b_totfield (fphi,qgrid3b,igrid3b,
...
c
         call grid_uind3b_totfield_513 (npole3b,pnum,fuind,fuinp,
     &         qgrid3b)
         call fftfront3b(planf3b,qgrid3b,iprime3b,ffttable3b)
c
c     account for the zeroth grid point for a finite system
...
c
         call fftback3b(planb3b,qgrid3b,iprime3b,ffttable3b)
       ! call fphi_uind3b_totfield (npole3b,pnum,fphid,fphip,fphidp,
     & ! qgrid3b,igrid3b,thetai1_3b,thetai2_3b,thetai3_3b)
        call fphi_uind3b_totfield_513 (npole3b,pnum,fphid,fphip,fphidp,
     & qgrid3b)
...
c     &                 + fuinp(k,i)*fphid(j3,i)
     ...
     &          !       + fuinp(k,l1)*fphid(j3,l1)
               if(doi) then
               e = e + fuind(k,l1)*fphi_totfield(k+1,i)
               f1 = f1 + (fuind(k,l1)+fuinp(k,l1))*fphi_totfield(j1,i)
     &                 + fuind(k,l1)*fphip(j1,i)
     &                 + fuinp(k,l1)*fphid(j1,i)
               f2 = f2 + (fuind(k,l1)+fuinp(k,l1))*fphi_totfield(j2,i)
     &                 + fuind(k,l1)*fphip(j2,i)
     &                 + fuinp(k,l1)*fphid(j2,i)
               f3 = f3 + (fuind(k,l1)+fuinp(k,l1))*fphi_totfield(j3,i)
     &                 + fuind(k,l1)*fphip(j3,i)
     &                 + fuinp(k,l1)*fphid(j3,i)
               end if

               if ((poltyp .eq. 'DIRECT').and.(doi)) then
...
c     &                    - fuinp(k,i)*fphid(j3,i)
                  f1 = f1 - fuind(k,l1)*fphip(j1,i)
     &                    - fuinp(k,l1)*fphid(j1,i)
                  f2 = f2 - fuind(k,l1)*fphip(j2,i)
     &                    - fuinp(k,l1)*fphid(j2,i)
                  f3 = f3 - fuind(k,l1)*fphip(j3,i)
     &                    - fuinp(k,l1)*fphid(j3,i)
               end if
            end do
            do k = 1, 10
...
c            dep(3,ii) = dep(3,ii) + frc(3,i)
            deptemp(1,ii) = deptemp(1,ii) + frc(1,i)
            deptemp(2,ii) = deptemp(2,ii) + frc(2,i)
            deptemp(3,ii) = deptemp(3,ii) + frc(3,i)
...
c
         ...
         !call fphi_to_cphi3b (npole3b,pnum,fphidp,cphi3b)
...
c
     ...
     &   !                 - cmp(8,l1)*cphi(6,l1) - cmp(9,l1)*cphi(10,l1)
            ...
            !trq(1,l1) = cmp(4,i)*cphi(3,i) - cmp(3,i)*cphi(4,i)
     &      !              + 2.0d0*(cmp(7,i)-cmp(6,i))*cphi(10,i)
     &      !            + cmp(9,i)*cphi(8,i) + cmp(10,i)*cphi(6,i)
     &      !            - cmp(8,i)*cphi(9,i) - cmp(10,i)*cphi(7,i)
            !trq(2,l1) = cmp(2,i)*cphi(4,i) - cmp(4,i)*cphi(2,i)
     &      !              + 2.0d0*(cmp(5,i)-cmp(7,i))*cphi(9,i)
     &      !            + cmp(8,i)*cphi(10,i) + cmp(9,i)*cphi(7,i)
     &      !            - cmp(9,i)*cphi(5,i) - cmp(10,i)*cphi(8,i)
            !trq(3,l1) = cmp(3,i)*cphi(2,i) - cmp(2,i)*cphi(3,i)
     &      !              + 2.0d0*(cmp(6,i)-cmp(5,i))*cphi(8,i)
     &      !            + cmp(8,i)*cphi(5,i) + cmp(10,i)*cphi(9,i)
     &      !            - cmp(8,i)*cphi(6,i) - cmp(9,i)*cphi(10,i)
