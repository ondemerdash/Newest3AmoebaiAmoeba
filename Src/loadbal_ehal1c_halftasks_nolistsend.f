...
c
c
      subroutine loadbal_ehal1c_half_nolistsend(evtmp,devtmp,virevtmp)
      use sizes
      use atomid
      use atoms
      use bound
...
c        if(numtasks_vdw.eq.int((numtasks-2)/2)) then
...
c      print*,"numtasks_vdw=",int((numtasks-2)/2)
c      print*,"loadbalehal1c taskid_offset=",taskid_offset
c      print*,"taskid,taskoffset",taskid,taskid_offset,
c     &  "start_vdw2(taskid_offset)", start_vdw2(taskid_offset)
c      print*,"taskid,taskoffset",taskid,taskid_offset,
c     &  "last_vdw2(taskid_offset)",last_vdw2(taskid_offset)
...
c
         do kk = 1, nvlst(ii)
            k = ivdw(vlst(kk,ii))
c         do kk = 1, nvlst_recv(iter3)
c            k = ivdw(vlst_recv(kk,iter3))
            kv = ired(k)
            mutk = mut(k)
            proceed = .true.
