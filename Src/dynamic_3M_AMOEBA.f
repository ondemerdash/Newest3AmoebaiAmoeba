...
c     expansion (3M-AMOEBA) in an MPI/OpenMP code. In this model, a cluster of molecules, 
c     supplied as an input argument on the command line, constitues the 'body' in the MBE. 
c     It has been implemented for the 'VERLET' and 'NOSE-HOOVER' integration methods. 
...
C of Velocity Verlet integration and update temperature and pressure
                 call empole1d_3b_Perm_selfeng_bcast
                 if (use_vcorr) then
                  call evcorr1 (elrc,vlrc)
                  ev = ev + elrc
