      subroutine grad_covemrecip_vandr_reduce_totfield_commsmall
      use mpole
      use mpidat
      use mpidat2
      use deriv
...
c        if(embedtyp.ne.'O') then
c        call emrecip1_3b_Perm2_totfield
c        else
        !  t5=mpi_Wtime()
...
c        end if
     ...
     &      reqs3,ierr)
      !  t6=mpi_Wtime()
c      print*,"Isend PermElec PME taskid",taskid,em
...
c     &   emrealt,viremrealt,demrealt,fieldnpolet,fieldpnpolet)
     ...
     & emrealt,viremrealt,demrealt)
      !t2=mpi_Wtime()
c      print*,"Computation PermElec Real taskid",taskid,emrealt
     ...
     &        evtmp,devtmp,virevtmp)
      !t4=mpi_Wtime()
c      print*,"Computation VdW taskid",taskid,evtmp
      end if
