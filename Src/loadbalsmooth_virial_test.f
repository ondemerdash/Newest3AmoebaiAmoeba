...
c         if (.not. allocated(mindex))  allocate (mindex(npole))
c         if (.not. allocated(minv))  allocate (minv(3*maxulst*npole))
...
c              call allocPermElec1 
     ...
     &       mpi_comm_world,ierr)
            ...
            if(listbcast) then
c            print*,"Before ewbcast"
c            call ewbcast_polar_load_balance
c            print*,"After ewbcast"
            call mpi_bcast(nmollst,nmol,mpi_integer,master,
     &       mpi_comm_world,ierr)
            call mpi_bcast(mollst,max2blst*nmol,mpi_integer,master,
     &       mpi_comm_world,ierr)
...
c            print*,"After all 2body list bcast"
            ...
            listbcast = .false.
c            print*,"Before bspline_fill"
            !call bspline_fill
c            print*,"After bspline_fill"
c            print*,"Before table_fill"
            !call table_fill
           ! print*,"After table_fill"
...
c            switchmode = 'MPOLE'
c            call switch (switchmode)
            switchmode = 'MPOLE'
            call switch (switchmode)


            !call ewNOsmallsmooth2_gradient_polar_load_balanced 
           call print_smooth2_gradient_polar(start,
...
c            listsend_mpole=.false.
     ...
     &       !            + dev(j,i) + dem(j,i)+dep3b(j,i)
             !     end do
                  print*,"dep3b_x",dep3b(1,i)
                  print*,"dep3b_y",dep3b(2,i)
                  print*,"dep3b_z",dep3b(3,i)
                 if (use(i) .and. mass(i).ne.0.0d0) then
                   speed = maxwell (mass(i),kelvin)
                   call ranvec (vec)
                   do j = 1, 3
     ...
     &       !            + dev(j,i) + dem(j,i)+dep3b(j,i)
       ...
       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b
              if(istep.eq.2) then 
                do i=1,3
                   do j=1,3
                     vir_analytic(j,i)=vir(j,i)
                     virep3b_analytic(j,i)=virep3b(j,i)
                     print*,"Step 2 virep3b",j,i,virep3b(j,i)
                   end do 
                end do 
