      subroutine grad_covemrecip_vandr_reduce_totfield_dEtensor_vir8 
      use mpole
      use mpidat
      use mpidat2
      use deriv
...
c         maxlocal_allthread=0
         largest_nelst=0
           do i=1,npole
c              maxlocal_allthread=maxlocal_allthread+nelst2(i)
              if(nelst2(i).gt.largest_nelst) then
                largest_nelst=nelst2(i)
              end if
           end do
c      maxlocal_allthread_tau=3*maxlocal_allthread

      !print*,"taskid=",taskid,"largest_nelst=",largest_nelst
c      print*,"taskid=",taskid,"maxlocal_allthread=",maxlocal_allthread
      
c      allocate (elstgrad(largest_nelst,npole))
      allocate (elsttau1_index(4,largest_nelst,npole))
c      allocate (elsttau1(4,largest_nelst,npole))
      allocate (elsttau2_index(4,largest_nelst,npole))
c      allocate (elsttau2(4,largest_nelst,npole))
...
c      allocate (frcxtau2ptot_3(9,largest_nelst,npole))
      ...
      call thg5_small_vir_nolistsend_Nn1_noewtens_notenssends2
