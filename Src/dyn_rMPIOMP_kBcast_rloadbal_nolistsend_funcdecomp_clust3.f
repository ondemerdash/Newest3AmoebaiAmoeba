...
c            listsend_vdw = .false.
            ...
            call grad_covemrecip_vandr_reduce_totfield 
             ...
             call clustsmooth2_gradient_polar_load_balanced1b2b(start,
...
c            !if(firstload_polz.and.(approxmode.ne.'1BODYMODE')) then
c            if(listbcast.and.(approxmode.ne.'1BODYMODE')
c     &                .and.do2waterclustlist) then
c             firstload_polz = .false.
...
C  COMMENTED OUT BELOW FOR TIMING PURPOSES ONLY          
            !if((taskid.lt.numtasks_polar1).and.mlistclust) then
            !  call clust1blists(start,start+offset-1, moli1rmndr) 
            !end if
...
c     & (efindif(3)-efindif(1))/2.0d0/delx,analyticgx(2)
c                print*,"Finite Diff PolEng, PolGradz",
c     & (efindif_ep3b(3)-efindif_ep3b(1))/2.0d0/delx,analyticg_dep3bx(2)
c              end if
c
