...
c
c
      subroutine torque (deptemp,i,trq1,trq2,frcx,frcy,frcz)
      use sizes
      use atoms
      use deriv
      use mpole
...
c
      if (axetyp .eq. 'Z-Only') then
         do j = 1, 3
            du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
c            dep(j,ia) = dep(j,ia) + du
c            dep(j,ib) = dep(j,ib) - du
...
c
         ...
         do j = 1, 3
            ...
            dv = -uv(j)*dphidu/(vsiz*uvsin)
c            dep(j,ia) = dep(j,ia) + du
c            dep(j,ic) = dep(j,ic) + dv
c            dep(j,ib) = dep(j,ib) - du - dv
            deptemp(j,ia) = deptemp(j,ia) + du
...
c            dep(j,ia) = dep(j,ia) + du
c            dep(j,ic) = dep(j,ic) + dv
c            dep(j,id) = dep(j,id) + dw
c            dep(j,ib) = dep(j,ib) - du - dv - dw
...
c
c
      subroutine torque2_3b (npole3b,pnum,trq,derivs)
      use sizes
      use atoms
      use mpole
      implicit none
      ...
      integer ia_l1,ib_l1,ic_l1,id_l1,l2
c
c
c     get the local frame type and the frame-defining atoms
c
...
c     result of infinitesimal rotation along these vectors
c
c         dphidu = -trq(1,i)*u(1) - trq(2,i)*u(2) - trq(3,i)*u(3)
c         dphidv = -trq(1,i)*v(1) - trq(2,i)*v(2) - trq(3,i)*v(3)
c         dphidw = -trq(1,i)*w(1) - trq(2,i)*w(2) - trq(3,i)*w(3)

         dphidu = -trq(1,l1)*u(1) - trq(2,l1)*u(2) - trq(3,l1)*u(3)
         dphidv = -trq(1,l1)*v(1) - trq(2,l1)*v(2) - trq(3,l1)*v(3)
         dphidw = -trq(1,l1)*w(1) - trq(2,l1)*w(2) - trq(3,l1)*w(3)

         if (axetyp .eq. 'Z-Bisect') then
c            dphidr = -trq(1,i)*r(1) - trq(2,i)*r(2) - trq(3,i)*r(3)
c            dphids = -trq(1,i)*s(1) - trq(2,i)*s(2) - trq(3,i)*s(3)

            dphidr = -trq(1,l1)*r(1) - trq(2,l1)*r(2) - trq(3,l1)*r(3)
            dphids = -trq(1,l1)*s(1) - trq(2,l1)*s(2) - trq(3,l1)*s(3)
...
c
         if (axetyp .eq. 'Z-Only') then
            do j = 1, 3
               du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
c               derivs(j,ia) = derivs(j,ia) + du
c               derivs(j,ib) = derivs(j,ib) - du
               derivs(j,ia_l1) = derivs(j,ia_l1) + du
               derivs(j,ib_l1) = derivs(j,ib_l1) - du
            end do
c
c     force distribution for the Z-then-X local coordinate method
c
            ...
            do j = 1, 3
               ...
               dv = -uv(j)*dphidu/(vsiz*uvsin)
c               derivs(j,ia) = derivs(j,ia) + du
c               derivs(j,ic) = derivs(j,ic) + dv
c               derivs(j,ib) = derivs(j,ib) - du - dv

               derivs(j,ia_l1) = derivs(j,ia_l1) + du
               derivs(j,ic_l1) = derivs(j,ic_l1) + dv
               derivs(j,ib_l1) = derivs(j,ib_l1) - du - dv
            end do
c
c     force distribution for the Bisector local coordinate method
c
...
c               derivs(j,ia) = derivs(j,ia) + du
c               derivs(j,ic) = derivs(j,ic) + dv
c               derivs(j,id) = derivs(j,id) + dw
c               derivs(j,ib) = derivs(j,ib) - du - dv - dw
               derivs(j,ia_l1) = derivs(j,ia_l1) + du
               derivs(j,ic_l1) = derivs(j,ic_l1) + dv
               derivs(j,id_l1) = derivs(j,id_l1) + dw
               derivs(j,ib_l1) = derivs(j,ib_l1) - du - dv - dw
            end do
c
c     force distribution for the 3-Fold local coordinate method
c        (correct for uv, uw and vw angles all equal to 90)
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine torque2_3bnew  --  convert all site torques to forces  ##
c     ##                                                              ##
c     ##################################################################
c
c
...
c
c
      subroutine torque2_3bnew (npole3b,pnum,trq,derivs)
      use sizes
      use atoms
      use mpole
      implicit none
