...
c               eksum = eksum * scale * scale   OLD
               eksum = eksum * exp(-vnh(1)*dt2)*exp(-vnh(1)*dt2)
               gnh(1) = (2.0d0*eksum-dble(nfree)*ekt) / qnh(1)
               expterm = exp(-vnh(2)*dt8)
               vnh(1) = expterm * (vnh(1)*expterm+gnh(1)*dt4)
               gnh(2) = (qnh(1)*vnh(1)*vnh(1)-ekt) / qnh(2)
...
c      print*,"In temper2 dt",dt
c      print*,"In temper2 nfree",nfree
c      print*,"In temper2 kelvin",kelvin
c      print*,"In temper2 tautemp",tautemp
c      print*,"In temper2 gasconst",gasconst 
c      print*,"In temper2 isothermal=",isothermal
c      print*,"In temper2 thermostat",thermostat
c
c
c     get instantaneous temperature from the kinetic energy
c
      call kinetic (eksum,ekin)
c      print*,"In temper2 eksum after 1st kinetic",eksum
      temp = 2.0d0 * eksum / (dble(nfree) * gasconst)
c       print*,"In temper2 temp after 1st kinetic",temp
...
c
         ...
         eta = eta * scale
c         print*,"In temper2 eta=",eta
c         print*,"In temper2 scale=",scale
c         print*,"In temper2 Bussi after 2nd kinetic temp=",temp    
         if (integrate .eq. 'RIGIDBODY') then
            do i = 1, ngrp
               do j = 1, 3
                  vcm(j,i) = scale * vcm(j,i)
...
c
      call kinetic (eksum,ekin)
      temp = 2.0d0 * eksum / (dble(nfree) * gasconst)
c      print*,"In temper2 eksum after last kinetic",eksum
c      print*,"In temper2 temp after last kinetic",temp
      return
      end
