...
c
      ...
      subroutine torque_3b_new_fij(npole3b,pnum,deptemp,
     &    i,trq1,trq2,frcx,frcy,frcz,deptempmat,k)
      use sizes
      use atoms
      use mpole
      implicit none
...
c            deptempmat(j,l2,ia_l1) = deptempmat(j,l2,ia_l1) + du
c            deptempmat(j,l2,ic_l1) = deptempmat(j,l2,ic_l1) + dv
c            deptempmat(j,l2,ib_l1) = deptempmat(j,l2,ib_l1) - du - dv

            deptempmat(j,k,ia) = deptempmat(j,k,ia) + du
            deptempmat(j,k,ic) = deptempmat(j,k,ic) + dv
            deptempmat(j,k,ib) = deptempmat(j,k,ib) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
