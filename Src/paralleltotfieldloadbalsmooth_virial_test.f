...
c
c
      program paralleltotfieldloadbalsmooth_virial_test
      use sizes
      use atoms
      use bath
      use bndstr
...
C BROADCAST SOME LOGICALS.
     ...
     & mpi_comm_world,ierr)
      call mpi_bcast(firstload_polz,1,mpi_logical,master,
     & mpi_comm_world,ierr)
      call initial
      call getxyz
...
c     &       mpi_comm_world,ierr)
            ...
            if(taskid.ge.numtasks_emreal+2) then
...
c             print*,"In verlet/nose/beeman setup,after vdw list shit"
            else if((taskid.gt.1).and.(taskid.lt.numtasks_emreal+2))then
               call mlist_par
...
c              end if
            ...
            if(approxmode.ne.'1BODYMODE') then
              ...
              if(firstload_polz.and.(taskid.eq.master)) then
...
c            call mpi_barrier(mpi_comm_world,ierr)
     ...
     &         !            +virep3b_recip(j,i)
                ...
                energy = eb + ea + eba + eub + eopb + et + ept  + ett
     &                   + ev + em+ep3b+ep3bmut!+ep3b_recip
c      call system_clock(t1,clock_rate)              
             allocate (derivs(3,n))
              efindif_ep3b(istep)=ep3b+ep3bmut!+ep3b_recip
...
c!$OMP END PARALLEL DO
              analyticgx(istep)=derivs(1,6)-dep3b(1,6)
     &              -dep3bmut(1,6)-dep3b1(1,6)
              analyticg_dep3bx(istep)=dep3b(1,6)+dep3bmut(1,6)
     &                   +dep3b1(1,6)

              analyticgy(istep)=derivs(2,6)-dep3b(2,6)-dep3bmut(2,6)
     &                   -dep3b1(2,6)
              analyticg_dep3by(istep)=dep3b(2,6)+dep3bmut(2,6)
     &                   +dep3b1(2,6)

              analyticgz(istep)=derivs(3,6)-dep3bmut(3,6)
     &                  -dep3b(3,6)-dep3b1(3,6)
...
c               call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
c                print*,"Finite Diff PairwiseEng, PairwiseGradz",
