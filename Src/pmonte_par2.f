...
c
c
      subroutine pmonte_par2 (epot,temp,istep)
      use sizes
      use atomid
      use atoms
      use bath
...
c
      ...
      if(taskid.eq.master) then
        print*,"2nd version parallelized pmonte for 3-amoeba"
        random_num=random()
      end if

      call mpi_bcast(random_num,1,mpi_real8,master,
     &   mpi_comm_world,ierr)
...
c
      !print*,"2nd version parallelized pmonte for 3-amoeba"
      if (random_num .lt. 1.0d0/dble(voltrial)) then
         xboxold = xbox
         yboxold = ybox
         zboxold = zbox
         alphaold = alpha
...
c
         if (.not.anisotrop .or. random_num.lt.0.5d0) then
            step = volmove * (2.0d0*random_num-1.0d0)
            volbox = volbox + step
...
c
         if (isothermal) then
            kt = gasconst * kelvin
            print*,"taskid=",taskid,"kelvin=",kelvin
         else
            kt = gasconst * temp
            print*,"taskid=",taskid,"temp=",temp
         end if
         ...
         if (integrate .eq. 'RIGIDBODY') then
            ...
            print*,"taskid=",taskid,"nuse=",nuse
            print*,"taskid=",taskid,"volbox=",volbox
            print*,"taskid=",taskid,"vold=",vold
         end if
