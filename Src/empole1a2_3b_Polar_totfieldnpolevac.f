...
c      subroutine empole1a2_3b_Polar_totfieldnpole_mod(npole3b,pnum,uind,
c     &   uinp,eptemp,deptemp,virtemp)
      subroutine empole1a2_3b_Polar_totfieldnpolevac(npole3b,pnum,
     &   npole3b1,pnum1,pnum1ref,uind,uinp,eptemp,deptemp,virtemp)
      use bound
      use sizes
      use atoms
      use chgpot
      ...
      integer npole3b,pnum(*),l1,l3,k1,npole3b1,pnum1(*),pnum1ref(*)
      integer k2,k3
      real*8 eptemp,deptemp(3,npole3b),virtemp(3,3)
c      real*8 eptemp,deptemp(3,npole),virtemp(3,3)
...
c               dsc3 = scale3 * dscale(k)
c               dsc5 = scale5 * dscale(k)
c               dsc7 = scale7 * dscale(k)
c               psc3 = scale3 * pscale(k)
c               psc5 = scale5 * pscale(k)
c               psc7 = scale7 * pscale(k)

               dsc3 = scale3 * dscale(k2)
               dsc5 = scale5 * dscale(k2)
               dsc7 = scale7 * dscale(k2)
               psc3 = scale3 * pscale(k2)
               psc5 = scale5 * pscale(k2)
               psc7 = scale7 * pscale(k2)
...
c     &                                (glip(7)+glip(2))*dsc5
c     &                                 )
c     &                + 0.5d0 * rr9 * (glip(3)*dsc7)
c               gfi(2) = -rr3*ck + rr5*sc(4) - rr7*sc(6)
c               gfi(3) = rr3*ci + rr5*sc(3) + rr7*sc(5)
c               gfi(4) = 2.0d0 * rr5
c               gfi(5) = rr7 * (scip(4)*dsc7)
c               gfi(6) = -rr7 * (scip(3)*dsc7)
...
c     LEFT OFF HERE!!!!
               temp3 = 0.5d0 * rr3 * ((gli(1)+gli(6))*pscale(k2)
     &                                  +(glip(1)+glip(6))*dscale(k2))
               temp5 = 0.5d0 * rr5 * ((gli(2)+gli(7))*pscale(k2)
     &                                  +(glip(2)+glip(7))*dscale(k2))
               temp7 = 0.5d0 * rr7 * (gli(3)*pscale(k2)
     &                                  +glip(3)*dscale(k2))
...
c               gti(2) = 0.5d0 * rr5 * (scip(4)*dsc5)
c               gti(3) = 0.5d0 * rr5 * (scip(3)*dsc5)
               gti(4) = gfi(4)
               gti(5) = gfi(5)
               gti(6) = gfi(6)
c
...
c
              ...
              if(doi) then
               !deptemp(1,i) = deptemp(1,i) + 0.5d0*ftm2i(1)
               !deptemp(2,i) = deptemp(2,i) + 0.5d0*ftm2i(2)
               !deptemp(3,i) = deptemp(3,i) + 0.5d0*ftm2i(3)
               !call torque_3b_new_npole(deptemp,i,
     &         ! ttm2,0.5d0*ttm2i,frcxi,frcyi,frczi)
               deptemp(1,k1) = deptemp(1,k1) + 0.5d0*ftm2i(1)
               deptemp(2,k1) = deptemp(2,k1) + 0.5d0*ftm2i(2)
               deptemp(3,k1) = deptemp(3,k1) + 0.5d0*ftm2i(3)
               call torque_3b_new(npole3b,pnum,deptemp,i,
     &          ttm2,0.5d0*ttm2i,frcxi,frcyi,frczi)

             !  deptemp(1,k) = deptemp(1,k) - 0.5d0*ftm2i(1)
             !  deptemp(2,k) = deptemp(2,k) - 0.5d0*ftm2i(2)
             !  deptemp(3,k) = deptemp(3,k) - 0.5d0*ftm2i(3)
              ! call torque_3b_new_npole(deptemp,k,
     &        !    ttm3,0.5d0*ttm3i,frcxk,frcyk,frczk)
               deptemp(1,k2) = deptemp(1,k2) - 0.5d0*ftm2i(1)
               deptemp(2,k2) = deptemp(2,k2) - 0.5d0*ftm2i(2)
               deptemp(3,k2) = deptemp(3,k2) - 0.5d0*ftm2i(3)
     ...
     &        !  ttm2,ttm2i,frcxi,frcyi,frczi)
               deptemp(1,k1) = deptemp(1,k1) + ftm2i(1)
               deptemp(2,k1) = deptemp(2,k1) + ftm2i(2)
               deptemp(3,k1) = deptemp(3,k1) + ftm2i(3)
               call torque_3b_new(npole3b,pnum,deptemp,i,
     &          ttm2,ttm2i,frcxi,frcyi,frczi)
     ...
     &       !     ttm3,ttm3i,frcxk,frcyk,frczk)            
               deptemp(1,k2) = deptemp(1,k2) - ftm2i(1)
               deptemp(2,k2) = deptemp(2,k2) - ftm2i(2)
               deptemp(3,k2) = deptemp(3,k2) - ftm2i(3)
               call torque_3b_new(npole3b,pnum,deptemp,k,
     &            ttm3,ttm3i,frcxk,frcyk,frczk)
              end if
...
c     account for partially excluded induced interactions
c     LEFT OFF HERE!!!!
               temp3 = 0.5d0 * rr3 * ((gli(1)+gli(6))*pscale(k)
     &                                  +(glip(1)+glip(6))*dscale(k))
               temp5 = 0.5d0 * rr5 * ((gli(2)+gli(7))*pscale(k)
     &                                  +(glip(2)+glip(7))*dscale(k))
               temp7 = 0.5d0 * rr7 * (gli(3)*pscale(k)
     &                                  +glip(3)*dscale(k))
...
c
              if(doi) then
               deptemp(1,i) = deptemp(1,i) + 0.5*ftm2i(1)
               deptemp(2,i) = deptemp(2,i) + 0.5*ftm2i(2)
               deptemp(3,i) = deptemp(3,i) + 0.5*ftm2i(3)
               call torque_3b_new_npole(deptemp,i,
     &          ttm2,0.5*ttm2i,frcxi,frcyi,frczi)

               deptemp(1,k) = deptemp(1,k) - 0.5*ftm2i(1)
               deptemp(2,k) = deptemp(2,k) - 0.5*ftm2i(2)
               deptemp(3,k) = deptemp(3,k) - 0.5*ftm2i(3)
               call torque_3b_new_npole(deptemp,k,
