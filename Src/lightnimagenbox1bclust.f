...
c
c
      subroutine lightnbox1bclust (cutoff,nsite,xsort,ysort,zsort,cnt)
      use sizes
      use bound
      use boxes
      use cell
...
c
      ...
      if (use_bounds) then
         ...
         xcut = min(xcut,xcell2_1b(cnt))
         ycut = min(ycut,ycell2_1b(cnt))
         zcut = min(zcut,zcell2_1b(cnt))
      end if
c
...
c
      if (use_bounds) then
         do i = 1, nsite
            ...
            ysort(i) = yfrac(i)
            zsort(i) = zfrac(i)
            do while (abs(xsort(i)) .gt. xcell2_1b(cnt))
               xsort(i) = xsort(i) - sign(xcell1b(cnt),xsort(i))
            end do
            do while (abs(ysort(i)) .gt. ycell2_1b(cnt))
               ysort(i) = ysort(i) - sign(ycell1b(cnt),ysort(i))
            end do
            do while (abs(zsort(i)) .gt. zcell2_1b(cnt))
               zsort(i) = zsort(i) - sign(zcell1b(cnt),zsort(i))
            end do
         end do
      end if
c
...
c
c
      subroutine imagen1b (xr,yr,zr,cnt)
      use boxes
      use boxes1bclust
      implicit none
      real*8 xr,yr,zr
...
c
      if (orthogonal) then
         ...
         yr = abs(yr)
         zr = abs(zr)
         if (xr .gt. xbox2_1b(cnt))  xr = xr - xbox1b(cnt)
         if (yr .gt. ybox2_1b(cnt))  yr = yr - ybox1b(cnt)
         if (zr .gt. zbox2_1b(cnt))  zr = zr - zbox1b(cnt)
