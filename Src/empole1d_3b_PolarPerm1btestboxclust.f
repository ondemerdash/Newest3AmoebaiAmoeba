      ...
      subroutine empole1d_3b_PolarPerm1btestboxclust(npole3b,pnum,
...
c       print*,"Cost of induce0a_3b_scf_vacomp1bclust",tot_time!t4-t3

c            t3=mpi_Wtime()
c            call system_clock(t3,clock_rate)
      call emrecip1_3b_PolarPermtestbox1bclust(npole3b,pnum,uind,uinp,
...
c      print*,"Done w emrecip1_3b_Polar_new in 1bclust"

c            t3=mpi_Wtime()
c            call system_clock(t3,clock_rate)
      call ereal1d_3b_1bclustPolarPermtestbox1bclust(npole3b,pnum,uind,
     & uinp,cnt,eptemp,deptemp,virtemp,emtemp,demtemp)
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)
c            t4=mpi_Wtime()
c       print*,"Cost of ereal1d_3b_1bclustPolarPerm",tot_time!t4-t3
c      print*,"Done w ereal1d_3b_1bclust in 1bclust"
...
c      print*,"Completed all steps of empole1d 1bclust"
c            t4=mpi_Wtime()
c            call system_clock(t4,clock_rate)
c       tot_time =  (t4-t3)/real(clock_rate)
...
c            t2=mpi_Wtime()
      ...
      ! print*,"tsk=",taskid,"TotCost Within empole1d_3b_1btest",tot_time
      return
      end
c
c     ################################################################
...
c
c
      subroutine ereal1d_3b_1bclustPolarPermtestbox1bclust(npole3b,pnum,
...
c
      f = electric / dielec
      mode = 'EWALD'
      call switchbox1bclust (mode,cnt)
c
...
c
     ...
     &            pscale(i14(j,ii)) = p4scale * p41scale
         ...
         !do kkk = 1, nelst(i)
         !   k = elst(kkk,i)
         do kkk = 1,nelst1b(l1,cnt)
            l3=elst1b(kkk,l1,cnt)
            k=pnum(l3)
            kk = ipole(k)
            xr = x(kk) - x(ii)
            yr = y(kk) - y(ii)
            zr = z(kk) - z(ii)
            call image1b (xr,yr,zr,cnt)
...
c
c
      subroutine emrecip1_3b_PolarPermtestbox1bclust(npole3b,pnum,uind,
...
c
     ...
     &             + recip1b(3,3,cnt)*r3
            ...
            if (term .gt. -50.0d0) then
               ...
               expterm = exp(term) / denom
               if (.not. use_bounds) then
                  expterm = expterm * (1.0d0-cos(pi*xbox1b(cnt)
     &             *sqrt(hsq)))
               else if (octahedron) then
                  if (mod(m1+m2+m3,2) .ne. 0)  expterm = 0.0d0
               end if
...
c
      ...
      do l1 = 1, npole3b
         ...
         f2 = dble(nfft2) * f2
         f3 = dble(nfft3) * f3
         frc(1,i) = recip1b(1,1,cnt)*f1 + recip1b(1,2,cnt)*f2 
     &      + recip1b(1,3,cnt)*f3
         frc(2,i) = recip1b(2,1,cnt)*f1 + recip1b(2,2,cnt)*f2 
     &      + recip1b(2,3,cnt)*f3
         frc(3,i) = recip1b(3,1,cnt)*f1 + recip1b(3,2,cnt)*f2 
     &      + recip1b(3,3,cnt)*f3
      end do
      e = 0.5d0 * e
      emtemp = emtemp + e
...
c
         ...
         call fphi_to_cphi3bclust1b (npole3b,pnum,fphidp,cphi,cnt)
c
