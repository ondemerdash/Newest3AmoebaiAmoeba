...
c
c
      subroutine induce0a_3b_scf_vacomp(npole3b,pnum,uind,uinp)
      use sizes
      use atoms
      use inform
      use iounit
...
c      real*8 ffttable3b(maxtable,3)
     ...
     &         allocate (tindex(2,npole3b*maxelst))
            if (.not.allocated(tdipdip))
     &         allocate (tdipdip(6,npole3b*maxelst))
         end if
...
c
         call dfield0a_3b_vacomp (npole3b,pnum,field,fieldp)
...
c
           call ufield0a_3b_vacomp (npole3b,pnum,field,fieldp,uind,uinp)
c
...
c            call uscale0b (mode,rsd,rsdp,zrsd,zrsdp)
c         else
          call uscale0a_3b_vacomp(npole3b,pnum,mode,rsd,rsdp,zrsd,zrsdp,
     &              minv,mindex)
            mode = 'APPLY'
          call uscale0a_3b_vacomp(npole3b,pnum,mode,rsd,rsdp,zrsd,zrsdp,
     &              minv,mindex)
...
c
c
      subroutine dfield0a_3b_vacomp (npole3b,pnum,field,fieldp)
      use sizes
      use atoms
      use bound
      use cell
...
!$OMP& elst,cut2,poltyp,ntpair,tindex,tdipdip,
!$OMP& toffset,toffset0,field,fieldp,fieldt,fieldtp,maxlocal,
!$OMP& npole3b,pnum,off2)
...
c
c
      subroutine ufield0a_3b_vacomp(npole3b,pnum,field,fieldp,uind,uinp)
      use sizes
      use atoms
      use bound
      use cell
...
c
c
      subroutine uscale0a_3b_vacomp (npole3b,pnum,mode,rsd,rsdp,zrsd,
     &  zrsdp)
      use sizes
      use atoms
      use limits
...
!$OMP& npole3b,pnum,x,y,z,ipole,off2)
...
!$OMP& np12,ip12,np13,ip13,np14,ip14,nulst,ulst,mindex,minv,npole3b,
!$OMP& off2,pnum)
...
c
c
      subroutine uscale0a_3b_vacomptest(npole3b,pnum,mode,rsd,rsdp,zrsd,
     &  zrsdp)
      use sizes
      use atoms
      use limits
