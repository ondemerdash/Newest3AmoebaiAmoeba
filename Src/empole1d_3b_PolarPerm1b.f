      ...
      subroutine empole1d_3b_PolarPerm1b(npole3b,pnum,
...
c            t3=mpi_Wtime()
            call system_clock(t3,clock_rate)

       call induce0a_3b_scf_vacomp1bclust(npole3b,pnum,uind,uinp,cnt)
c            t4=mpi_Wtime()
            call system_clock(t4,clock_rate)
       tot_time =  (t4-t3)/real(clock_rate)
      
       print*,"Cost of induce0a_3b_scf_vacomp1bclust",tot_time!t4-t3

c            t3=mpi_Wtime()
            call system_clock(t3,clock_rate)
      call emrecip1_3b_PolarPerm(npole3b,pnum,uind,uinp,
     & eptemp,deptemp,virtemp,emtemp,demtemp)
c            t4=mpi_Wtime()
            call system_clock(t4,clock_rate)
       tot_time =  (t4-t3)/real(clock_rate)
       print*,"Cost of emrecip1_3b_PolarPerm",tot_time!t4-t3
...
c            t3=mpi_Wtime()
            call system_clock(t3,clock_rate)
      call ereal1d_3b_1bclustPolarPerm(npole3b,pnum,uind,uinp,cnt,
     & eptemp,deptemp,virtemp,emtemp,demtemp)
            call system_clock(t4,clock_rate)
       tot_time =  (t4-t3)/real(clock_rate)
c            t4=mpi_Wtime()
       print*,"Cost of ereal1d_3b_1bclustPolarPerm",tot_time!t4-t3
...
c         end if
         call torque3_PolarPerm (npole3b,pnum,
...
c            t4=mpi_Wtime()
       ...
       print*,"Cost of SelfEng SelfTorque",tot_time

c            t2=mpi_Wtime()
            call system_clock(t2,clock_rate)
       tot_time =  (t2-t1)/real(clock_rate)
      
       print*,"TotalCost Within empole1d_3b_PolarPerm1b",tot_time
      return
      end
c
c     ################################################################
...
c
c
      subroutine ereal1d_3b_1bclustPolarPerm(npole3b,pnum,uind,uinp,cnt,
     & eptemp,deptemp,virtemp,emtemp,demtemp)
      use sizes
      use atoms
      use bound
...
c
      allocate (mscale(npole3b))
      allocate (pscale(npole3b))
      allocate (dscale(npole3b))
      allocate (uscale(npole3b))
      allocate (demo1(3,npole3b))
      allocate (demo2(3,npole3b))
...
c
               dorl = .false.
               dorli = .false.
               if (mscale(l3) .ne. 1.0d0)  dorl = .true.
...
c
               do j = 1, 3
                  ftm2(j) = f * (ftm2(j)-(1.0d0-mscale(l3))*ftm2r(j))
                  ftm2i(j) = f * (ftm2i(j)-ftm2ri(j))
                  ttm2(j) = f * (ttm2(j)-(1.0d0-mscale(l3))*ttm2r(j))
                  ttm2i(j) = f * (ttm2i(j)-ttm2ri(j))
                  ttm3(j) = f * (ttm3(j)-(1.0d0-mscale(l3))*ttm3r(j))
                  ttm3i(j) = f * (ttm3i(j)-ttm3ri(j))
               end do
c
c     increment gradient due to force and torque on first site
c
               demo1(1,l1) = demo1(1,l1) + ftm2(1)
               demo1(2,l1) = demo1(2,l1) + ftm2(2)
               demo1(3,l1) = demo1(3,l1) + ftm2(3)
...
c
               demo2(1,l3) = demo2(1,l3) - ftm2(1)
               demo2(2,l3) = demo2(2,l3) - ftm2(2)
               demo2(3,l3) = demo2(3,l3) - ftm2(3)
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine emrecip1_3b_PolarPerm  --  mpole Ewald recip en  ##
c     ##                                                              ##
c     ##################################################################
c
c
...
c
c
      subroutine emrecip1_3b_PolarPerm(npole3b,pnum,uind,uinp,
     & eptemp,deptemp,virtemp,emtemp,demtemp)
      use sizes
      use atoms
      use bound
