...
c     &  ntpair_dEtmp,dEindextmp,dEd1tmp,dEd2tmp,dEp1tmp,dEp2tmp)
      subroutine thg6_small_vir_nolistsend 
...
c      print*,"last=",last_emreal2(taskid)
      ...
      !maxlocal = int(dble(sz)*dble(maxsize_elst(taskid))/dble(nthread))
      maxlocal = int(dble(sz)*dble(maxelst)/dble(nthread))
      maxlocaltau=3*maxlocal

c      maxlocal = int(dble(sz)*dble(maxsize_elst(taskid)))
      !maxlocal = int(dble(sz)*dble(maxelst))
       print*,"THG6"

c      print*,"In Tens taskid=",taskid,"maxlocal=",maxlocal
c      print*,"In Tens taskid=",taskid,"sz=",sz
c      print*,"maxsize_elst(taskid)=",maxsize_elst(taskid)
c      print*,"MPIOMPTHG VERSION"
c
...
C   FIRST, TORQUE TERMS W/ EWALD SCREENING
      ...
      fact2=0.5d0*bn(1)
      fact3=2.0d0*fact1
      tau1d(1)=tau1d(1)+xr*(fact1*dixr(1)-bn(3)*rxqir(1))
     &        +fact3*(yr*qi(3)-zr*qi(2))
      tau1p(1)=tau1d(1)
      tau1d(2)=tau1d(2)+fact2*di(3)+yr*(fact1*dixr(1)-bn(3)*rxqir(1))
     &        +fact3*(qir(3)+yr*qi(6)-zr*qi(5))
      tau1p(2)=tau1d(2)
      tau1d(3)=tau1d(3)-fact2*di(2)+zr*(fact1*dixr(1)-bn(3)*rxqir(1))
     &        +fact3*(-qir(2)+yr*qi(9)-zr*qi(8))
      tau1p(3)=tau1d(3)
      tau1d(4)=tau1d(4)-fact2*di(3)+xr*(fact1*dixr(2)-bn(3)*rxqir(2))
     &        +fact3*(-qir(3)+zr*qi(1)-xr*qi(3))
      tau1p(4)=tau1d(4)
      tau1d(5)=tau1d(5)+yr*(fact1*dixr(2)-bn(3)*rxqir(2))
     &        +fact3*(zr*qi(4)-xr*qi(6))
      tau1p(5)=tau1d(5)
      tau1d(6)=tau1d(6)+fact2*di(1)+zr*(fact1*dixr(2)-bn(3)*rxqir(2))
     &        +fact3*(qir(1)+zr*qi(7)-xr*qi(9))
      tau1p(6)=tau1d(6)
      tau1d(7)=tau1d(7)+fact2*di(2)+xr*(fact1*dixr(3)-bn(3)*rxqir(3))
     &        +fact3*(qir(2)+xr*qi(2)-yr*qi(1))
      tau1p(7)=tau1d(7)
      tau1d(8)=tau1d(8)-fact2*di(1)+yr*(fact1*dixr(3)-bn(3)*rxqir(3))
     &        +fact3*(-qir(1)+xr*qi(5)-yr*qi(4))
      tau1p(8)=tau1d(8)
      tau1d(9)=tau1d(9)+zr*(fact1*dixr(3)-bn(3)*rxqir(3))
     &        +fact3*(xr*qi(8)-yr*qi(7))
      tau1p(9)=tau1d(9)

      tau2d(1)=tau2d(1)+xr*(fact1*dkxr(1)+bn(3)*rxqkr(1))
     &         -fact3*(yr*qk(3)-zr*qk(2))
      tau2p(1)=tau2d(1)
      tau2d(2)=tau2d(2)+fact2*dk(3)+yr*(fact1*dkxr(1)+bn(3)*rxqkr(1))
     &         -fact3*(qkr(3)+yr*qk(6)-zr*qk(5))
      tau2p(2)=tau2d(2)
      tau2d(3)=tau2d(3)-fact2*dk(2)+zr*(fact1*dkxr(1)-bn(3)*rxqkr(1))
     &          -fact3*(-qkr(2)+yr*qk(9)-zr*qk(8))
      tau2p(3)=tau2d(3)
      tau2d(4)=tau2d(4)-fact2*dk(3)+xr*(fact1*dkxr(2)+bn(3)*rxqkr(2))
     &          -fact3*(-qkr(3)+zr*qk(1)-xr*qk(3))
      tau2p(4)=tau2d(4)
      tau2d(5)=tau2d(5)+yr*(fact1*dkxr(2)+bn(3)*rxqkr(2))
     &          -fact3*(zr*qk(4)-xr*qk(6))
      tau2p(5)=tau2d(5)
      tau2d(6)=tau2d(6)+fact2*dk(1)+zr*(fact1*dkxr(2)+bn(3)*rxqkr(2))
     &          -fact3*(qkr(1)+zr*qk(7)-xr*qk(9))
      tau2p(6)=tau2d(6)
      tau2d(7)=tau2d(7)+fact2*dk(2)+xr*(fact1*dkxr(3)+bn(3)*rxqkr(3))
     &          -fact3*(qkr(2)+xr*qk(2)-yr*qk(1))
      tau2p(7)=tau2d(7)
      tau2d(8)=tau2d(8)-fact2*dk(1)+yr*(fact1*dkxr(3)+bn(3)*rxqkr(3))
     &           -fact3*(-qkr(1)+xr*qk(5)-yr*qk(4))
      tau2p(8)=tau2d(8)
      tau2d(9)=tau2d(9)+zr*(fact1*dkxr(3)+bn(3)*rxqkr(3))
     &           -fact3*(xr*qk(8)-yr*qk(7))
      tau2p(9)=tau2d(9)

C   NOW W/OUT SCREENING

      fact1=0.5d0*rr5*dsc5
      fact2=0.5d0*rr3*dsc3
      fact3=2.0d0*fact1
      fact4=rr7*dsc7
      tau1d(1)=tau1d(1)-(xr*(fact1*dixr(1)-fact4*rxqir(1))
     &         +fact1*2.0d0*(yr*qi(3)-zr*qi(2)))
      tau1d(2)=tau1d(2)-(fact2*di(3)+yr*(fact1*dixr(1)
     &         -fact4*rxqir(1))+fact3*(qir(3)+yr*qi(6)-zr*qi(5)))
      tau1d(3)=tau1d(3)-(-fact2*di(2)+zr*(fact1*dixr(1)
     &         -fact4*rxqir(1))+fact3*(-qir(2)+yr*qi(9)-zr*qi(8)))


      tau1d(4)=tau1d(4)-(-fact2*di(3)+xr*(fact1*dixr(2)-fact4*rxqir(2))
     &             +fact3*(-qir(3)+zr*qi(1)-xr*qi(3)))
      tau1d(5)=tau1d(5)-(fact1*dixr(2)*yr 
     &             + fact3*(zr*qi(4)-xr*qi(6)) - fact4*rxqir(2)*yr)
      tau1d(6)=tau1d(6)-(fact2*di(1) + fact1*dixr(2)*zr 
     &            +fact3*(qir(1)+zr*qi(7)-xr*qi(9)) -fact4*rxqir(2)*zr)

      tau1d(7)=tau1d(7)-(fact2*di(2) + fact1*dixr(3)*xr 
     &           +fact3*(qir(2)+xr*qi(2)-yr*qi(1)) -fact4*rxqir(3)*xr)
      tau1d(8)=tau1d(8)-(-fact2*di(1) + fact1*dixr(3)*yr 
     &         +fact3*(-qir(1)+xr*qi(5)-yr*qi(4)) -fact4*rxqir(3)*yr)
      tau1d(9)=tau1d(9)-(fact1*dixr(3)*zr 
     &         +fact3*(xr*qi(8)-yr*qi(7)) -fact4*rxqir(3)*zr)
