...
c
c
      program loadbalsmooth_virial_test 
      use sizes
      use atoms
      use bath
      use bndstr
...
C BROADCAST SOME LOGICALS.
     ...
     & mpi_comm_world,ierr)
      ...
      if(taskid.eq.master) then
        ...
        if(.not.allocated(dEd1)) allocate (dEd1(9,n*maxelst))
        if(.not.allocated(dEd2)) allocate (dEd2(9,n*maxelst))
        if(.not.allocated(dEp1)) allocate (dEp1(9,n*maxelst))
        if(.not.allocated(dEp2)) allocate (dEp2(9,n*maxelst))
        if(.not.allocated(dEindex)) allocate (dEindex(2,n*maxelst))
...
C USING ONLY 'numtasks_emreal' TASKS    
      ...
      if (approxmode.eq.'1BODYMODE') then
        ...
        allocate(ntpair_start(0:numtasks-1))
        allocate(ntpair_last(0:numtasks-1))
        allocate(ntpair_start_rmndr(0:numtasks-1))
        allocate(ntpair_last_rmndr(0:numtasks-1))
      end if
c
...
c     &           viremreal(i,j)+virev(i,j)
     ...
     &       mpi_comm_world,ierr)
            ...
            if(taskid.eq.master) then
             ...
             call gradient_emrecip_totfield
              ...
              if(approxmode.eq.'1BODYMODE') then
                call get_ntpair_dE_task_nouselist
              end if
            end if
           call mpi_barrier(mpi_comm_world,ierr)

            if(approxmode.eq.'1BODYMODE') then
            !print*,"Before uind bcast"
            call mpi_bcast(uind,3*n,mpi_real8,master2,
     &       mpi_comm_world,ierr)
            !print*,"Before uinp bcast"
            call mpi_bcast(uinp,3*n,mpi_real8,master2,
     &       mpi_comm_world,ierr)
            call mpi_bcast(ep3b,1,mpi_real8,master2,
     &       mpi_comm_world,ierr)
            else
     ...
     &       mpi_comm_world,ierr)
      ...
      call mpi_bcast(ntpair_dE,1,mpi_integer,master2,
     &              mpi_comm_world,ierr)
      call mpi_bcast(dEindex,2*ntpair_dE,mpi_integer,master2,
     &              mpi_comm_world,ierr)
      call mpi_bcast(dEd1,9*ntpair_dE,mpi_real8,master2,
     &              mpi_comm_world,ierr)
      call mpi_bcast(dEd2,9*ntpair_dE,mpi_real8,master2,
     &              mpi_comm_world,ierr)
      call mpi_bcast(dEp1,9*ntpair_dE,mpi_real8,master2,
     &              mpi_comm_world,ierr)
      call mpi_bcast(dEp2,9*ntpair_dE,mpi_real8,master2,
     &              mpi_comm_world,ierr)
            if(approxmode.eq.'1BODYMODE') then
             call mpi_bcast(ntpair_start,numtasks,mpi_integer,master2,
     &              mpi_comm_world,ierr)
             call mpi_bcast(ntpair_last,numtasks,mpi_integer,master2,
     &              mpi_comm_world,ierr)
             call mpi_bcast(ntpair_start_rmndr,remainder,mpi_integer,
     &             master2,mpi_comm_world,ierr)
           !  call mpi_bcast(ntpair_last_rmndr,remainder,mpi_integer,
     &     !        master2,mpi_comm_world,ierr)
            end if

            if(approxmode.eq.'1BODYMODE') then
c             call ewtotfieldsmallsmooth2_gradient_polar_1body_dEtensor(
c     & start,start+offset-1,moli1rmndr)
             call ewtotfieldsmallsmooth2_gradient_polar_1body_dEtensor
            else
            call ewtotfieldsmooth2_gradient_polar_load_balanced_dEtensor
            end if
           call mpi_barrier(mpi_comm_world,ierr)
...
C THEREFORE, LIST MUST BE BROADCAST. ALSO BROADCAST VARIABLES TO DO WITH LOAD BALANCING
     ...
     &             master1,emreal_comm,ierr)
            ...
            if(taskid.le.numtasks_emreal-1) then
             call gradient_emreal_reduce_load_balanced_totfield_dEtensor
            end if
     ...
     &       mpi_comm_world,ierr)
            ...
            if(approxmode.eq.'1BODYMODE') then
              if(itermode.ne.1) then
              call ewtotfieldsmallsmooth2_gradient_polar_1body_multiter(
     &          start,last,start2,last2,moli1rmndr)
              else
               call ewtotfieldsmallsmooth2_gradient_polar_1body(
