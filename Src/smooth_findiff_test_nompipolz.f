...
c
c
      program smoothdynamic_3b_parallel_simult_master_too2
      use sizes
      use atoms
      use bath
      use bndstr
...
c            print*,"After all 2body list bcast"
            ...
            if(taskid.eq.master) then
             ...
             call smooth_gradient_polar(1,nmol,0)
            else if(taskid.eq.master1) then
             call gradient_emreal_bcast
            end if
            
c            call smooth_gradient_polar(start,start+offset-1,moli1)
...
c
      ...
      call mpi_bcast(delx,1,mpi_real8,master,
     & mpi_comm_world,ierr)
...
c            call mpi_barrier(mpi_comm_world,ierr)
     ...
     &                   + dev(j,i) + dem(j,i)+dep3b(j,i)
       ...
       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b
              analyticg(istep)=derivs(1,4)
              analyticg_dep3b(istep)=dep3b(1,4) 
              deallocate (derivs)
c              call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
                print*,"Finite Diff TotEng, TotGrad",
     & (efindif(3)-efindif(1))/2.0d0/delx,analyticg(2)
                print*,"Finite Diff PolEng, PolGrad",
     & (efindif_ep3b(3)-efindif_ep3b(1))/2.0d0/delx,analyticg_dep3b(2)
