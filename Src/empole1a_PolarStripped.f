...
c
c
      subroutine empole1a_PolarStripped
      use sizes
      use atoms
      use bound
      use boxes
...
c
      ...
      call switch (mode)
c      off2=1.0d12
      print*,"off2 in empole1a_PolarStripped",off2
c
...
c               call torque (i,ttm2,ttm2i,frcxi,frcyi,frczi)
               call torque_pol (i,ttm2,ttm2i,frcxi,frcyi,frczi)
c
...
c               call torque (k,ttm3,ttm3i,frcxk,frcyk,frczk)
               call torque_pol (k,ttm3,ttm3i,frcxk,frcyk,frczk)
c
...
c               vir(3,3) = vir(3,3) + vzz
     ...
     &                  + zky*frcyk(3) + zkz*frczk(3)
               ...
               virpol(1,1) = virpol(1,1) + vxx
               virpol(2,1) = virpol(2,1) + vyx
               virpol(3,1) = virpol(3,1) + vzx
               virpol(1,2) = virpol(1,2) + vyx
               virpol(2,2) = virpol(2,2) + vyy
               virpol(3,2) = virpol(3,2) + vzy
               virpol(1,3) = virpol(1,3) + vzx
               virpol(2,3) = virpol(2,3) + vzy
               virpol(3,3) = virpol(3,3) + vzz
