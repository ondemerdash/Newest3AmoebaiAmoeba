...
c     &    3*maxsizeclust,clustcount))
     ...
     &    3*maxsizeclust,clustcount))
        if(.not. allocated(ep2bmat)) 
     &           allocate(ep2bmat(clustcount,clustcount))
        if(.not. allocated(dep2bmat)) allocate(dep2bmat(3,
     &    2*3*maxsizeclust,clustcount,clustcount))
        if(.not. allocated(virep2bmat)) allocate(virep2bmat(3,
     &    3,clustcount,clustcount))
        if(.not. allocated(em2bmat)) 
     &        allocate(em2bmat(clustcount,clustcount))
        if(.not. allocated(dem2bmat)) allocate(dem2bmat(3,
     &    2*3*maxsizeclust,clustcount,clustcount))
       end if
      else
...
c            listsend_vdw = .false.
           !allocate (taskidvdw(0:numtasks_vdw2-1))
     ...
     &            vdw_comm,ierr)
           deallocate(taskidvdw)
            ...
            call grad_cov_v_reduce_commsmall

            call clust_gradient_PolarPerm1b2bsimult_reducemat_commsmall(
     &      start,start+offset-1, moli1rmndr)
            !end if
