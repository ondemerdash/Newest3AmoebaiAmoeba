...
c     &  ntpair_dEtmp,dEindextmp,dEd1tmp,dEd2tmp,dEp1tmp,dEp2tmp)
      subroutine thg5_small_vir_nolistsend_Nn_notenssends
      use sizes
      use atoms
      use boxes
      use chgpot
...
c!$OMP& toffset0,dEindextmp,ntpair_dE,dEd1,dEd2,dEp1,dEp2,
...
c!$OMP& frcztau1dtot, frcztau1ptot,frcxtau1dtot,frcxtau1ptot,
c!$OMP& frcytau1dtot,frcytau1ptot,
c!$OMP& frcztau2dtot,frcztau2ptot,frcxtau2dtot,frcxtau2ptot,
c!$OMP& frcytau2dtot,frcytau2ptot,
c!$OMP& sz,elstgrad,elsttau1,elsttau2,nlocal,elsttau1_index,
c!$OMP& elsttau2_index,
c!$OMP& nlocaltau1,nlocaltau2)
...
c          mscale(i15(j,ii)) = m5scale
        ...
        !   k = elst_recv(kkk,iter3)
          kk = ipole(k)
            ...
            kx = xaxis(k)
            ky = yaxis(k)
c            usek = (use(kk) .or. use(kz) .or. use(kx) .or. use(ky))
c            proceed = .true.
c            if (use_group)  call groups (proceed,fgrp,ii,kk,0,0,0,0)
c            if (.not. use_intra)  proceed = .true.
c            if (proceed)  proceed = (usei .or. usek)
c            if (.not. proceed)  goto 10
          xr = x(kk) - x(ii)
          yr = y(kk) - y(ii)
          zr = z(kk) - z(ii)
          call image (xr,yr,zr)
...
c
     ...
     &          +0.5d0*bn(4)*sc(6)) + fact1*dk(1))
            t2xyd = t2xyd + xr*(yr*(fact1*ck-fact2*sc(4)
     &        +0.5d0*bn(4)*sc(6)) + fact1*dk(2))
            t2xzd = t2xzd + xr*(zr*(fact1*ck-fact2*sc(4)
     &         +0.5d0*bn(4)*sc(6)) + fact1*dk(3))
     ...
     &          +0.5d0*bn(4)*sc(5)) - fact1*di(1))
            t1xyd = t1xyd - xr*(yr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn(4)*sc(5)) - fact1*di(2))
            t1xzd = t1xzd - xr*(zr*(fact1*ci +fact2*sc(3)
     &          +0.5d0*bn(4)*sc(5)) - fact1*di(3))
...
c!$OMP CRITICAL
           ...
           call torque3_dEtensor_72015_taunew3_pt1_vir_notenssends(i,
     &  ttm2i,tau1d,tau1p,nlocaltau1,
     &  nlocal,
     &  k,kkk)
c
...
c
     ...
     &  !frcxtaulocal2d,frcxtaulocal2p,frcytaulocal2d,frcytaulocal2p,k)
           ...
           call torque3_dEtensor_72015_taunew3_pt2_vir_notenssends(i,
     &  ttm3i,tau2d,tau2p,nlocaltau2,
     &  nlocal,
     &  k,kkk)
c!$OMP END CRITICAL
