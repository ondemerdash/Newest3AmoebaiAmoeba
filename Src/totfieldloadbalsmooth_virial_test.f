...
c
c
      program dir1bewtotfield_dyn_rMPIOMP_kBcast_rloadbal
      use sizes
      use atoms
      use bath
      use bndstr
...
c            print*,"After all 2body list bcast"
     ...
     &             master1,emreal_comm,ierr)
            ...
            if(taskid.le.numtasks_emreal-1) then
             call gradient_emreal_reduce_load_balanced_totfield_uindcalc
            end if
...
c            if(approxmode.eq.'1BODYMODE') then
             call ewtotfieldsmallsmooth2_gradient_polar_1body_multiter2
c            else
c            call ewtotfieldsmallsmooth2_gradient_polar_load_balanced
c            end if
...
c            listsend_mpole=.false.
     ...
     &       mpi_comm_world,ierr)
              ...
              if(taskid.eq.master) then
                 call empole1d_3b_PolarPerm_selfeng_bcast

                 do i=1,3
                   do j=1,3
                    vir(i,j)=vir(i,j)+virep3b(i,j)+viremreal_tmp(i,j)
     &                       + viremrecip(i,j)+virep3bmut(j,i)
       print*,"j i virep3b(j,i)",j,i,virep3bmut(j,i)
                   end do
                 end do
     ...
     &                   + ev + em+ep3b+ep3bmut
               ...
               print*,"ep3b in verlet/nose/beeman setup",ep3bmut
...
c                  print*,"dep3b_x",i,dep3b(1,i)+dep3bmut(1,i)
c                  print*,"dep3b_y",i,dep3b(2,i)+dep3bmut(2,i)
c                  print*,"dep3b_z",i,dep3b(3,i)+dep3bmut(3,i)

                  print*,"dep3b_x",i,dep3bmut(1,i)
                  print*,"dep3b_y",i,dep3bmut(2,i)
                  print*,"dep3b_z",i,dep3bmut(3,i)
...
c     & mpi_comm_world,ierr)
         ...
         if (integrate .eq. 'VERLET') then
          do istep = 1, nstep
            ...
            if(taskid.eq.master) then 
             ! Commented Out for Debug!!call verlet_pt1 (istep,dt)
c              y(6)=y(6)+delx
             call ptest_parallel
...
C  PERFORM LOAD BALANCING FOR REAL-SPACE PERM ELEC BASED ON THE NUMBER OF NEIGHBORS OF EACH OUTER LOOP MOLECULE INDEX
     ...
     &       mpi_comm_world,ierr)
            ...
            !  if(use_ewald) then
            !    call dfield0c_totfield
            !  else 
            !    call dfield0a_totfield
            !  end if
            !end if
...
C THEREFORE, LIST MUST BE BROADCAST. ALSO BROADCAST VARIABLES TO DO WITH LOAD BALANCING
     ...
     &             master1,emreal_comm,ierr)
            ...
            if(taskid.eq.master) then
             ...
             call gradient_emrecip_totfield
cccc            end if
c            else if(taskid.eq.master2) then
...
c            call mpi_barrier(mpi_comm_world,ierr)
     ...
     &                   + ev + em+ep3b+ep3bmut
              ...
              efindif_ep3b(istep)=ep3bmut!+ep3b
     ...
     &                 + dev(j,i) + dem(j,i)+dep3b(j,i)+dep3bmut(j,i)
       ...
       print*," istep Vdw,PermElec,Polz",istep,ev,em,ep3bmut!+ep3b
              analyticg_dep3bx(istep)=dep3bmut(1,6)!+dep3b(1,6)

c              analyticgy(istep)=derivs(2,6)
              analyticg_dep3by(istep)=dep3bmut(2,6)!+dep3b(2,6)

c              analyticgz(istep)=derivs(3,6)
              analyticg_dep3bz(istep)=dep3bmut(3,6)!+dep3b(3,6)

              if(istep.eq.2) then
                do i=1,3
                   do j=1,3
                 vir_analytic(j,i)=vir(j,i)-virep3b(j,i)-virep3bmut(j,i)
                 virep3b_analytic(j,i)=virep3bmut(j,i)!+virep3b(j,i)
                print*,"Step 2 virep3b",j,i,virep3bmut(j,i)!+virep3b(j,i)
                   end do
                end do
              ...
              deallocate (derivs)
               ! Commented Out for Debug!!call verlet_pt2(istep,dt,energy)
              if(istep.eq.3) then
      dedv_vir = (vir_analytic(1,1)+vir_analytic(2,2)+vir_analytic(3,3))
     &         / (3.0d0*volbox)
              dedv_fd = (efindif(3)-efindif(1))
     &        / (2.0d0*delta*volbox)

      dedv_vir_ep3b = (virep3b_analytic(1,1)+virep3b_analytic(2,2)
     &          +virep3b_analytic(3,3))/ (3.0d0*volbox)
      dedv_fd_ep3b =(efindif_ep3b(3)-efindif_ep3b(1))
     &         / (2.0d0*delta*volbox)

      write (iout,31)  dedv_vir
   31 format (/,'PairwsdE/dV(Virial-based):',f27.16,' Kcal/mol/A**3')
      write (iout,32)  dedv_fd
   32 format ('PairwsdE/dV(Finite Diff):',f27.16,' Kcal/mol/A**3')
      write (iout,33)  dedv_vir_ep3b
   33 format (/,'PolzdE/dV (Virial-based):',f27.16,' Kcal/mol/A**3')
      write (iout,34)  dedv_fd_ep3b
   34 format ('PolzdE/dV (Finite Diff) :',f27.16,' Kcal/mol/A**3')
              end if
