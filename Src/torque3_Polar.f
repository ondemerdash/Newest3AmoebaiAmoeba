...
c
c
      subroutine torque3_Polar (i,trq1,trq2,frcx,frcy,frcz,depi)
      use sizes
      use atoms
      use deriv
      use mpole
...
c
c      dphidu = -trq1(1)*u(1) - trq1(2)*u(2) - trq1(3)*u(3)
c      dphidv = -trq1(1)*v(1) - trq1(2)*v(2) - trq1(3)*v(3)
c      dphidw = -trq1(1)*w(1) - trq1(2)*w(2) - trq1(3)*w(3)
c      if (axetyp .eq. 'Z-Bisect') then
c         dphidr = -trq1(1)*r(1) - trq1(2)*r(2) - trq1(3)*r(3)
c         dphids = -trq1(1)*s(1) - trq1(2)*s(2) - trq1(3)*s(3)
c      end if
c
...
c            demi(j,ia) = demi(j,ia) + du
c            demi(j,ib) = demi(j,ib) - du
c            frcz(j) = frcz(j) + du
c         end do
c
...
c            demi(j,ia) = demi(j,ia) + du
c            demi(j,ic) = demi(j,ic) + dv
c            demi(j,ib) = demi(j,ib) - du - dv
c            frcz(j) = frcz(j) + du
c            frcx(j) = frcx(j) + dv
c         end do
c
...
c         do j = 1, 3
c            du = ur(j)*dphidr/(usiz*ursin) + us(j)*dphids/usiz
c            dv = (vssin*s(j)-vscos*t1(j))*dphidu
c     &              / (vsiz*(ut1sin+ut2sin))
c            dw = (wssin*s(j)-wscos*t2(j))*dphidu
c     &              / (wsiz*(ut1sin+ut2sin))
c            demi(j,ia) = demi(j,ia) + du
c            demi(j,ic) = demi(j,ic) + dv
c            demi(j,id) = demi(j,id) + dw
c            demi(j,ib) = demi(j,ib) - du - dv - dw
c            frcz(j) = frcz(j) + du
c            frcx(j) = frcx(j) + dv
c            frcy(j) = frcy(j) + dw
c         end do
...
c         do j = 1, 3
c            du = uw(j)*dphidw/(usiz*uwsin)
c     &              + uv(j)*dphidv/(usiz*uvsin)
c     &              - uw(j)*dphidu/(usiz*uwsin)
c     &              - uv(j)*dphidu/(usiz*uvsin)
c            dv = vw(j)*dphidw/(vsiz*vwsin)
c     &              - uv(j)*dphidu/(vsiz*uvsin)
c     &              - vw(j)*dphidv/(vsiz*vwsin)
c     &              + uv(j)*dphidv/(vsiz*uvsin)
c            dw = -uw(j)*dphidu/(wsiz*uwsin)
c     &              - vw(j)*dphidv/(wsiz*vwsin)
c     &              + uw(j)*dphidw/(wsiz*uwsin)
c     &              + vw(j)*dphidw/(wsiz*vwsin)
c            du = du / 3.0d0
c            dv = dv / 3.0d0
c            dw = dw / 3.0d0
c            demi(j,ia) = demi(j,ia) + du
