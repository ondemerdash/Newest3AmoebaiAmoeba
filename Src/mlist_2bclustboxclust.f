...
c
c
      subroutine mlist_2bclustboxclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use boxes
...
c
      ...
      if (domlstclust2b(counter)) then
         domlstclust2b(counter) = .false.
         if (octahedron) then
            call mbuild_2bclustboxclust(npole3b,pnum,counter)
         else
            call mlight_2bclustboxclust(npole3b,pnum,counter)
         end if
         return
      end if
c
...
!$OMP DO schedule(guided)
      do l1=1,npole3b
         ...
         yi = y(ii)
         zi = z(ii)
         xr = xi - xmold2b(l1,counter)
         yr = yi - ymold2b(l1,counter)
         zr = zi - zmold2b(l1,counter)
...
c            listsend_mpole =.true.
            update(l1) = .true.
            xmold2b(l1,counter) = xi
            ymold2b(l1,counter) = yi
            zmold2b(l1,counter) = zi
            nelst2b(l1,counter) = 0
           ! do k = i+1, npole
            do l3=l1+1,npole3b
               k=pnum(l3)
               kk = ipole(k)
               ...
               if (r2 .le. mbuf2clust) then
                  nelst2b(l1,counter) = nelst2b(l1,counter) + 1
                  elst2b(nelst2b(l1,counter),l1,counter) = l3
               end if
            end do
         end if
      end do
...
c            do k = 1, i-1
            do l3 = 1, l1-1
               if (.not. update(l3)) then
                  xr = xi - xmold2b(l3,counter)
                  yr = yi - ymold2b(l3,counter)
                  zr = zi - zmold2b(l3,counter)
                  call imagen2b (xr,yr,zr,counter)
                  r2 = xr*xr + yr*yr + zr*zr
                  if (r2 .le. mbuf2clust) then
!$OMP CRITICAL
                     do j = 1, nelst2b(l3,counter)
                        if (elst2b(j,l3,counter) .eq. l1)  goto 20
                     end do
                     nelst2b(l3,counter) = nelst2b(l3,counter) + 1
                     elst2b(nelst2b(l3,counter),l3,counter) = l1
   20                continue
!$OMP END CRITICAL
                  else if (r2 .le. mbufxclust) then
!$OMP CRITICAL
                     do j = 1, nelst2b(l3,counter)
                        if (elst2b(j,l3,counter) .eq. l1) then
                           elst2b(j,l3,counter) = 
     &                       elst2b(nelst2b(l3,counter),l3,counter)
                           nelst2b(l3,counter) = nelst2b(l3,counter) - 1
                           goto 30
                        end if
                     end do
...
!$OMP DO schedule(guided)
      do i = 1, npole3b
         if (nelst2b(i,counter) .ge. maxelst) then
            write (iout,40)
...
c
c
      subroutine mbuild_2bclustboxclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use iounit
...
c
c
      subroutine mlight_2bclustboxclust(npole3b,pnum,counter)
      use sizes
      use atoms
      use bound
      use cell
...
c
      do l1 = 1, npole3b
         ...
         nelst2b(l1,counter) = 0
         ii = ipole(i)
         xmold2b(l1,counter) = x(ii)
         ymold2b(l1,counter) = y(ii)
         zmold2b(l1,counter) = z(ii)
         xsort(l1) = x(ii)
         ysort(l1) = y(ii)
         zsort(l1) = z(ii)
      end do
...
c      do i = 1, npole
   ...
   10    continue
         do j = start, stop
            ...
            if (r2 .le. mbuf2clust) then
               nelst2b(l1,counter) = nelst2b(l1,counter) + 1
               elst2b(nelst2b(l1,counter),l1,counter) = k
            end if
   20       continue
         end do
         if (repeat) then
...
c     check to see if the neighbor list is too long
c
         if (nelst2b(l1,counter) .ge. maxelst) then
            write (iout,30)
