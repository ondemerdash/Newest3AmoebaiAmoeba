...
c
c
      subroutine mdstat_propertiesDaveDeTo (istep,dt,etot,epot,ekin,
     & temp,pres)
      use sizes
      use atoms
      use bath
...
c
     ...
     &    - atmsph*vol_ave/prescon
         ...
         isothermal_k = (vol2_ave - vol_ave**2)*1000000
     &                 /(gasconst*temp_ave*vol_ave*prescon)

         heat_cap = (h2_ave - h_ave**2)*1000
     &             /(gasconst*temp_ave**2*nmol)

         alpha_coeff = (hv_ave - vol_ave*h_ave)*10000
  ...
  370    format(' Isothermal Compress.', 9x, f19.8, ' 10^-6 <Atm^-1>') 


         write(iout,380) heat_cap
  380    format(' Heat Capacity', 16x, f19.8, ' <cal/(mol*K)>') 


         write(iout,390) alpha_coeff
  390    format(' Thermal Expansioon Coeff', 5x, f19.8, 
     &          ' 10^-4  <Kelvin^-1>') 
      end if
...
c
      ...
      xdpl_sum = xdpl_sum + xdpl
      xdpl2_sum = xdpl2_sum + xdpl**2
      ydpl_sum = ydpl_sum + ydpl
      ydpl2_sum = ydpl2_sum + ydpl**2
      zdpl_sum = zdpl_sum + zdpl
      zdpl2_sum = zdpl2_sum + zdpl**2
      
      if (modstep .eq.0) then

         dpl2_ave = xdpl2_sum/dble(iprint) - xdpl_sum**2/dble(iprint)**2
     &    +  ydpl2_sum/dble(iprint) - ydpl_sum**2/dble(iprint)**2
     &    +  zdpl2_sum/dble(iprint) - zdpl_sum**2/dble(iprint)**2

         diel = 1 + 4*pi*dpl2_ave
     &    /(3*vol_ave*boltzmann*kelvin)
         
         write(iout, 400) diel
  400    format(' Dielectric Constant', 6x, f19.8, ' <e0>')
