...
c                end if
            ...
            if(.not.inputkmeansclust) then
            call mpi_bcast(clust,4*nmol,mpi_integer,master,
     &          mpi_comm_world,ierr)
...
c            listbcast = .false.
            ...
            if((taskid.lt.numtasks_polar1).and.mlistclust) then
              if(useboxclust) then
                !call clust1blatticeboxes(start,start+offset-1, 
     &          !     moli1rmndr)
              end if
              if(useboxclust) then
               ! call clust1blistsboxclust(start,start+offset-1, 
     &         !      moli1rmndr)
              else
...
c     &             master1,mpi_comm_world,ierr)
            call mpi_barrier(mpi_comm_world,ierr)
c            if(firstload_emreal) then
c               firstload_emreal = .false.
c              call mpi_bcast(numtasks_emreal2,1,mpi_integer,
c     &             master2,mpi_comm_world,ierr)
c               call mpi_bcast(start_emreal2,numtasks_emreal,mpi_integer,
c     &             master2,mpi_comm_world,ierr)
c               call mpi_bcast(last_emreal2,numtasks_emreal,mpi_integer,
c     &             master2,mpi_comm_world,ierr)
c            end if
...
c            listsend_vdw = .false.
            ...
            if(all2bclust) then
             call clust_gradient_PolarPerm1b2ball(start,
     &        start+offset-1, moli1rmndr)
            else
             call clust_gradient_PolarPerm1b2b(start,
     &        start+offset-1, moli1rmndr)
            end if
