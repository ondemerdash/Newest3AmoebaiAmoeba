...
c            print*,"taskid",taskid,"listsend_vdw",listsend_vdw
     ...
     &             master,mpi_comm_world,ierr)
               call mpi_bcast(maxsize_vlst,numtasks_vdw,mpi_integer,
     &             master,mpi_comm_world,ierr)
               call sendvlist_load_balanced3_sizelist_half
            end if
            listsend_vdw = .false.
...
c            call bcast_lattice
      ...
      call system_clock(t2,clock_rate)
      tot_time =  (t2-t1)/real(clock_rate)
      print*,"Timing vdw_sizelist_only",tot_time
              end if
...
C  PERFORM LOAD BALANCING FOR REAL-SPACE PERM ELEC BASED ON THE NUMBER OF NEIGHBORS OF EACH OUTER LOOP MOLECULE INDEX
      ...
      call system_clock(t2,clock_rate)
      tot_time =  (t2-t1)/real(clock_rate)
      print*,"Timing emreal_sizelist_only",tot_time
              end if
            else if(taskid.eq.master2
     &              .and.(approxmode.ne.'1BODYMODE')) then
                call mollist2bodyOO6_8_par
...
!$OMP END PARALLEL DO
      ...
      call system_clock(t2,clock_rate)
      tot_time =  (t2-t1)/real(clock_rate)
      print*,"Timing Verlet fullstep v",tot_time
