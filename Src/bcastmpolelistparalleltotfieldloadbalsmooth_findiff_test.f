...
C  PERFORM LOAD BALANCING FOR REAL-SPACE PERM ELEC BASED ON THE NUMBER OF NEIGHBORS OF EACH OUTER LOOP MOLECULE INDEX
                ...
                ! firstload_emreal = .false.
                 call emreal_load_balance_sizelist2
                !end if
...
c            print*,"After all 2body list bcast"
     ...
     &       !      master1,emreal_comm,ierr)
               call mpi_bcast(maxsize_elst2,1,mpi_integer,
     &             master1,emreal_comm,ierr)
               print*,"maxsize_elst2=",maxsize_elst2
               if(.not.allocated(nelst) ) allocate (nelst(n))
              if(.not.allocated(elst) ) allocate (elst(maxelst,n))
               call mpi_bcast(nelst,n,mpi_integer,
     &             master1,emreal_comm,ierr)
               call mpi_bcast(elst,maxelst*n,mpi_integer,
     &             master1,emreal_comm,ierr)
...
c              call gradient_emreal_bcast
            ...
            if(taskid.le.numtasks_emreal-1) then
             call gradient_emreal_reduce_load_balanced_totfield_bcast
            end if
