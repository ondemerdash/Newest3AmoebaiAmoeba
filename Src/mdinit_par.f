...
c         if (.not. allocated(vnopolz)) allocate (vnopolz(3,n))
c         if (.not. allocated(anopolz)) allocate (anopolz(3,n))
      end if
...
c       END OF MDINIT_PT1_MASTER
c
c     try to restart using prior velocities and accelerations
c
c      dynfile = filename(1:leng)//'.dyn'
c      call version (dynfile,'old')
c      inquire (file=dynfile,exist=exist)
c      if (exist) then
c         idyn = freeunit ()
c         open (unit=idyn,file=dynfile,status='old')
c         rewind (unit=idyn)
c         call readdyn (idyn)
c         close (unit=idyn)
c
...
c
c      else if (integrate .eq. 'RIGIDBODY') then
c         do i = 1, ngrp
c            speed = maxwell (grpmass(i),kelvin)
c            call ranvec (vec)
c            do j = 1, 3
c               vcm(j,i) = speed * vec(j)
c               wcm(j,i) = 0.0d0
c               lm(j,i) = 0.0d0
c            end do
c         end do
...
c
c      else if (integrate .eq. 'RESPA') then
c         allocate (derivs(3,n))
c         call gradslow (e,derivs)
c         do i = 1, n
c            if (use(i) .and. mass(i).ne.0.0d0) then
c               speed = maxwell (mass(i),kelvin)
c               call ranvec (vec)
c               do j = 1, 3
c                  v(j,i) = speed * vec(j)
...
c            end if
c         end do
c         call gradfast (e,derivs)
c         do i = 1, n
c            if (use(i) .and. mass(i).ne.0.0d0) then
c               do j = 1, 3
c                  aalt(j,i) = -convert * derivs(j,i) / mass(i)
c               end do
c            else
c               do j = 1, 3
c                  aalt(j,i) = 0.0d0
