...
c      maxlocal = int(dble(sz)*dble(maxsize_elst(taskid))/dble(nthread))
      ...
      do i=start_emreal2(taskid),last_emreal2(taskid)
           ...
           maxlocal=maxlocal+nelst_recv(j)
      end do 
      maxlocal = int(dble(maxlocal)/dble(nthread))
      maxlocaltau=3*maxlocal
      !maxlocal = int(dble(sz)*dble(maxelst)/dble(nthread))
      !maxlocaltau=3*maxlocal
c       print*,"THG5"

      !print*,"In Tens taskid=",taskid,"maxlocal=",maxlocal
      !print*,"In Tens taskid=",taskid,"maxlocaltau=",maxlocaltau
c
...
!$OMP& toffset0tau2,maxlocaltau,frcxtau1d,frcytau1d,frcztau1d,
!$OMP& frcxtau1p,frcytau1p,frcztau1p,
!$OMP& frcxtau2d,frcytau2d,frcztau2d,
!$OMP& frcxtau2p,frcytau2p,frcztau2p)
