...
c
c
      program dEtensorewtotfield_dyn_rMPIOMP_kBcast_rloadbal_vMPIOMP
      use sizes
      use atoms
      use bath
      use bndstr
...
C BROADCAST SOME LOGICALS.
     ...
     & mpi_comm_world,ierr)
      ...
      call mpi_bcast(firstload_vdw,1,mpi_logical,master,
     & mpi_comm_world,ierr)
...
c      if (.not. allocated(pollist))  allocate (pollist(n))
     ...
     &       allocate (epsilon4(maxclass,maxclass))
      ...
      if (.not. allocated(ibnd)) allocate(ibnd(2,nbond))
      if (.not. allocated(bk)) allocate(bk(nbond))
      if (.not. allocated(bl)) allocate(bl(nbond))

      if (.not. allocated(iang)) allocate(iang(4,nangle))
...
c       print*,"Doing uind allocs"
     ...
     &          allocate (start_vdw2(0:numtasks-1))
         if (.not. allocated(last_vdw2))
     &          allocate (last_vdw2(0:numtasks-1))
         if (.not. allocated(maxsize_vlst))
     &         allocate (maxsize_vlst(0:numtasks-1))
...
c            call mpi_barrier(mpi_comm_world,ierr)
            ...
            call alloc_erecip
c            call mpi_bcast(x,n,mpi_real8,master,
c     &      mpi_comm_world,ierr)
c            call mpi_bcast(y,n,mpi_real8,master,
c     &      mpi_comm_world,ierr)
c            call mpi_bcast(z,n,mpi_real8,master,
c     &      mpi_comm_world,ierr)
...
c     &       mpi_comm_world,ierr)
            call prep_pole
               ...
               call vlist_par
c              if(firstload_vdw.and.(taskid.eq.numtasks_emreal-1)) then
              if(firstload_vdw) then
                call vdw_load_balance_sizelist
                firstload_vdw=.false.
              end if
               call mlist_par
c                if(firstload_emreal.and.(taskid.eq.master1)) then
...
c            if(firstload_vdw) then
c                firstload_vdw=.false.
c              call mpi_bcast(numtasks_vdw2,1,mpi_integer,
c     &            numtasks_emreal-1,mpi_comm_world,ierr)
c              call mpi_bcast(start_vdw2,numtasks_vdw,mpi_integer,
c     &             numtasks_emreal-1,mpi_comm_world,ierr)
c               call mpi_bcast(last_vdw2,numtasks_vdw,mpi_integer,
c     &             numtasks_emreal-1,mpi_comm_world,ierr)
c            end if
...
c            end if
c              call gradient_emrecip_totfield_send


             call gradient_vdw_reduce_load_balanced_allreduce

c            if(taskid.le.numtasks_emreal-1) then
         call gradientemrealallreduce_loadbalanced_totfield_dEtensorV
c            end if
c            call mpi_barrier(mpi_comm_world,ierr)

c            if(approxmode.eq.'1BODYMODE') then
c            call mpi_bcast(uind,3*n,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
c            call mpi_bcast(uinp,3*n,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
c            call mpi_bcast(ep3b,1,mpi_real8,master1,
c     &       mpi_comm_world,ierr)
c            else
...
c            if(approxmode.eq.'1BODYMODE') then
           call ewtotfieldsmooth2_gradient_polar_1body_dEtensor_mpiall
c            else
...
c              if(taskid.eq.master) then
     ...
     &                       + viremrecip(j,i)+virev(j,i)
                   if(taskid.eq.master) then
                   print*,"i=",i,"j=",j,"virep3b(i,j)",virep3b(i,j)
                   end if
                   end do
                 end do
...
c       print*," istep Vdw, PermElec Eng, Pol Eng",istep,ev,em,ep3b
...
C THEREFORE, LIST MUST BE BROADCAST. ALSO BROADCAST VARIABLES TO DO WITH LOAD BALANCING
     ...
     &             master1,emreal_comm,ierr)
            ...
            call gradient_emreal_reduce_load_balanced_totfield_dEtensorV
            end if
