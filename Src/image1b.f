...
c
c
      subroutine image1b (xr,yr,zr,cnt)
      use sizes
      use boxes
      use cell
      use boxes1bclust
...
c      if (orthogonal) then
         do while (abs(xr) .gt. xcell2_1b(cnt))
            xr = xr - sign(xcell1b(cnt),xr)
         end do
         do while (abs(yr) .gt. ycell2_1b(cnt))
            yr = yr - sign(ycell1b(cnt),yr)
         end do
         do while (abs(zr) .gt. zcell2_1b(cnt))
            zr = zr - sign(zcell1b(cnt),zr)
         end do
