...
C  ARRAYS DEFINED IN MASTER IN VARIOUS ROUTINES CALLED IN 'mechanic_pme4' THAT
C  NEED TO BE ALLOCATED ON OTHER TASKS BEFORE THEY ARE BROADCAST FROM MASTER
...
C PERFORM CRUDE LOADBALANCING FOR REAL-SPACE ELECTROSTATIC CALC       
...
C  CALLS TO 'bounds' AND TO 'replica' IN 'gradient_setup'
              call gradient_setup  
C  In 'alloc3b_vdwlist_erecip':  ALLOCATE POLARIZATION GRADIENT VECTOR AND PME GRADIENT VECTOR ON
C  MASTER TASK.  INITIALIZE POLARIZATION ENERGY, GRADIENT, AND VIRIAL TO ZERO.
C  INITIALIZE PME ENERGY, GRADIENT, AND VIRIAL TO ZERO. LASTLY, BUILD/UPDATE VDW NEIGHBOR LIST AND
...
C THEREFORE, LIST MUST BE BROADCAST.
            if(listsend_mpole) then
              call sendmlist
            end if
            listsend_mpole = .false.
...
C MASTER1 TASK PERFORMS ENERGY/GRADIENT/VIRIAL CALC OF PARTICLE MESH EWALD. THE VALUE IS 'SENT' BACK
C TO MASTER BY MPI_REDUCE.  TASKS OTHER THAN MASTER1 REDUCE ZERO.
            call gradient_emrecip_reduce
