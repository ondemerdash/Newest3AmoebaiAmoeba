...
c      end if
      ...
      print*,"Successful completion of mdinit_pt1_master on all tasks"
...
C BROADCAST COORDINATES AT EACH STEP
     ...
     &      mpi_comm_world,ierr)
            call mpi_bcast(z,n,mpi_real8,master,
     &      mpi_comm_world,ierr)
            call mpi_bcast(dt,1,mpi_real8,master,
     &      mpi_comm_world,ierr)
...
c              call verlet_pt2(istep,dt,energy)
            ...
            call mpi_bcast(energy,1,mpi_real8,master,
     &       mpi_comm_world,ierr)

            call verlet_pt2_pmonte(istep,dt,energy)
          end do 
         end if 
