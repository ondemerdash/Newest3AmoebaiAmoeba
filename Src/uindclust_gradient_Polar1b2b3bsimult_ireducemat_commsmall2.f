      subroutine uindclst_gradientPolar1b2b3bsimult_ireducematcommsmall2
     & (start,
     & last,moli1rmndr)
      use mpole
      use mpidat
      ...
      real*8, allocatable :: uind1bt(:,:)
      real*8, allocatable :: uind2bt(:,:)
      real*8, allocatable :: uind3bt(:,:)
      real*8, allocatable :: uind1btmat(:,:,:)
      real*8, allocatable :: uind2btmat(:,:,:)
...
c            virem3bt(j,i)=0.0d0
      ...
      allocate(uind2btmat(3,2*3*maxsizeclust,num2bsave))
...
c           print*,"taskid=",taskid,"1body Computation Cost=",t2-t1
     ...
     &   .and.(taskid.lt.(numtasks_polar+numtasks_polar1))) then
c          if(mlistclust) then
c           t5=mpi_Wtime()
          call uindClustEmpole1plz2b_NoOmplistRtrn2BmatNoSubtr1bSimult(
     &   !    ep2bt,virep2bt,dep2bt,ntript,em2bt,dem2bt,ep2btmat,
     &   !    dep2btmat,virep2btmat)
     &       ep2bt,virep2bt,dep2bt,ntript,ep2btmat,
     &       dep2btmat,virep2btmat,uind2bt,uind2btmat)
c           t6=mpi_Wtime()
...
c          end if
     ...
     &    .and. (taskid.lt.(numtasks_polar+numtasks_polar1
     &         +numtasks_polar3b))) then 
        call uindClustEmpole1plz_3b_NoOmplistRtrn3BNoSubtr1b2bSimult(
     &       ep3bt,virep3bt,dep3bt,ntript,uind3bt)
...
c            t3=mpi_Wtime()
     ...
     &      mpi_real8,mpi_sum,master,emreal_comm,reqs21,ierr)
       ...
       call mpi_reduce(uind1btmat,uind1bmat,
     &      3*3*maxsizeclust*clustcount,mpi_real8,
...
c     &       mpi_sum,master,emreal_comm,reqs23,ierr)
     ...
     &          mpi_sum,master,emreal_comm,reqs23,ierr)
                  ...
                  call mpi_reduce(uind1bt,uind1b,npole*3,mpi_real8,
     &          mpi_sum,master,emreal_comm,ierr)
...
c      deallocate(dem1bt)
     ...
     &          mpi_sum,master,emreal_comm2,reqs27,ierr)
                  ...
                  call mpi_reduce(uind2bt,uind2b,npole*3,mpi_real8,
     &          mpi_sum,master,emreal_comm2,ierr)
...
c     &          mpi_sum,master,emreal_comm2,ierr)
                  ...
                  call mpi_ireduce(ep2btmat,ep2bmatmod,num2bsave,
     &            mpi_real8,mpi_sum,master,emreal_comm2,reqs28,ierr)
                  call mpi_ireduce(dep2btmat,dep2bmatmod,
     &                3*2*3*maxsizeclust*num2bsave,mpi_real8,
     &                mpi_sum,master,emreal_comm2,reqs29,ierr)
                  call mpi_ireduce(virep2btmat,virep2bmatmod,
     &                3*3*num2bsave,mpi_real8,
     &                mpi_sum,master,emreal_comm2,reqs30,ierr)

                  call mpi_reduce(uind2btmat,uind2bmatmod,
     &                3*2*3*maxsizeclust*num2bsave,mpi_real8,
     &                mpi_sum,master,emreal_comm2,ierr)
        deallocate(dep2bt)
        deallocate(uind2bt)
c        deallocate(dem2bt)
     ...
     &      mpi_sum,master,emreal_comm3,reqs31,ierr)
           call mpi_ireduce(dep3bt,dep3b,npole*3,mpi_real8,
     &     mpi_sum,master,emreal_comm3,reqs32,ierr)
           call mpi_ireduce(virep3bt,virep3b,3*3,mpi_real8,
     &     mpi_sum,master,emreal_comm3,reqs33,ierr)

           call mpi_reduce(uind3bt,uind3b,npole*3,mpi_real8,
     &     mpi_sum,master,emreal_comm3,ierr)
      deallocate(dep3bt)
      deallocate(uind3bt)
      end if
      return
      ...
      subroutine uindsum_gradient_Polar1b2b3bsimult_ireducematcommsmall2
c     & (start,
c     & last,moli1rmndr)
      use mpole
      use mpidat
...
c                     dem(j,i) = dem(j,i)-dem1bmat(j,l1,clust1)
                      ...
                      uind2b(j,i) = uind2b(j,i)-uind1bmat(j,l1,clust1)
...
c                      dem(j,i) = dem(j,i)-dem1bmat(j,l1,clust2)
                       ...
                       uind2b(j,i) = uind2b(j,i)-uind1bmat(j,l1,clust2)
     ...
     &                  last_mollst_polar3b(k3,clust1,iter1),2
          ...
          do l1=1,npole3b
             ...
             do j=1,3
                ...
                uind3b(j,i)=uind3b(j,i)-uind2bmatmod(j,l1,ind)
             end do
          end do
          do i=1,3
            do j=1,3
          ...
          do l1=1,np3
             ...
             do j=1,3
                ...
                uind3b(j,i)=uind3b(j,i)-uind2bmatmod(j,l2,ind)
             end do  
          end do
          do i=1,3
            do j=1,3
          ...
          do l1 =1,np1
             ...
             do j=1,3
                ...
                uind3b(j,i)=uind3b(j,i)+uind1bmat(j,l1,clust1)
             end do
          end do
          do i=1,3
             do j=1,3
          ...
          do l1=1,np2
             ...
             do j=1,3
                ...
                uind3b(j,i)=uind3b(j,i)+uind1bmat(j,l1,clust2)
             end do
          end do
          do i=1,3
             do j=1,3
          ...
          do l1=1,np3
             ...
             do j=1,3
                ...
                uind3b(j,i)=uind3b(j,i)+uind1bmat(j,l1,clust3)
             end do
          end do
          do i=1,3
             do j=1,3
      ...
      do i=1,npole
        do j=1,3
           ...
           uind3b(j,i)=uind1b(j,i)+uind2b(j,i)+uind3b(j,i)
        end do
      end do
      do i=1,3
        do j=1,3
      ...
      deallocate(uind2b)
      deallocate(uind1b)
      !print*,"Done with sum_gradient_Polar1b2b3bsimult_ireducemat"
      end if
      return
      end
