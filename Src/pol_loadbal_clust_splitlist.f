      subroutine polar_load_balance_clust_splitlist
      use mpidat
      use neigh3b
      use molcul
      use neigh2clust
     ...
     &          allocate (molnew(clustcount))
      ...
      do i=1,clustcount
        if(nmollst(i).ne.0) then
          ...
          do j=1,nmollst(i)
             print*,"cluster_index=",i,"neighclust",mollst(j,i)
          end do
        end if
      end do
...
c     &   last_polar(i)
     ...
     &          start_mollst_polar(k,j,i),last_mollst_polar(k,j,i)
      ...
      subroutine bcast_polar_load_balance_clust_splitlist
      use mpidat
      use molcul
      use neigh2clust
      implicit none
      ...
      call mpi_bcast(max_nmollst,1,mpi_integer,master,
     &   mpi_comm_world,ierr)
...
c     &   mpi_comm_world,ierr)
      ...
      call mpi_bcast(start_mollst_polar,numtasks*clustcount*max_nmollst
     &   ,mpi_integer,master,mpi_comm_world,ierr)
      call mpi_bcast(last_mollst_polar,numtasks*clustcount*max_nmollst
     &   ,mpi_integer,master,mpi_comm_world,ierr)
      call mpi_bcast(num_mollst_chunk,numtasks*clustcount
     &   ,mpi_integer,master,mpi_comm_world,ierr)
